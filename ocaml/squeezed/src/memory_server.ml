...
let transfer_reservation_to_domain dbg session_id reservation_id domid =
  wrap dbg (fun () ->
      Xenctrl.with_intf (fun xc ->
          try
            let reservation_id_path =
              reservation_path _service session_id reservation_id
            in
            let kib =
              Client.immediate (get_client ()) (fun xs ->
...
let calculate_boot_time_host_free_memory () =
  ...
  | Some domain0_bytes ->
      ...
      let boot_time_host_free_pages =
        host_free_pages
        + host_scrub_pages
        + Int64.to_nativeint domain0_total_pages
      in
      let boot_time_host_free_kib =
        pages_to_kib (Int64.of_nativeint boot_time_host_free_pages)
      in
...
let calc_constant_boot_time_host_free_memory constant_count_min interval =
  ...
  let rec calc_constant last constant_count =
    ...
    let constant_count =
      match last with
      | Some old_free_memory
        when Int64.(abs (sub free_memory old_free_memory) <= tolerance) ->
          constant_count + 1
      | _ ->
          1
    in
