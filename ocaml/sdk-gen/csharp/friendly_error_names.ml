...
let _ =
            ...
            (List.map
               (fun (x, y) ->
                 `O
                   ...
                   ; ("i8in_error_description_replace_newlines", `String (Str.global_replace (Str.regexp_string "\n") "\n        ///   " y
                   ) )
                   ]
               )
               errors
  ...
  render_file ("FriendlyErrorNames.Designer.mustache", designer_file) json templdir destdir ;
