...
let assoc key pairs =
  try List.assoc key pairs with Not_found -> raise (Missing_key key)
...
let variety_of_string x =
  ...
  | "metadata" ->
      `metadata
  | x ->
      raise (Parse_failure (Printf.sprintf "Unknown variety: %s" x))
...
let funct_of_string x =
  match String.lowercase_ascii x with "root" -> Root | _ -> Unknown

let string_of_funct = function Root -> "root" | _ -> "unknown"

type mode = [`RO | `RW]

let mode_of_string x =
  match String.lowercase_ascii x with
  | "rw" | "w" ->
      `RW
  | "r" ->
      `RO
  | x ->
      raise (Parse_failure (Printf.sprintf "Unknown mode: %s" x))

let string_of_mode = function `RW -> "rw" | `RO -> "r"

type vbd = {device: string; funct: funct; mode: mode; vdi: vdi}
...
let parse_appliance attrs children =
  ...
  let find_element name xs =
    ...
    | element :: _ ->
        element
    | [] ->
        raise (Parse_failure (Printf.sprintf "Failed to find element: %s" name))
  in
  let child_string node =
    ...
    | _ ->
        raise
          (Parse_failure (Printf.sprintf "Failed to find PCData within element"))
  in
  let vmconfig_of_xml node =
    ...
    | Xml.Element ("vm", _, children) ->
        ...
        let memory, vcpus, distrib, distrib_version =
          match find_element "config" children with
        ...
        in
        let default_assoc default key pairs =
          try List.assoc key pairs with Not_found -> default
        in
        ...
        let vbdconfig_of_xml node =
          ...
          | Xml.Element ("vbd", attrs, _) ->
              ...
              {device; funct; mode; vdi}
          | _ ->
              raise (Parse_failure "expected VBD")
        in
    ...
    | _ ->
        raise (Parse_failure "expected VM")
  in
...
let of_xml node =
  ...
  | _ ->
      raise (Parse_failure "expected appliance or vm")
