...
let _ =
  ...
  error Api_errors.vmss_has_vm []
    ~doc:"There is at least one VM assigned to schedule snapshot." ();
  error Api_errors.vm_assigned_to_schedule_snapshot ["vm"; "vmss"]
    ~doc:"This VM is assigned to a schedule snapshot." ();
...
let vm_set_schedule_snapshot = call
  ~name:"set_schedule_snapshot"
  ~in_oss_since:None
  ~in_product_since:rel_dundee
  ~doc:"Set the value of the schedule snapshot field"
  ~params:[Ref _vm, "self", "The VM";
...
let vm =
	...
	field ~writer_roles:_R_VM_POWER_ADMIN ~qualifier:StaticRO ~in_product_since:rel_dundee ~default_value:(Some (VRef (Ref.string_of Ref.null))) ~ty:(Ref _vmss) "schedule_snapshot" "Ref pointing to a schedule snapshot for this VM";
	field ~writer_roles:_R_POOL_OP ~qualifier:StaticRO ~in_product_since:rel_dundee ~default_value:(Some (VBool false)) ~ty:Bool "is_snapshot_from_vmss" "true if this snapshot was created by the schedule snapshot";
...
let vmss_snapshot_now = call ~flags:[`Session]
  ...
  ~in_oss_since:None
  ~in_product_since:rel_dundee
  ~params:[Ref _vmss, "vmss", "Schedule Snapshot to execute";]
  ~doc:"This call executes the schedule snapshot immediately"
  ~allowed_roles:_R_POOL_OP
  ~result:(String, "An XMLRPC result")
  ()
...
let vmss_set_retention_value = call ~flags:[`Session]
  ~name:"set_retention_value"
  ~in_oss_since:None
  ~in_product_since:rel_dundee
  ~allowed_roles:_R_POOL_OP
  ~params:[
...
let vmss_set_frequency = call ~flags:[`Session]
  ...
  ~params:[
    Ref _vmss, "self", "The schedule snapshot";
    vmss_frequency, "value", "the schedule snapshot frequency"
  ]
...
let vmss_set_type = call ~flags:[`Session]
  ...
  ~params:[
    Ref _vmss, "self", "The schedule snapshot";
    vmss_type, "value", "the schedule snapshot type"
  ]
  ()

let vmss =
  create_obj ~in_db:true ~in_oss_since:None ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:true ~name:_vmss ~descr:"VM Schedule Snapshot"
    ~gen_events:true
    ~in_product_since:rel_dundee
    ~doccomments:[]
    ~messages_default_allowed_roles:_R_POOL_OP
  ...
  ~contents:[
    uid _vmss;
    namespace ~name:"name" ~contents:(names None RW) ();
    field ~qualifier:RW ~ty:Bool "enabled" "enable or disable this schedule snapshot" ~default_value:(Some (VBool true));
    field ~qualifier:StaticRO ~ty:vmss_type "type" "type of the snapshot schedule";
    field ~qualifier:StaticRO ~ty:Int "retention_value" "maximum number of snapshots that should be stored at any time" ~default_value:(Some (VInt 7L));
    ...
    field ~qualifier:DynamicRO ~ty:(Set (Ref _vm)) "VMs" "all VMs attached to this schedule snapshot";
  ]
  ()
...
(*  (_alert, "task"), (_task, "alerts"); *)
    ...
    (_vm, "schedule_snapshot"), (_vmss, "VMs");
