...
let _ =
  ...
  error Api_errors.vmss_has_vm []
    ~doc:"There is at least one VM assigned to scheduled snapshot." ();
  error Api_errors.vm_assigned_to_scheduled_snapshot ["vm"; "vmss"]
    ~doc:"This VM is assigned to a scheduled snapshot." ();
...
let vm_set_scheduled_snapshot = call
  ~name:"set_scheduled_snapshot"
  ~in_oss_since:None
  ~in_product_since:rel_dundee
  ~doc:"Set the value of the scheduled snapshot field"
  ~params:[Ref _vm, "self", "The VM";
...
let vm =
	...
	field ~writer_roles:_R_VM_POWER_ADMIN ~qualifier:StaticRO ~in_product_since:rel_dundee ~default_value:(Some (VRef (Ref.string_of Ref.null))) ~ty:(Ref _vmss) "scheduled_snapshot" "Ref pointing to a scheduled snapshot for this VM";
	field ~writer_roles:_R_POOL_OP ~qualifier:StaticRO ~in_product_since:rel_dundee ~default_value:(Some (VBool false)) ~ty:Bool "is_vmss_snapshot" "true if this snapshot was created by the scheduled snapshot";
...
let vmss_snapshot_now = call ~flags:[`Session]
  ...
  ~in_oss_since:None
  ~in_product_since:rel_dundee
  ~params:[Ref _vmss, "vmss", "Scheduled Snapshot to execute";]
  ~doc:"This call executes the scheduled snapshot immediately"
  ~allowed_roles:_R_POOL_OP
  ~result:(String, "An XMLRPC result")
  ()
...
let vmss_set_frequency = call ~flags:[`Session]
  ...
  ~in_product_since:rel_dundee
  ~params:[
    Ref _vmss, "self", "The scheduled snapshot";
    vmss_frequency, "value", "the scheduled snapshot frequency"
  ]
...
let vmss_set_type = call ~flags:[`Session]
  ...
  ~params:[
    Ref _vmss, "self", "The scheduled snapshot";
    vmss_type, "value", "the scheduled snapshot type"
  ]
  ()

let vmss =
  create_obj ~in_db:true ~in_oss_since:None ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:true ~name:_vmss ~descr:"VM Scheduled Snapshot"
    ~gen_events:true
    ~in_product_since:rel_dundee
    ~doccomments:[]
    ~messages_default_allowed_roles:_R_POOL_OP
  ...
  ~contents:[
    uid _vmss;
    namespace ~name:"name" ~contents:(names None RW) ();
    field ~qualifier:RW ~ty:Bool "enabled" "enable or disable this scheduled snapshot" ~default_value:(Some (VBool true));
    field ~qualifier:StaticRO ~ty:vmss_type "type" "type of the scheduled snapshot";
    ...
    field ~qualifier:StaticRO ~ty:vmss_frequency "frequency" "frequency of taking snapshot from scheduled snapshot";
    ...
    field ~qualifier:DynamicRO ~ty:(Set (Ref _vm)) "VMs" "all VMs attached to this scheduled snapshot";
  ]
  ()
...
(*  (_alert, "task"), (_task, "alerts"); *)
    ...
    (_vm, "scheduled_snapshot"), (_vmss, "VMs");
