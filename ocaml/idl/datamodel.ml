  ...
  let t =
    create_obj ~in_db:true ~in_product_since:rel_rio ~in_oss_since:oss_since_303 ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:true ~name:_vm ~descr:"A virtual machine (or 'guest')."
      ...
      ~messages:[ snapshot; snapshot_with_quiesce; clone; copy; revert; checkpoint;
                  ...
                  assert_operation_valid operations _vm _self;
                  update_allowed_operations operations _vm _self;
...
let emergency_calls =
  [ (Pool.t,Pool.slave_reset_master);
    (Pool.t,Pool.transition_to_master);
    (Pool.t,Pool.ping_slave);
    (Session.t,Session.slave_local_login);
    (Session.t,Session.slave_local_login_with_password);
    (Session.t,Session.local_logout);
    (Host.t,Host.propose_new_master);
    (Host.t,Host.commit_new_master);
    (Host.t,Host.abort_new_master);
    (Host.t,Host.local_assert_healthy);
    (Host.t,Host.signal_networking_change);
    (Host.t,Host.local_management_reconfigure);
    (Host.t,Host.ha_xapi_healthcheck);
    (Host.t,Host.emergency_ha_disable);
    (Host.t,Host.management_disable);
    (Host.t,Host.get_system_status_capabilities);
    (Host.t,Host.is_in_emergency_mode);
    (Host.t,Host.shutdown_agent);
  ]
