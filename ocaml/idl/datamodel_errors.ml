...
let _ =
  ...
  error Api_errors.vm_call_plugin_rate_limit ["VM"; "interval"; "wait"]
    ~doc:"There is a minimal interval required between consecutive plug-in calls made on the same VM, please wait before retry." ();
  ...
  error Api_errors.invalid_repository_name ["name"]
    ~doc:"The name of the repository is invalid." ();
