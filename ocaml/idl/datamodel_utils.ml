...
module Types = struct
  ...
  let to_string ty = rpc_of_ty ty |> Rpc.to_string
  ...
  let rec fold_left f accu ty =
    ...
    | Set ty ->
        fold_left f accu ty
    | Map (key, value) ->
        fold_left f (fold_left f accu key) value
    | _ ->
        accu
...
module Relations = struct
  ...
  let other_end_of api ((a, b) as one_end) =
    ...
    | [] ->
        failwith
          (Printf.sprintf "Couldn't find other end of relation (%s,%s)" a b)
    | _ ->
        failwith "Found multiple other ends of relation?!"

  let is_in_relation api x =
    let rels = relations_of_api api in
    List.mem_assoc x rels
...
let find_self_parameter (msg : message) =
  ...
  | _ ->
      failwith
        (Printf.sprintf "Failed to determine self parameter for message %s"
           msg.msg_name
        )
...
let default_doccomments =
  [
    ( "create"
    , fun x -> sprintf "Create a new %s instance, and return its handle." x.name
    )
  ; ("destroy", fun x -> sprintf "Destroy the specified %s instance." x.name)
  ; ( "get_by_uuid"
    , fun x ->
  ...
  ; ( "get_record"
    , fun x ->
        sprintf "Get a record containing the current state of the given %s."
          x.name
    )
  ; ( "get_record_internal"
    , fun x ->
        sprintf
          "returns a record containing the state of an instance of class %s"
          x.name
    )
  ; ( "get_allowed_messages"
    , fun _ ->
        sprintf
          "dynamically compute the set of message names which it is legal to \
           send to the object at this point"
    )
  ; ( "get_all"
    , fun x ->
  ...
  ; ( "get_all_records"
    , fun x ->
        sprintf
          "Return a map of %s references to %s records for all %s known to the \
           system."
...
let self_of_obj x =
  ...
  ; param_doc= "reference to the object"
...
let new_messages_of_field x order fld =
  ...
  let getter =
    ...
    ; msg_errors= []
    ; msg_doc=
        Printf.sprintf "Get the %s field of the given %s."
          (String.concat "/" fld.full_name)
          x.name
    ; msg_allowed_roles= fld.field_getter_roles
    ; msg_tag= FromField (Getter, fld)
    }
  in
  let setter =
    {
    ...
    ; msg_errors= []
    ; msg_doc=
        Printf.sprintf "Set the %s field of the given %s."
          (String.concat "/" fld.full_name)
          x.name
  ...
  let is_many_to_many =
    ...
    && Relations.classify api (this, Relations.other_end_of api this)
       = (`Many, `Many)
  in
  ...
  | Set t, _, _ ->
      if order = 0 then
        ...
        [
          setter
        ; (* only makes sense to the database *)
          {
            common with
          ...
          ; msg_doc=
              sprintf
                "Add the given value to the %s field of the given %s.  If the \
        ...
        ; {
          ...
          ; msg_doc=
              sprintf
                "Remove the given value from the %s field of the given %s.  If \
  ...
  | Map (k, v), _, _ ->
      if order = 0 then
        ...
        ; (* only makes sense to the database *)
          ...
          ; msg_allowed_roles= fld.field_setter_roles
          ; msg_map_keys_roles=
              List.map (fun (k, w) -> (k, w)) fld.field_map_keys_roles
          ; msg_tag= FromField (Add, fld)
          }
        ; {
            common with
          ...
          ; msg_doc=
              sprintf
                "Remove the given key and its corresponding value from the %s \
                 field of the given %s.  If the key is not in that Map, then \
                 do nothing."
                (String.concat "/" fld.full_name)
                x.name
          ; msg_allowed_roles= fld.field_setter_roles
...
let messages_of_obj (x : obj) document_order : message list =
  ...
  let common =
    ...
    ; msg_pool_internal= false
    ; msg_session= false
    ; msg_release= x.obj_release
    ; msg_lifecycle= x.obj_lifecycle
  ...
  let dtor =
    ...
    ; msg_has_effect= true
    ; msg_allowed_roles= x.obj_allowed_roles
    ; msg_tag= FromObject Delete
    }
  in
  (* Get by UUID *)
  let uuid =
    let name = "get_by_uuid" in
    {
      common with
      msg_name= name
    ; msg_params=
  ...
  let get_by_name_label =
        ...
        [
          {
            param_type= String
          ; param_name= "label"
          ; param_doc= "label of object to return"
          ; param_release= x.obj_release
          ; param_default= None
          }
        ]
    ...
    ; msg_has_effect= false
    ; msg_allowed_roles= x.obj_implicit_msg_allowed_roles
    ; msg_tag= FromObject GetByLabel
    }
  in
  (* Get Record *)
  let get_record =
    let name = "get_record" in
    {
      common with
      msg_name= name
    ; msg_params= [self]
    ...
    ; msg_has_effect= false
    ; msg_allowed_roles= x.obj_implicit_msg_allowed_roles
    ; msg_tag= FromObject GetRecord
    }
  in

  (* Get Record (private db version) *)
  let get_record_internal =
    let name = "get_record_internal" in
    {
      common with
      msg_name= name
    ; msg_params= [self]
    ...
    ; msg_db_only= true
    ; msg_release=
        {opensource= []; internal= []; internal_deprecated_since= None}
    ; (* internal messages not in an any API releases... *)
      msg_has_effect= false
        ...
        [
          {
            param_type= String
          ; param_name= "expr"
          ; param_doc= "expression representing records to fetch"
          ; param_release= x.obj_release
          ; param_default= None
          }
        ]
    ; msg_result=
        Some (Map (Ref x.name, Record x.name), "records of all matching objects")
    ; msg_release=
        {
          opensource= []
        ; internal= x.obj_release.internal
  ...
  let get_all_records =
    ...
    ; msg_params= []
    ; msg_result=
        Some (Map (Ref x.name, Record x.name), "records of all objects")
    ; msg_release=
        {
          opensource= []
        ; internal= x.obj_release.internal
...
let add_implicit_messages ?(document_order = false) (api : api) =
  let objs = objects_of_api api and rels = relations_of_api api in
  let objs =
    List.map
      (fun obj ->
...
let rec has_been_removed = function
  ...
  | [(Removed, _, _)] ->
      true
  | _ :: other_transitions ->
      has_been_removed other_transitions
