...
let introduce_remote_pool =
  call ~name:"introduce_remote_pool" ~in_oss_since:None ~lifecycle:[]
    ...
    ~params:
      ...
      ; ( String
        , "address"
        , "The host name or a string representation of the IP address of the \
           coordinator of the remote pool repository"
        )
      ; ( String
        , "certificate"
...
let t =
  create_obj ~name:_repository ~descr:"Repository for updates" ~doccomments:[]
    ...
    ~contents:
      ...
      ; field ~qualifier:StaticRO ~lifecycle:[] ~ty:String
          ~default_value:(Some (VString "")) "address"
          "The hostname of the host which hosts this repository"
      ; field ~qualifier:StaticRO ~lifecycle:[] ~ty:String
