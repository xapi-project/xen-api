...
let introduce_remote_pool =
  call ~name:"introduce_remote_pool" ~in_oss_since:None ~lifecycle:[]
    ~doc:
      "Add the configuration for a new remote pool repository (note updates \
       can be synced from it only after the remote pool has synced updates and \
       applied the updates)"
    ~params:
      [
        (String, "name_label", "The name of the repository")
      ...
      ; ( String
        , "binary_url"
        , "Base URL of binary packages of applied updates in this remote pool \
           repository"
        )
      ; ( String
        , "certificate"
