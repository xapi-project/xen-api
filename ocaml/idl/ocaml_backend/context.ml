...
(* String constants used to build up and to recognise other strings. *)
let unix_domain_socket_str = "Unix domain socket"
let http_request_from_str = "HTTP request from"

let string_of_origin = function
  | Http (req, fd) -> 
    let peer = match Unix.getpeername fd with
      | Unix.ADDR_UNIX _ -> unix_domain_socket_str
	  | Unix.ADDR_INET _ -> "Internet" in (* unfortunately all connections come from stunnel on localhost *)
    Printf.sprintf "%s %s with User-Agent: %s" http_request_from_str peer (default "unknown" req.Http.Request.user_agent)
  | Internal -> "Internal"

let is_origin_string_unix_socket origin_str =
  String.startswith (http_request_from_str^" "^unix_domain_socket_str) origin_str
