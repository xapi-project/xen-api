...
let record_exists api : O.Module.t =
  let ref_exists (obj : obj) =
    ...
    O.Let.make
      ...
      ~ty:"bool" ~body:[body] ()
  in
  O.Module.make ~name:_db_exists ~preamble:[]
    ~elements:
      (List.map
...
let db_check api : O.Module.t =
  let check_refs (obj : obj) =
    ...
    in
    let wrapper f =
      Printf.sprintf "(runcheck \"%s\" %s \"%s\" %s)" obj.name Client._self
        (String.concat "/" f.full_name)
        (check f)
    in
    let body =
      if fields = [] then
        ...
        [getrecord; String.concat " &&\n     " (List.map wrapper fields)]
    in
    O.Let.make
      ~name:("_" ^ OU.ocaml_of_obj_name obj.name)
      ~params:[Gen_common.context_arg; self obj]
  ...
  let all_records (obj : obj) =
    ...
    let fold =
      if obj.DT.in_database then
        Printf.sprintf
          "List.fold_left (&&) true (List.map (fun self -> _%s ~%s ~self) \
           (%s.%s.get_all ~%s))"
          obj_name Gen_common.context _db_action obj_name Gen_common.context
      else
        "true"
    in
    O.Let.make ~name:("all_" ^ obj_name) ~params:[Gen_common.context_arg]
      ~ty:"bool" ~body:[fold] ()
  in
  let all (objs : obj list) =
    let one obj =
      let obj_name = OU.ocaml_of_obj_name obj.name in
      Printf.sprintf "(all_%s ~%s)" obj_name Gen_common.context
    in
    O.Module.Let
