...
let writer_permission name nperms =
  ...
  let permission_name_label =
    ...
    (* lowercase here asked by GUI team *)
  in
...
let writer_stdout static_roles_permissions static_permissions_roles =
  ...
  ^ "let get_refs permissions = List.map (fun p->Ref.of_string (Ref.ref_prefix \
     ^ p.role_uuid)) permissions\n\n"
  ^ List.fold_left
...
let rec concat = function
  | xperm, rs, [] ->
      let r1, r2 =
        List.partition (fun (r, _) -> r = internal_role_local_root) rs
      in
      let r, perms =
        match r1 with [] -> (internal_role_local_root, []) | r1 :: _ -> r1
      in
      (r, xperm :: perms) :: r2
...
*)
  ...
  let sort_fn a b =
    (* 0 if equal, + if a>b, - if a<b *)
    role_idx (a, Datamodel_roles.roles_all)
    - role_idx (b, Datamodel_roles.roles_all)
  in
...
let gen_permissions_of_static_roles highapi =
  ...
  let api_roles_permissions =
    get_roles_permissions_of_objs ([], all_objs)
    (*api*)
  in
  ...
  let _, permissions_roles =
    (* ignore the _local_root_ permission *)
    List.partition
