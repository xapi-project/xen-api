...
(* generates: _VM *)
let ocaml_of_record_name_rpc x = escape x

(* generates: _VM_foo *)
let ocaml_of_record_field_rpc x = escape (String.concat "_" x)
...
let rec alias_of_ty = function
  ...
  | DateTime ->
      "datetime"
  | Set ty ->
      sprintf "%s_set" (alias_of_ty ty)
  | Enum (name, _) ->
      String.uncapitalize_ascii name
  | Map (k, v) ->
      sprintf "%s_to_%s_map" (alias_of_ty k) (alias_of_ty v)
  | Ref x ->
      sprintf "ref_%s" x
  | Record x ->
