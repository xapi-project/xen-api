...
let context_of_http_req ?session_id ~generate_task_for ~supports_async ~label ~http_req ~fd =
  (* Check to see if there is a foreign database associated with this session *)
  let database = match session_id with
  | Some sid ->
    (try
      Some (Hashtbl.find Db_backend.foreign_databases sid)
    with Not_found ->
      None)
  | None -> None
  in
  match http_req.Http.task with
   | Some task_id -> 
       Context.from_forwarded_task ?session_id ~origin:(Context.Http(http_req,fd)) (Ref.of_string task_id)
   | None ->
       if generate_task_for && supports_async  
       then 
         let subtask_of = Pervasiveext.may Ref.of_string http_req.Http.subtask_of in
         Context.make ?session_id ?subtask_of ?database ~task_in_database:true ~origin:(Context.Http(http_req,fd)) label
       else 
         Context.make ?session_id ?database ~origin:(Context.Http(http_req,fd)) label
...
let do_dispatch ?session_id ?forward_op ?self _type called_async supports_async called_fn_name op_fn
  ...
  then XMLRPC.Fault(0l, "No async mode for this operation (rpc: "^called_fn_name^")")
    ...
    let __context = context_of_http_req ?session_id ~generate_task_for ~supports_async ~label ~http_req ~fd in
    if called_async
	    then begin
...
let exec_with_forwarded_task ?session_id ?origin task_id f =
  exec_with_context
    ~__context:(Context.from_forwarded_task ?session_id ?origin task_id)
   (fun ~__context -> f __context)
