  ...
  match http_req.Http.task with
   ...
   | None ->
       ...
       then 
         ...
         Context.make ?session_id ?subtask_of ~task_in_database:true ~origin:(Context.Http(http_req,fd)) label
       else 
         Context.make ?session_id ~origin:(Context.Http(http_req,fd)) label
