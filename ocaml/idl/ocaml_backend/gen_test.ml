...
let rec gen_test_type highapi ty =
  let rec aux = function
    ...
    | DT.Set (DT.Enum (_, y)) ->
        Printf.sprintf "[ %s ]"
          (String.concat ";" (List.map (fun (x, _) -> OU.constructor_of x) y))
    | DT.Set x ->
        Printf.sprintf "[ %s ]" (aux x)
    | DT.Map (x, y) ->
        Printf.sprintf "[ (%s,%s) ]" (aux x) (aux y)
    | DT.Ref _ ->
        Printf.sprintf "(Ref.of_string \"OpaqueRef:foo\")"
    | DT.Record x ->
        gen_record_type highapi x
    | _ ->
...
and gen_record_type highapi record =
  ...
  in
  let regular_def fld =
    sprintf "%s=%s" (field fld) (gen_test_type highapi fld.DT.ty)
  in
