...
let rpc_headers ?task_id ?subtask_of ~version host path content_length = [
...
let check_reusable (x: Unix.file_descr) = 
  ...
  let xml = Xmlrpc.string_of_call (Rpc.call unknown_msg []) in
  ...
  try
    ...
    if List.mem_assoc "content-length" headers then begin
      ...
      let response = Xmlrpc.response_of_string tmp in
      match response.Rpc.contents with
      | Rpc.Enum [ Rpc.String code; Rpc.String param ] when
          not response.Rpc.success && code = Api_errors.message_method_unknown && param = unknown_msg ->  true
      | _ ->
...
let do_http_rpc host port ?(unixsock=None) headers body f =
  ...
  try
    let content_length, _ = http_rpc_fd s headers body in
    let result = f content_length s in
    Unix.close s;
    result
  with e ->
    Unix.close s;
...
(** Take an optional content_length together with a socket
    and return the XMLRPC response as an XML document *)
let read_rpc_response content_length s = 
  try 
    if content_length > -1 then begin
      let buffer = String.make content_length '\000' in
      let rec really_read off n =
        if n <> 0 then
          let m = Unix.read s buffer off n in
          really_read (off+m) (n-m) in
      really_read 0 content_length;
      Xmlrpc.response_of_string buffer
    end else
      Xmlrpc.response_of_in_channel (Unix.in_channel_of_descr s)
  with
  | Unix.Unix_error(Unix.ECONNRESET, _, _) -> raise Connection_reset

let read_xml_rpc_response content_length s = 
  try 
    if content_length > -1 then begin
      let buffer = String.make content_length '\000' in
      let rec really_read off n =
...
let generic_rpc ?task_id ?subtask_of req version host path =
  let req_string = Xmlrpc.string_of_call req in
  let headers = rpc_headers ?task_id ?subtask_of ~version host path (String.length req_string) in
    req_string, headers

(** Perform a complete XMLRPC over a fresh connection. *)
let do_rpc ?task_id ?subtask_of ~version ~host ~port ~path (req: Rpc.call) : Rpc.response =
  let req_string, headers = generic_rpc ?task_id ?subtask_of req version host path in
  do_http_rpc host port headers req_string read_rpc_response

let do_secure_rpc ?task_id ?subtask_of ?use_external_fd_wrapper ?use_stunnel_cache ~version ~host ~port ~path (req: Rpc.call) : Rpc.response =
  let req_string, headers = generic_rpc ?task_id ?subtask_of req version host path in
  do_secure_http_rpc ?use_external_fd_wrapper ?use_stunnel_cache ?task_id ~host ~port ~headers ~body:req_string read_rpc_response

let do_rpc_unix ?task_id ?subtask_of ~version ~filename ~path (req: Rpc.call) : Rpc.response =
  let req_string, headers = generic_rpc ?task_id ?subtask_of req version "localhost" path in  
  do_http_rpc "" 0 headers ~unixsock:(Some filename) req_string read_rpc_response
...
let do_rpc_persistent ~host ~path (s: Unix.file_descr) (req: Rpc.call) : Rpc.response = 
  let req_string = Xmlrpc.string_of_call req in
  (* Persistent connections requires HTTP 1.1 *)
  let headers = rpc_headers ~version:"1.1" host path (String.length req_string + 0) in
  let content_length, _ = http_rpc_fd s headers req_string in
  if content_length < 0 then raise Content_length_required;
  read_rpc_response content_length s
