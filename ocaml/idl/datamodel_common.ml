...
let meaningful_vm_has_vendor_device_schema_major_vsn =
  dundee_tech_preview_release_schema_major_vsn

let meaningful_vm_has_vendor_device_schema_minor_vsn =
  dundee_tech_preview_release_schema_minor_vsn
...
let tech_preview_releases =
  [
    ...
    , vgpu_tech_preview_release_schema_minor_vsn
    )
  ; ( dundee_tech_preview_release_schema_major_vsn
    , dundee_tech_preview_release_schema_minor_vsn
    )
...
let call ~name ?(doc = "") ?(in_oss_since = Some "3.0.3") ?result
  ...
  *)
  ...
  let call_release =
    {
      internal=
        ( match get_published lifecycle with
        | Some published ->
            get_product_releases published
        | None ->
            ["closed"]
        )
  ...
  ; msg_result= result
  ; msg_doc= doc
  ; msg_session= List.mem `Session flags
  ...
  ; msg_errors= List.map (Hashtbl.find Datamodel_errors.errors) errs
  ; msg_secret= secret
...
let errnames_of_call c = List.map (fun e -> e.err_name) c.msg_errors
...
let namespace ?(get_field_writer_roles = fun x -> x)
    ?(get_field_reader_roles = fun x -> x) ?(idempotent = false) ~name ~contents
    () =
  let rec prefix = function
    | Namespace (x, xs) ->
        Namespace (x, List.map prefix xs)
    | Field x ->
        Field
          {
            x with
...
let default_field_writer_roles = _R_POOL_ADMIN
...
let create_obj ?lifecycle ~in_oss_since ~gen_constructor_destructor ~gen_events
  ...
  let contents_default_writer_roles =
    if contents_default_writer_roles = None then
      ...
      contents_default_writer_roles
  in
  let get_field_reader_roles = function
    | None ->
        contents_default_reader_roles
    | r ->
        r
  in
  let get_field_writer_roles = function
    | None ->
        contents_default_writer_roles
    | r ->
        r
  in
  let get_msg_allowed_roles = function
    | None ->
        messages_default_allowed_roles
    | r ->
        r
  ...
  let contents =
    List.map
      (function
        | Namespace (n, cs) ->
            namespace ~get_field_writer_roles ~get_field_reader_roles ~name:n
              ~contents:cs ~idempotent:true ()
        | Field f ->
            Field
              {
                f with
