      ...
      ; {
          param_type=
            Enum
              ( "need_update"
              , [
                  ("yes", "Certificates need updating")
                ; ("no", "Certificates are up to date")
                ; ( "unspecified"
                  , "Caller did not provide the update argument"
                  )
                ]
              )
        ; param_name= "update"
        ; param_doc=
            "If 'yes', set secureboot_certificates_state to \
             update_available. If 'no', set to ok. If omitted \
             (defaults to 'unspecified'), run certificate check to \
             determine the state."
