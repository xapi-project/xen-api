    ...
    create_obj ~in_db:true ~in_product_since:rel_rio ~in_oss_since:oss_since_303 ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:true ~name:_vm ~descr:"A virtual machine (or 'guest')."
      ...
      ~contents:
        ([ uid _vm;
           ...
           field ~in_oss_since:None ~internal_only:false ~lifecycle:[Published, rel_miami, "Marshalled value containing VM record at time of last boot, updated dynamically to reflect the runtime state of the domain"; Changed, rel_next, "Become static to allow Suspended VM creation"] ~qualifier:StaticRO ~ty:String "last_booted_record" "marshalled value containing VM record at time of last boot" ~default_value:(Some (VString ""));
