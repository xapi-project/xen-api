...
let to_assoc_list (x: restrictions) = 
	[ (_restrict_connection,string_of_bool x.restrict_connection);
		...
		(_restrict_cpu_masking,            string_of_bool (not x.enable_cpu_masking   ));
...
let of_assoc_list x = 
	...
	{ 
		...
		enable_vswitch_controller = Opt.default most_permissive.enable_vswitch_controller (Opt.map not (find bool_of_string _restrict_vswitch_controller));
		enable_cpu_masking    = Opt.default most_permissive.enable_cpu_masking    (Opt.map not (find bool_of_string _restrict_cpu_masking));
