...
let test_bad_fill_rate () =
  Alcotest.match_raises "Creating a token bucket with 0 fill rate should fail"
    ...
    (fun () -> ignore (Token_bucket.create ~burst_size:1.0 ~fill_rate:0.0)) ;
  Alcotest.match_raises
    "Creating a token bucket with negative fill rate should fail"
    (function Invalid_argument _ -> true | _ -> false)
    (fun () -> ignore (Token_bucket.create ~burst_size:1.0 ~fill_rate:(-1.0)))
