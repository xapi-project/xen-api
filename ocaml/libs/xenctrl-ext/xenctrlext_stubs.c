...
static void raise_unix_errno_msg(int err_code, const char *err_msg)
{
    CAMLparam0();
    value args[] =
        { unix_error_of_code(err_code), caml_copy_string(err_msg) };
...
static int xcext_domain_set_timer_mode(xc_interface *xch,
                                       unsigned int domid, int mode)
{
...
CAMLprim value stub_xenctrlext_domain_update_channels(value xch_val,
                                                      value domid,
                                                      value store_port,
                                                      value console_port)
{
    ...
    caml_release_runtime_system();
    int retval =
        xc_set_hvm_param(xc, Int_val(domid), HVM_PARAM_STORE_EVTCHN,
                         Int_val(store_port));
    if (!retval)
        retval =
            xc_set_hvm_param(xc, Int_val(domid), HVM_PARAM_CONSOLE_EVTCHN,
 ...
 * necessary.
 */
static void ocaml_int64_array_to_c_array(value o, uint32_t *c,
                                         mlsize_t c_len)
{
...
__attribute__((weak))
void xc_combine_cpu_featuresets(const uint32_t *p1, const uint32_t *p2,
...
__attribute__((weak))
const char *xc_cpu_featuresets_are_compatible(const uint32_t *vm,
                                              const uint32_t *host,
                                              size_t len, char err[128]);

/* int64 array (vm) -> int64 array (host) -> string option (None on success, string on failure) */
CAMLprim value stub_xenctrlext_featuresets_are_compatible(value vm,
                                                          value host)
{
    CAMLparam2(vm, host);
    CAMLlocal1(result);
                                         ...
                                         value prot_flags, value pages)
{
    ...
    result =
        caml_ba_alloc_dims(CAML_BA_CHAR | CAML_BA_C_LAYOUT |
                           CAML_BA_EXTERNAL, 1, retval,
                           (long) 4096 * pages_length);
...
CAMLprim value stub_xenctrlext_domain_claim_pages(value xch_val,
                                                  value domid_val,
                                                  value numa_node_val,
                                                  value nr_pages_val)
{
...
#ifdef XEN_DOMCTL_NUMA_OP_GET_NODE_PAGES
CAMLprim value stub_xc_domain_numa_get_node_pages(value xch_val,
                                                  value domid);
#endif

CAMLprim value stub_xc_domain_numa_get_node_pages_wrapper(value xch_val,
                                                          value domid)
{
#ifdef XEN_DOMCTL_NUMA_OP_GET_NODE_PAGES
