  ...
  functor
    ...
    struct
      let list_domains xc =
        ...
        debug "Current domains: %s"
          (String.concat ", " (List.map string_of_int ids)) ;
        List.fold_left
          (fun map (k, v) -> IntMap.add k v map)
          IntMap.empty (List.combine ids dis)

      let domain_looks_different a b =
        match (a, b) with
        | None, Some _ ->
            true
        | Some _, None ->
        ...
        | Some a', Some b' ->
            ...
            || a'.shutdown
               && b'.shutdown
               && a'.shutdown_code <> b'.shutdown_code
      ...
      let watch_xenstore () =
        try
          with_xc_and_xs (fun xc xs ->
              ...
              let add_watches_for_domain xs domid uuid =
                ...
                List.iter (watch ~xs token)
                  (Actions.interesting_paths_for_domain domid uuid) ;
              ...
              let remove_watches_for_domain xs domid =
                ...
                if IntMap.mem domid !uuids then (
                  let uuid = IntMap.find domid !uuids in
                  let token = Actions.watch_token domid in
                  List.iter (unwatch ~xs token)
                    (Actions.interesting_paths_for_domain domid uuid) ;
              ...
              let look_for_different_domains () =
                ...
                List.iter
                  (fun domid ->
                    ...
                    if Actions.unmanaged_domain domid id then (
                      debug "However domain %d is not managed by us: ignoring"
                        domid ;
                      if IntMap.mem domid !uuids then (
                        debug "Cleaning-up the remaining watches for: domid %d"
                          domid ;
        ...
        with e ->
          debug "Caught exception attempting to watch xenstore: %s"
