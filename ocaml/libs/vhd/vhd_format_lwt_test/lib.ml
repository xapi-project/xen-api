  ...
  fold_left
    (fun offset x ->
      ...
      | `Empty y ->
          ...
          List.iter
            (fun (x, _) ->
              if x >= offset && x < Int64.add offset y then
                failwith
                  (Printf.sprintf "Sector %Ld is not supposed to be empty" x)
            )
            expected ;
      ...
      | `Sectors data ->
          let rec loop offset remaining =
            if Cstruct.length remaining = 0 then
              ...
              not (List.mem_assoc offset expected)
            then
              failwith
                (Printf.sprintf "Sector %Ld is not supposed to be written to"
                   offset
                )
            else
...
let verify t contents =
  ...
  let capacity =
    Int64.(
      shift_left
        (of_int t.Vhd.header.Header.max_table_entries)
        (t.Vhd.header.Header.block_size_sectors_shift + sector_shift)
    )
  in
  ( if capacity < t.Vhd.footer.Footer.current_size then
      fail
        (Failure
           (Printf.sprintf
              "insufficient capacity in vhd: max table entries = %d; capacity \
               = %Ld; current_size = %Ld"
              t.Vhd.header.Header.max_table_entries capacity
  ...
  Vhd_IO.openchain filename false >>= fun t' ->
  check_written_sectors t' contents >>= fun () ->
  check_raw_stream_contents t' contents >>= fun () -> Vhd_IO.close t'
