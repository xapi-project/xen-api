...
let check_resize size =
  ...
  Vhd_IO.openchain filename false >>= fun vhd' ->
  assert_equal ~printer:Int64.to_string newsize
    vhd.Vhd.footer.Footer.current_size ;
  Vhd_IO.close vhd'
...
let check_parent_parent_dir () =
  ...
  Vhd_IO.create_difference ~filename:leaf_path ~parent:vhd ~relative_path:true
    ()
  >>= fun vhd' ->
...
let absolute_sector_of vhd {block; sector} =
  if vhd.Vhd.header.Header.max_table_entries = 0 then
    None
  else
    let block =
      match block with
      | First ->
          0
      | Last ->
          vhd.Vhd.header.Header.max_table_entries - 1
    in
    let sectors_per_block =
      1 lsl vhd.Vhd.header.Header.block_size_sectors_shift
    in
    let relative_sector =
      match sector with First -> 0 | Last -> sectors_per_block - 1
    in
    Some
      Int64.(
        add
          (mul (of_int block) (of_int sectors_per_block))
  ...
  | Write (position, message) -> (
      ...
      let vhd =
        ...
        | None ->
            failwith "no vhd open"
      in
      match absolute_sector_of vhd position with
      | Some sector ->
          Vhd_IO.write vhd sector [data] >>= fun () ->
          (* Overwrite means we forget any previous contents *)
          let contents =
            List.filter (fun (x, _) -> x <> sector) state.contents
          in
...
let verify state =
  match state.child with None -> return () | Some t -> verify t state.contents
...
let run program =
  ...
  >>= fun () -> cleanup final_state

let all_program_tests =
  List.map
    (fun p -> string_of_program p >:: fun () -> Lwt_main.run (run p))
    programs
...
let _ =
  ...
  let check_resize size =
    Printf.sprintf "check_resize_%Ld" size >:: fun () ->
    Lwt_main.run (check_resize size)
  in

  let check_empty_snapshot size =
    Printf.sprintf "check_empty_snapshot_%Ld" size >:: fun () ->
    Lwt_main.run (check_empty_snapshot size)
  in

  (* Switch to the 'nobody' user so we can test file permissions *)
  let nobody = Unix.getpwnam "nobody" in
  ( try Unix.setuid nobody.Unix.pw_uid
    with e ->
  ...
  let suite =
    ...
    >::: [
           "create" >:: create
         ; ( "check_parent_parent_dir" >:: fun () ->
             Lwt_main.run (check_parent_parent_dir ())
           )
