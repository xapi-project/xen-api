...
let forall_sectors f offset bufs =
  let rec one offset = function
    | [] ->
        return ()
    | b :: bs ->
        f offset b >>= fun () -> one (Int64.succ offset) bs
  in
...
let read t offset bufs =
  match t.vhd with
  | None ->
      return (Rresult.R.error `Disconnected)
  | Some vhd ->
      forall_sectors
        (fun offset sector ->
          (Vhd_IO.read_sector vhd offset sector >>= function
           | false ->
               Cstruct.blit zero 0 sector 0 512 ;
               return ()
           | true ->
               return ()
          )
...
let write t offset bufs =
  ...
  | None ->
      return (Rresult.R.error `Disconnected)
  | Some vhd ->
      Vhd_IO.write vhd offset bufs >>= fun () -> return (Rresult.R.ok ())
