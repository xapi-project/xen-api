...
let rec allpairs xs ys =
  ...
  | [] ->
      []
  | x :: xs ->
      List.map (fun y -> (x, y)) ys @ allpairs xs ys
...
let descr_of_operation = function
  ...
  | Snapshot ->
      ["filename := Vhd.snapshot(current)"; "current := Vhd.open(filename)"]
  | Write (p, message) ->
      [
        Printf.sprintf
          "Vhd.write(current, block = %s, sector = %s, data = \"%s\")"
          (string_of_choice p.block)
...
let string_of_operation = function
  | Create x ->
      Printf.sprintf "Create:%Ld" x
  | Snapshot ->
      "Snapshot"
  | Write (p, _) ->
      Printf.sprintf "Write:%s:%s" (string_of_choice p.block)
        (string_of_choice p.sector)
