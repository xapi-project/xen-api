...
let test_no_backtraces () =
  ...
  try Backtrace_test_lib.Log.with_thread_associated "Backtrace lab" foo () with _ -> ()

let reraise = ref false

let test_reraise () =
  Printexc.record_backtrace true ;
  try Backtrace_test_lib.Log.with_thread_associated "Backtrace lab" bar () with _ -> ()

let v1_with_backtrace = ref false

let test_v1_with_backtrace () =
  Printexc.record_backtrace false ;
  try Backtrace_test_lib.Log.with_thread_associated_old "Backtrace lab" foo () with _ -> ()

let raise_again = ref false

let test_raise_again () =
  Printexc.record_backtrace true ;
  try Backtrace_test_lib.Log.with_thread_associated "Backtrace lab" baz () with _ -> ()
