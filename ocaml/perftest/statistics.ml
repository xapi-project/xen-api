...
let gaussian mu sigma x =
  1.0
  /. (sigma *. sqrt (2.0 *. pi))
  *. exp (-.((x -. mu) ** 2.0) /. (2.0 *. sigma *. sigma))

module Hist = struct
  type t = {
      bin_start: float array
    ; bin_end: float array
    ; bin_count: float array
          (* height of each bin: multiply by width to get area *)
  }
  ...
  let integrate (x : t) =
    ...
    let result =
      make x.bin_start.(0) x.bin_end.(Array.length x.bin_end - 1) n
    in
    let area = ref 0. in
  ...
  let add (x : t) (y : float) =
    try
      for i = 0 to Array.length x.bin_start - 1 do
        if x.bin_start.(i) <= y && y <= x.bin_end.(i + 1) then (
...
module LogNormal = struct
  ...
  let sigma (points : float list) =
    ...
    let normal_mean = Normal.mean points in
    let v =
      (exp (normal_sigma *. normal_sigma) -. 1.)
      *. exp ((2. *. normal_mean) +. (normal_sigma *. normal_sigma))
    in
    sqrt v
end
