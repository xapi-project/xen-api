...
let make_iscsi session_id pool network =
  try
    let iscsi_iso =
      match find_iscsi_iso session_id with
      | Some vdi ->
          vdi
      | None ->
...
let make ~rpc ~session_id ~pool ~vm ~networks ~storages =
  ...
  for i = 0 to Array.length storages - 1 do
    ...
    for j = 0 to vm.num - 1 do
      ...
      in
      let clone =
        Client.VM.clone ~rpc ~session_id ~vm:wintemplate ~new_name:newname
      in
      Client.VM.add_tags ~rpc ~session_id ~self:clone ~value:vm.tag ;
      Client.VM.remove_from_other_config ~rpc ~session_id ~self:clone
        ~key:"disks" ;
      for userdevice = 0 to vm.vbds - 1 do
        Printf.printf " - creating VDI %d for VM %d on SR %d of %d\n%!"
          userdevice j i (Array.length storages) ;
        let newdisk =
          Client.VDI.create ~rpc ~session_id ~name_label:"Guest disk"
            ~name_description:"" ~sR:storages.(i) ~virtual_size:4194304L
            ~_type:`user ~sharable:false ~read_only:false ~xenstore_data:[]
      ...
      Client.VM.set_memory_static_min ~rpc ~session_id ~self:clone
        ~value:16777216L ;
      Client.VM.set_memory_dynamic_min ~rpc ~session_id ~self:clone
        ~value:16777216L ;
      Client.VM.set_memory_dynamic_max ~rpc ~session_id ~self:clone
        ~value:16777216L ;
      Client.VM.set_memory_static_max ~rpc ~session_id ~self:clone
        ~value:16777216L ;
      if vm.has_affinity && Array.length storages = Array.length host_refs then
        Client.VM.set_affinity ~rpc ~session_id ~self:clone ~value:host_refs.(i)
    done
  done
