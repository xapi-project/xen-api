...
let do_secure_http_rpc ?(use_external_fd_wrapper=true) ?(use_stunnel_cache=false) ?task_id ~host ~port ?(unixsock=None) ~headers ~body f =
  let st_proc = 
  ...
  let s_pid = st_proc.Stunnel.pid in
    ...
    (fun () ->
       if use_stunnel_cache
       then Stunnel_cache.add st_proc
       else Stunnel.disconnect st_proc)
