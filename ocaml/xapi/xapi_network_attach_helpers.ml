...
let assert_can_see_named_networks ~__context ~vm ~host reqd_nets =
  let is_network_available_on host net =
    ...
    if pifs <> [] then
      ...
      let hosts =
        List.map (fun self -> Db.PIF.get_host ~__context ~self) pifs
      in
      List.mem host hosts
    else
      ...
      then (
        ...
        let vms =
          List.map (fun self -> Db.VM.get_record ~__context ~self) vms
        in
        let vms =
          List.filter (fun vm -> vm.API.vM_power_state = `Running) vms
        in
