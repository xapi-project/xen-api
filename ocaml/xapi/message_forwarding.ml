...
module Forward = functor(Local: Custom_actions.CUSTOM_ACTIONS) -> struct
	...
	module Bond = struct
		let create ~__context ~network ~members ~mAC ~mode ~hashing_algorithm =
			...
			let local_fn = Local.Bond.create ~network ~members ~mAC ~mode ~hashing_algorithm in
			(* The management interface on the slave may change during this operation, so expect connection loss.
			...
			in
			let fn () =
				do_op_on ~local_fn ~__context ~host (fun session_id rpc -> Client.Bond.create rpc session_id network members mAC mode hashing_algorithm) in
			tolerate_connection_loss fn success
