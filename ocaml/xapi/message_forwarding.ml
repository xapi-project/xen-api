...
module Forward = functor(Local: Custom_actions.CUSTOM_ACTIONS) -> struct
	...
	module VM = struct
		...
		*)
		...
		let set_schedule_snapshot ~__context ~self ~value =
			info "VM.set_schedule_snapshot: self = '%s'; " (vm_uuid ~__context self);
			Local.VM.set_schedule_snapshot ~__context ~self ~value
