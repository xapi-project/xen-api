...
let set_servertime ~__context ~self ~value =
  ...
  try
    ...
    let naive_in_utc =
      let x = Date.to_utc value |> Date.to_rfc3339 in
      Scanf.sscanf x "%04i%02i%02iT%02i:%02i:%02i%s" (fun y mon d h min s _ ->
          Printf.sprintf "%04i-%02i-%02i %02i:%02i:%02i" y mon d h min s
      )
    in
    if not_utc then f ["set-timezone"; "UTC"] |> ignore ;
    f ["set-time"; naive_in_utc] |> ignore ;
    if not_utc then f ["set-timezone"; tz] |> ignore ;
