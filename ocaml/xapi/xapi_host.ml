...
let sync_vlans ~__context ~host =
  ...
  let get_network_of_pif_underneath_vlan vlan_pif_rec =
    ...
    | _ -> failwith (Printf.sprintf "Cannot find vlan from a vlan master PIF:%s" vlan_pif_rec.API.pIF_uuid)
  in
...
let sync_tunnels ~__context ~host =
  ...
  let get_network_of_transport_pif access_pif_rec =
    ...
    | _ -> failwith (Printf.sprintf "PIF %s has no tunnel_access_PIF_of" access_pif_rec.API.pIF_uuid)
  in
