...
let copy_bonds_from_master ~__context () =
	...
	let maybe_create_bond_for_me bond = 
		...
		let bond_hashing_algorithm = bond.API.bond_hashing_algorithm in
		...
		| [], _ ->
			...
			let (_: API.ref_Bond) = Xapi_bond.create ~__context ~network ~members:my_slave_pif_refs ~mAC:"" ~mode:bond_mode ~hashing_algorithm:bond_hashing_algorithm in ()
		...
		then
			begin
				...
				| [(pif_ref,_)] -> (* this is the PIF on which we want as transport PIF; let's make it *)
					ignore (Xapi_tunnel.create ~__context ~transport_PIF:pif_ref
