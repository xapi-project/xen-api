...
let try_create_sr_from_record ~__context ~_type ~device_config ~dr_task
    ~sr_record =
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      (* Create the SR record. *)
      debug "Introducing SR %s" sr_record.uuid ;
      let sr =
        Client.SR.introduce ~rpc ~session_id ~uuid:sr_record.uuid
          ~name_label:sr_record.name_label
          ~name_description:sr_record.name_description ~_type ~content_type:""
      ...
      with e ->
        ...
        warn "Could not successfully attach SR %s - caught %s" sr_record.uuid
          (Printexc.to_string e) ;
        let pbds =
          Xapi_sr.get_pbds ~__context ~self:sr ~attached:true ~master_pos:`Last
        in
...
let create ~__context ~_type ~device_config ~whitelist =
  ...
  let sr_records =
    try parse_sr_probe probe_result
    with Failure msg ->
      Helpers.internal_error "SR probe response was malformed: %s" msg
  in
  (* If the SR record has a UUID, make sure it's in the whitelist. *)
  let sr_records =
    List.filter (fun sr_record -> List.mem sr_record.uuid whitelist) sr_records
  in
  ...
  List.iter
    (fun sr_record ->
      try
        ...
        debug
          "SR %s has already been introduced, so not adding it to this \
