...
let update_metadata_latest ~__context =
  ...
  let cached_vdis =
    Hashtbl.fold (fun vdi _ vdi_list -> vdi :: vdi_list) db_vdi_cache []
  in
  List.iter
    (fun vdi ->
      ...
      List.iter
        (fun vdi ->
          ...
          try
            ...
            let pool_uuid =
              Db.Pool.get_uuid ~__context:__foreign_database_context ~self:pool
            in
...
let handle_metadata_vdis ~__context ~sr =
  ...
  let vdis_of_this_pool, vdis_of_foreign_pool =
    List.partition
