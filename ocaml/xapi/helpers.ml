...
module Stunnel : sig
  ...
  let is_cc_mode_enabled () =
    ...
    | unknown ->
        info
          "CC_PREPARATIONS has unexpected value '%s'. assuming CC mode is \
           disabled"
          unknown ;
        false
  ...
  module Config : sig
    ...
    let rewrite_xapi_ssl_config_file ~accept =
      let conf_contents =
        let cc_mode = is_cc_mode_enabled () in
        String.concat "\n"
          [
            "; autogenerated by xapi"
          ; Printf.sprintf "fips = %s" (if cc_mode then "yes" else "no")
          ; (if cc_mode then "" else "pid = /var/run/xapissl.pid")
