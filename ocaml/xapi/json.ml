...
let rec xmlrpc_to_json xml =
  ...
  in
  match xml with
  | Element ("i4", _, [PCData i]) ->
      i
  | Element ("boolean", _, [PCData b]) ->
      b
  | Element ("string", _, [PCData s]) ->
      "\"" ^ escape_string s ^ "\""
  | PCData s ->
      "\"" ^ escape_string s ^ "\""
  | Element ("double", _, [PCData d]) ->
      float_to_string (float_of_string d)
  | Element ("dateTime.iso8601", _, [PCData d]) ->
      "\"" ^ d ^ "\""
  | Element ("base64", _, [PCData b]) ->
      "\"" ^ b ^ "\""
      ...
      let map_func elts =
        ...
        | Element (n, _, _) ->
            Printf.fprintf stderr "%s"
              ("Bad XMLRPC (expecting member, got " ^ n ^ ")") ;
            "<bad!>"
        | _ ->
            failwith "Bad XMLRPC"
      in
  ...
  | Element (n, _, _) ->
      Printf.fprintf stderr "%s"
        ("Bad XMLRPC (expecting something, got " ^ n ^ ")") ;
      "<bad2 (got " ^ n ^ ">"
