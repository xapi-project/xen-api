...
let parse_xenpm_response resp =
  let parse_max_cstate_line line =
    let* cstate_word =
      ...
      with _ -> raise (Parse_cmd_output_failure resp)
    in
    match cstate_word with
    | "unlimited" ->
        None
    | s -> (
      try Scanf.sscanf s "C%d" Option.some
      with _ -> raise (Parse_cmd_output_failure resp)
    )
  in
...
let parse_xencmdline_response resp =
  ...
  | ["max_cstate"; state] -> (
    try (Some (int_of_string state), None)
    with Failure _ -> raise (Parse_cmd_output_failure resp)
  )
...
let xen_cmdline_get () =
  ...
  let resp =
    try Helpers.call_script !Xapi_globs.xen_cmdline_script args |> String.trim
    with e ->
      raise
        (Command_failure
