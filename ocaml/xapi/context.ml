...
let get_http_other_config http_req =
  ...
  |> List.filter (fun (k, v) -> Astring.String.is_prefix ~affix:http_other_config_hdr k)
...
let of_http_req ?session_id ?(internal_async_subtask=false) ~generate_task_for ~supports_async ~label ~http_req ~fd =
  ...
  in
  if internal_async_subtask then new_task_context ()
  else begin
