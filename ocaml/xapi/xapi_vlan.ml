...
let create ~__context ~tagged_PIF ~tag ~network =
  ...
  begin
    ...
    | Network_sriov_logical sriov :: _ ->
      Xapi_network_helpers.assert_network_compatible_with_vlan_on_sriov ~__context ~network ~sriov ~tagged_PIF
    | _ ->
      Xapi_network_helpers.assert_network_compatible_with_vlan_on_bridge ~__context ~network
  end;
