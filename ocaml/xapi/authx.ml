...
module AuthX : Auth_signature.AUTH_MODULE = struct
  ...
  let with_cmd cmd params_list fn =
    let debug_cmd =
      cmd
      ^ " "
      ^ List.fold_right (fun p pp -> "\"" ^ p ^ "\" " ^ pp) params_list ""
    in
    ...
    in
    let output_lines = Stdext.Xstringext.String.split '\n' output_str in
    fn output_lines
  ...
  *)
  ...
  let query_group_membership subject_identifier =
    ...
    (* here we remove the prefix u or g *)
    let sanitized_subject_id =
      String.sub sanitized_subject_id 1 (String.length sanitized_subject_id - 1)
    in
    match subject_identifier.[0] with
    | 'u' ->
