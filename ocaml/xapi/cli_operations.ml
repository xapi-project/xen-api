...
let gen_cmds rpc session_id =
	(make_param_funs (Client.Pool.get_all) (Client.Pool.get_all_records_where) (Client.Pool.get_by_uuid) (pool_record) "pool" [] ["uuid";"name-label";"name-description";"master";"default-SR"] rpc session_id) @
		...
		(make_param_funs (Client.VM.get_all) (Client.VM.get_all_records_where) (Client.VM.get_by_uuid) (vm_record) "snapshot" [("is-a-snapshot","true")] ["name-label";"name-description";"uuid";"snapshot_of"; "snapshot_time"; "is-snapshot-from-vmss"] rpc session_id) @
		...
		(make_param_funs (Client.VMSS.get_all) (Client.VMSS.get_all_records_where) (Client.VMSS.get_by_uuid) (vmss_record) "vmss" [] ["uuid";"name-label";"name-description";"enabled";"type";"retention-value";"frequency";"schedule";"last-run-time";"VMs"] rpc session_id) @
		(*
...
let vm_create printer rpc session_id params =
	...
	let vm = Client.VM.create ~rpc ~session_id ~name_label ~name_description ~user_version:0L ~is_a_template:false
		...
		~schedule_snapshot:Ref.null ~is_snapshot_from_vmss:false
		~appliance:Ref.null
		~start_delay:0L
		~shutdown_delay:0L
		~order:0L
...
let vmss_create printer rpc session_id params =
	...
	let retention_value = Int64.of_string(get "retention-value" ~default:"7") in
	let ref = Client.VMSS.create ~rpc ~session_id ~name_label ~name_description
		~enabled ~_type:ty ~retention_value ~frequency ~schedule
	in
