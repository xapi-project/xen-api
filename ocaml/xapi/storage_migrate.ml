...
let local_url () = Http.Url.(Http { host="127.0.0.1"; auth=None; port=None; ssl=false }, { uri = Constants.sm_uri; query_params=[] } )
...
module State = struct
  ...
  module Receive_state = struct
    ...
    let t_of_rpc x = match Rpcmarshal.unmarshal t.Rpc.Types.ty x with | Ok y -> y | Error (`Msg m) -> failwith (Printf.sprintf "Failed to unmarshal Receive_state.t: %s" m)
  end
  ...
  module Send_state = struct
    ...
    let typ_of_tapdev = Rpc.Types.(Abstract { aname = "tapdev"; test_data = []; rpc_of = Tapctl.rpc_of_tapdev; of_rpc=(fun x -> Ok (Tapctl.tapdev_of_rpc x)) })

    type handle = Scheduler.handle
    let typ_of_handle = Rpc.Types.(Abstract { aname = "handle"; test_data = []; rpc_of = Scheduler.rpc_of_handle; of_rpc=(fun x -> Ok (Scheduler.handle_of_rpc x)) })
    ...
    let rpc_of_t = Rpcmarshal.marshal t.Rpc.Types.ty
    let t_of_rpc x = match Rpcmarshal.unmarshal t.Rpc.Types.ty x with | Ok y -> y | Error (`Msg m) -> failwith (Printf.sprintf "Failed to unmarshal Send_state.t: %s" m)
  end
  ...
  module Copy_state = struct
    ...
    let rpc_of_t = Rpcmarshal.marshal t.Rpc.Types.ty
    let t_of_rpc x = match Rpcmarshal.unmarshal t.Rpc.Types.ty x with | Ok y -> y | Error (`Msg m) -> failwith (Printf.sprintf "Failed to unmarshal Copy_state.t: %s" m)
  end
...
let rpc ~srcstr ~dststr (url, pool_secret) =
  let remote_url_of_ip ip =
    ...
    | _ ->
      remote_url ip
  in
  let local_fn = Helpers.make_remote_rpc_of_url ~srcstr ~dststr (url, pool_secret) in
...
module Local = StorageAPI(Idl.Exn.GenClient(struct let rpc call = rpc ~srcstr:"smapiv2" ~dststr:"smapiv2" (local_url ()) call end))
...
let tapdisk_of_attach_info (backend:Storage_interface.backend) =
  ...
  | [] ->
    debug "No XenDisk implementation in backend: %s" (Storage_interface.(rpc_of backend) backend |> Rpc.to_string);
    None
...
let copy' ~task ~dbg ~sr ~vdi ~url ~dest ~dest_vdi =
  ...
  let module Remote = StorageAPI(Idl.Exn.GenClient(struct let rpc = rpc ~srcstr:"smapiv2" ~dststr:"dst_smapiv2" (storage_url remote_url) end)) in
  ...
  | Some alm ->
    begin
      ...
      | Some remote_info ->
        ...
        let remote_url = Http.Url.of_string remote_info.State.Send_state.url |> storage_url in
    ...
    (fun (id, send_state) ->
       begin
         ...
         List.iter log_and_ignore_exn
           [ (fun () -> stop dbg id);
             (fun () -> Local.DP.destroy dbg send_state.State.Send_state.local_dp true) ]
       end)
    send_ops;
  List.iter
    (fun (id, copy_state) ->
       begin
         ...
         List.iter log_and_ignore_exn
           [ (fun () -> Local.DP.destroy dbg copy_state.State.Copy_state.leaf_dp true);
             (fun () -> Local.DP.destroy dbg copy_state.State.Copy_state.base_dp true) ];
         let remote_url = Http.Url.of_string copy_state.State.Copy_state.remote_url |> storage_url in
         let module Remote = StorageAPI(Idl.Exn.GenClient(struct let rpc = rpc ~srcstr:"smapiv2" ~dststr:"dst_smapiv2" remote_url end)) in
         List.iter log_and_ignore_exn
           [ (fun () -> Remote.DP.destroy dbg copy_state.State.Copy_state.remote_dp true);
             (fun () -> Remote.VDI.destroy dbg copy_state.State.Copy_state.dest_sr copy_state.State.Copy_state.copy_vdi) ]
       end)
    copy_ops;
  List.iter
    (fun (id, recv_state) ->
...
let post_detach_hook ~sr ~vdi ~dp =
  ...
  Option.iter (fun r ->
      ...
      let t = Thread.create (fun () ->
          ...
          log_and_ignore_exn
            (fun () -> Remote.DATA.MIRROR.receive_finalize "Mirror-cleanup" id);
  ...
  try
    ...
    try
      ...
      let remote_base = match nearest with
        ...
        | None ->
          debug "Creating a blank remote VDI";
          Remote.VDI.create dbg dest { local_vdi with sm_config = [] }  in
...
let wrap ~dbg f =
  let task = Storage_task.add tasks dbg (fun task ->
      ...
      | Api_errors.Server_error(code, params) ->
        raise (Storage_error (Backend_error(code, params)))
      | Storage_error Unimplemented msg -> raise (Storage_error (Unimplemented msg))
      | e ->
        raise (Storage_error (Internal_error(Printexc.to_string e)))) in
  let _ = Thread.create
...
let update_snapshot_info_src ~dbg ~sr ~vdi ~url ~dest ~dest_vdi ~snapshot_pairs =
  let remote_url = Http.Url.of_string url |> storage_url in
  let module Remote =
    StorageAPI(Idl.Exn.GenClient(struct
