...
let rec make_vdis_and_vbds __context vmref i =
  if i = 0 then
    ...
    let snapshot_time = Clock.Date.epoch in
...
let make_tasks __context tasks =
  let create_description label =
    Printf.sprintf "my task is doing %s stuff" label
  in
  let pick_random l =
  ...
  for _ = 0 to tasks - 1 do
    ...
    in
    let task =
      Xapi_task.create ~__context ~label ~description:(create_description label)
    in
...
let do_populate ~vms ~vdis_per_vm ~networks ~srs ~tasks =
  Server_helpers.exec_with_new_task "populating dummy debug info"
    (fun __context ->
      ...
      let template =
        List.hd (Db.VM.get_by_name_label ~__context ~label:"Debian Sarge 3.1")
      in
