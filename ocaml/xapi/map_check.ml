...
let add_defaults requirements kvpairs =
  let key_is_present requirement =
    List.exists (fun (key, _) -> key = requirement.key) kvpairs
  in
  List.fold_left
    (fun acc requirement ->
...
let assert_value ~field ~key ~attr ~value =
  ...
  | EnumSet range ->
      ...
      let vs = Xapi_stdext_std.Xstringext.String.split_on_char ',' value in
      List.fold_right
        (fun v acc ->
...
let assert_key ~field ~ks ~key ~value =
  ...
  if not (List.mem_assoc key ks) then
    err field key value
  else
...
let assert_keys ~ty ~ks ~value ~db =
  ...
  with_ks ~kss:ks ~fn:(fun field (xt, ks) ->
      if xt = ty then
        Some
          ( assert_req_values ~field ~ks ~vs:value ;
            (* for this ks, each key value must be valid *)
            List.map
              (fun (k, v) -> (k, assert_key ~field ~ks ~key:k ~value:v))
              value
          )
      else
        None
