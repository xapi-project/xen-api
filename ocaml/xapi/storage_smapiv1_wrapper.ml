              ...
              (fun vdi_t ->
                ...
                try
                  ignore
                    (List.fold_left
                       (fun _ op ->
                         perform_nolock context ~dbg ~name:"VDI.destroy_datapath_nolock" ~dp ~sr ~vdi ~vm op
                       )
                       vdi_t ops
                    )
                with e ->
      ...
      let attach3 context ~dbg ~dp ~sr ~vdi ~vm ~read_write =
        ...
        with_vdi sr vdi (fun () ->
            remove_datapaths_andthen_nolock context ~dbg ~sr ~vdi ~vm Vdi.leaked
              (fun () ->
                let state =
                  perform_nolock context ~dbg ~name:"VDI.attach3" ~dp ~sr ~vdi ~vm
      ...
      let activate3 context ~dbg ~dp ~sr ~vdi ~vm =
        ...
        with_vdi sr vdi (fun () ->
            remove_datapaths_andthen_nolock context ~dbg ~sr ~vdi ~vm Vdi.leaked
              (fun () ->
                ignore
                  (perform_nolock context ~dbg ~dp ~name:"VDI.activate3" ~sr ~vdi ~vm
                     Vdi_automaton.Activate
                  )
            )
        )
      ...
      let deactivate context ~dbg ~dp ~sr ~vdi ~vm =
        ...
        with_vdi sr vdi (fun () ->
            remove_datapaths_andthen_nolock context ~dbg ~sr ~vdi ~vm Vdi.leaked
              (fun () ->
                ignore
                  (perform_nolock context ~dbg ~name:"VDI.deactivate" ~dp ~sr ~vdi ~vm
                     Vdi_automaton.Deactivate
                  )
            )
        )
      ...
      let detach context ~dbg ~dp ~sr ~vdi ~vm =
        ...
        with_vdi sr vdi (fun () ->
            remove_datapaths_andthen_nolock context ~dbg ~sr ~vdi ~vm Vdi.leaked
              (fun () ->
                ignore
                  (perform_nolock context ~dbg ~name:"VDI.detach" ~dp ~sr ~vdi ~vm
                     Vdi_automaton.Detach
                  )
            )
        )
