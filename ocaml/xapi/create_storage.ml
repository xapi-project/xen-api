...
let maybe_remove_tools_sr rpc session_id __context =
  ...
  let unplug_and_maybe_destroy sr =
    ...
    | Api_errors.Server_error (e, _) when e = Api_errors.sr_has_pbd ->
      info "Tools SR %s could not be removed: it is still in use by another host" (Ref.string_of sr)
    | e ->
      warn "Failed to remove redundant Tools SR %s: %s"
        (Ref.string_of sr) (Printexc.to_string e)
  in
...
let initialise_storage (me: API.ref_host) rpc session_id __context : unit =
  let create_pbds_for_shared_srs () =
    ...
    let shared_srs = List.filter (fun (_,sr_rec) ->
      sr_rec.API.sR_shared &&
      (!Xapi_globs.create_tools_sr || not sr_rec.API.sR_is_tools_sr)) srs in
...
let initialise_storage_localhost rpc session_id : unit =
  Server_helpers.exec_with_new_task "initialising storage"
    (fun context->
