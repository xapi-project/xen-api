...
let checksum_supported_extension =
  [checksum_extension_xxh; checksum_extension_sha]

(* Supported checksum formats *)

type vdi = string (* directory prefix in tar file *) * API.ref_VDI * Int64.t

(* size to send/recieve *)
...
let new_progress_record __context (prefix_vdis : vdi list) =
  {
    total_size=
      List.fold_left (fun tot (_, _, s) -> Int64.add tot s) 0L prefix_vdis
...
let made_progress __context progress n =
  ...
  let fraction_complete =
    Int64.to_float so_far_MiB /. Int64.to_float total_size_MiB
  in
...
let get_chunk_numbers_in_increasing_order descriptor_list offset =
  ...
  let rec range acc start_chunk end_chunk =
    if end_chunk < start_chunk then
      acc
    else
      range (end_chunk :: acc) start_chunk Int64.(add end_chunk minus_one)
  in
  let is_empty e =
  ...
  let get_non_empty_chunks descriptor offset increment =
    ...
    | false ->
        ...
        let end_chunk =
          Int64.(div (add (add descriptor.length offset) minus_one) increment)
        in
  ...
  let add_new acc b =
    match acc with hd :: _ when Int64.equal b hd -> acc | _ -> b :: acc
  in
  ...
  let rec add_unique_chunks acc = function
    | [] ->
        acc
    | x :: xs ->
        add_unique_chunks (add_new acc x) xs
  in
...
let verify_inline_checksum ifd checksum_table hdr =
  ...
  then (
    let msg =
      Printf.sprintf
        "Expected to find a supported inline checksum file, instead found a \
         file called: %s"
        file_name
    in
          ...
          if remaining > 0L then (
            ...
            let suffix =
              String.sub file_name
                ...
                (String.length file_name - String.length prefix - 1)
            in
            ...
            let num_zero_blocks =
              int_of_string suffix - int_of_string last_suffix - 1
            in
            let skipped_size =
              Int64.mul
                (Int64.of_int !firstchunklength)
                (Int64.of_int num_zero_blocks)
            in
            ...
            let buffer =
              if length = chunk_size then
                ...
                Bytes.make (Int64.to_int length) '\000'
            in
            ...
            let csum =
              ...
              | m when m = checksum_extension_xxh ->
                  Printf.sprintf "%016LX" (XXHash.XXH64.hash buffer_string)
              | m when m = checksum_extension_sha ->
                  Sha1.to_hex (Sha1.string buffer_string)
              | _ ->
                  let msg =
                    Printf.sprintf "Found unsupported checksum file: %s"
                      csum_file_name
                  in
                  error "%s" msg ; raise (Failure msg)
            in
