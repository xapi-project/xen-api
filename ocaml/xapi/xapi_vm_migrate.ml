...
let rec migrate_with_retries ~__context max try_no dbg vm_uuid xenops_vdi_map xenops_vif_map xenops =
        ...
        if try_no >= max then
                ...
                with e ->
                        let isCancelEx = ref 0 in
                        let extra = match e with
                                        | Xenops_interface.Cancelled s ->
                                                isCancelEx := 1;
                                                " (expected if the VM shuts down)"
                                        | _ -> "" in

                        info "xenops: will retry migration: caught %s%s in attempt %d of %d."
                                (Printexc.to_string e) extra try_no max;

                        (* CA-140684 VM Migrate completes even though the operation is cancelled [via XenCenter]
                         * If VM Migrate is being cancelled by user manually, don't try again even
                         * Xenops_interface.Cancelled exception is thrown out *)
