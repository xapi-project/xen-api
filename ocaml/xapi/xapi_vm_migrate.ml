  ...
  with e ->
    ...
    begin
      ...
      | Some mid ->
        ...
        if m.Mirror.failed
        then raise (Api_errors.Server_error(Api_errors.mirror_failed,[Ref.string_of vconf.vdi]));
      | None ->
        ()
    end;
...
let migrate_send'  ~__context ~vm ~dest ~live ~vdi_map ~vif_map ~options =
  ...
  try
    ...
    let new_vm =
      ...
      (* If it's an intra-pool migrate, the VBDs will still be 'currently-attached=true'
