  ...
  try
    ...
    Helpers.call_api_functions ~__context (fun rpc session_id ->
        if not is_intra_pool && not copy then begin
          ...
          List.iter (fun self ->
            Db.VM.destroy ~__context ~self; (* this does not automatically destroy any associated VDA record *)
            Xapi_vda.find_vda ~__context ~vm:self (* manually destroy VDA record if one exists *)
              |> Opt.iter (fun vda ->
