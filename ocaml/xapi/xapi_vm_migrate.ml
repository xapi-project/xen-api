  ...
  try
    ...
    let all_vdis =
      ...
      let eldest_first =
        ...
        in
        List.sort (fun v1 v2 -> compare (key v1) (key v2))
      in
      let children =
        List.fold_left
          (fun acc v ->
          ...
          VdiMap.empty all_vdis
        |> VdiMap.map eldest_first
      in
      let rec walk v acc rest =
        match (VdiMap.find_opt v.vdi children, rest) with
        | (None | Some []), [] ->
            v :: acc
        | (None | Some []), hd :: tl ->
            walk hd (v :: acc) tl
        | Some (first :: others), _ ->
            walk first (v :: acc) (List.append others rest)
      in
      ...
      |> List.map (fun root ->
          let tree = List.rev (walk root [] []) in
          (tree_size tree, tree)
      )
