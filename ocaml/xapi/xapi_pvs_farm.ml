...
let forget ~__context ~self =
  ...
  let running_proxies = Pvs_proxy_control.get_running_proxies ~__context ~farm:self in
...
let set_name ~__context ~self ~value =
  let px = Pvs_proxy_control.get_running_proxies ~__context ~farm:self in
  if px <> [] then
...
let sr_is_in_use ~__context ~self sr =
  Pvs_proxy_control.get_running_proxies ~__context ~farm:self
