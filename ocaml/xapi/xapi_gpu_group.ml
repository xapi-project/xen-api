...
let destroy ~__context ~self =
  ...
  let connected =
    List.filter
      (fun self -> Db.VGPU.get_currently_attached ~__context ~self)
      vgpus
  in
  if connected <> [] then
    raise
...
let get_remaining_capacity_internal ~__context ~self ~vgpu_type =
  ...
  if capacity > 0L then
    ...
    Error (choose_exception exceptions)
