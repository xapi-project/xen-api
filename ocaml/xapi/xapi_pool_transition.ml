...
let attempt_two_phase_commit_of_new_master ~__context (manual: bool) (peer_addresses: string list) (my_address: string) = 
  ...
  if !hosts_which_failed <> [] then begin
    ...
    Db_cache.DBCache.flush_and_exit (Db_connections.preferred_write_db ()) Xapi_globs.restart_return_code
  end;
  ...
  if not(manual) then
    ...
    else Db_cache.DBCache.flush_and_exit (Db_connections.preferred_write_db ()) Xapi_globs.restart_return_code;
