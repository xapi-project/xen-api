...
type response = Success | Exception of exn | NoResponse
  ...
  | __context, timeout, delay, req ->
      ...
      Mutex.execute req.resp_mutex (fun () ->
          if !(req.resp) = NoResponse then (
            ...
            let resources =
              Locking_helpers.Thread_state.get_acquired_resources_by_task
                req.task
            in
  ...
  with exn ->
    if req.enable_log then
      warn "Exception handling remote request %s: %s"
        (Option.value ~default:"" req.request.Http.Request.body)
