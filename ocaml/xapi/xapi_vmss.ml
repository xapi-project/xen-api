...
let schedule_frequency_hourly_keys =
  ( schedule_field
  , [
...
let set_schedule ~__context ~self ~value =
  assert_licensed ~__context ;
  let value =
    Map_check.assert_keys ~ty:"" ~ks:schedule_all_keys ~value
...
let add_to_schedule ~__context ~self ~key ~value =
  ...
  with e ->
    Db.VMSS.add_to_schedule ~__context ~self ~key
      ~value:(List.assoc key !schedule_backup) ;
    raise e
...
let create ~__context ~name_label ~name_description ~enabled ~_type
    ~retained_snapshots ~frequency ~schedule : API.ref_VMSS =
  assert_licensed ~__context ;
  ...
  let (_ : (string * string) list) =
    Map_check.assert_keys
      ~ty:(Record_util.vmss_frequency_to_string frequency)
      ~ks:schedule_keys ~value:schedule ~db:[]
  in
...
let destroy ~__context ~self =
  ...
  if List.length vms > 0 then (* we can't delete a VMSS that contains VMs *)
  ...
  if is_vmss_snapshot ~__context then
    ...
    try
      ...
      Db.Task.add_to_other_config ~__context ~self:task
        ~key:vmss_snapshot_other_config_show_in_xencenter ~value:"" ;
      Db.Task.add_to_other_config ~__context ~self:task
        ~key:vmss_snapshot_other_config_applies_to ~value:(Ref.string_of vm)
    with e ->
