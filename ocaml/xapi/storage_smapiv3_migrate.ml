...
let nbd_proxy_startup_delay = 0.5
...
(** A VM snapshot tree node projected onto one disk (see
    [find_lineage_vdi_on_snapshot_vm]). Branching means the VM was reverted to
    that snapshot: one child is on the active path to the live VM, the rest are
    orphaned branches that must still be reproduced on the destination. *)
...
(* [lineage] is the [VDI.snapshots] set of the disk being migrated *)
...
(** Snapshot subtree rooted at [snapshot_vm], projected onto the disk's
    [lineage]: a snapshot VM with no VDI in [lineage] is dropped and its
    disk-bearing descendants promoted into its place. *)
...
(** Snapshot tree of the disk [vdi], roots sorted oldest first. Disk identity
    comes from [VDI.snapshots], not the VBD slot: that survives revert and slot
    reuse. *)
...
let prepare_branch_vdi ~ctx ~dest_snapshot =
  ...
  in
  (* The caller only arms [cleanup] after we return, so a failure here must undo
     the clone itself. *)
  let nbd_uri =
    try
      let backend =
...
let mirror_node_into ~ctx ~working_vdi ~working_dp ~nbd_uri ~counter ~total node
  ...
  in
  Unix.sleepf nbd_proxy_startup_delay ;
  let dest_snapshot =
...
(** [working_vdi] is the destination VDI impersonating the source leaf:
    [mirror_vdi] at the root, a fresh clone of the parent snapshot inside each
    inactive subtree. Inactive children must run first, so the active-path child
    inherits the original [mirror_vdi] and the deepest active node is left
    positioned for live-leaf mirroring. *)
...
module MIRROR : SMAPIv2_MIRROR = struct
  ...
  let send_start _ctx ~dbg ~task_id:_ ~dp ~sr ~vdi ~image_format ~mirror_vm
    ...
    | Mirror.SMAPIv3_mirror {nbd_export; mirror_datapath; mirror_vdi} -> (
        ...
        try
          (* Do not re-activate the dest leaf readonly here: it stays writable
             from receive_start3 and qemu-dp needs a writable mirror target. *)
