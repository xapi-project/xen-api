...
let get_pif_device_internal ~__context ~interface_tables ~pif_rec =
  let dbg = Context.string_of_task __context in
...
(* For different pif types, get the proper device name
   ...
   - VLAN_untagged: get the tagged PIF, then get the device of the tagged PIF
   - Network_sriov_logical: get the physical PIF, then get the device of the physical PIF
...
let get_pif_device ~__context ~interface_tables ~pif_rec =
  ...
  | VLAN_untagged vlan ->
      let tagged_pif_ref = Db.VLAN.get_tagged_PIF ~__context ~self:vlan in
      let tagged_pif = Db.PIF.get_record ~__context ~self:tagged_pif_ref in
      get_pif_device_internal ~__context ~interface_tables ~pif_rec:tagged_pif
  | Network_sriov_logical sriov ->
      let physical_pif_ref =
        Db.Network_sriov.get_physical_PIF ~__context ~self:sriov
      in
      let physical_pif = Db.PIF.get_record ~__context ~self:physical_pif_ref in
      get_pif_device_internal ~__context ~interface_tables ~pif_rec:physical_pif
  | Physical _ ->
      get_pif_device_internal ~__context ~interface_tables ~pif_rec
  | Tunnel_access _ | Bond_master _ ->
      (* Tunnel device: tunnel<N>, Bond device: bond<N>, they are not related
         with a physical network interface. No need to handle the name update. *)
      pif_rec.API.pIF_device
