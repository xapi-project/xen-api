...
let vLAN_create ~__context ~tagged_PIF ~tag ~network =
	...
	(* Check that the tagged PIF is not a VLAN itself - CA-25160.
	 * This check can be skipped using the allow_vlan_on_vlan FIST point. *)
	let origtag = Db.PIF.get_VLAN ~__context ~self:tagged_PIF in
	if origtag >= 0L && not (Xapi_fist.allow_vlan_on_vlan ())
	then raise (Api_errors.Server_error
		(Api_errors.pif_is_vlan, [Ref.string_of tagged_PIF]));

	(* Check the VLAN tag is sensible;
  ...
  let other_keys =
	...
	then raise (Api_errors.Server_error
		(Api_errors.pif_vlan_exists, [device]));

	if Db.PIF.get_tunnel_access_PIF_of
		~__context ~self:tagged_PIF <> []
	then raise (Api_errors.Server_error
	...
	let t = make_tables ~__context ~host in
	let vlan = Ref.make ()
	and vlan_uuid = Uuid.to_string (Uuid.make_uuid ()) in

	(* NB we attach the untagged PIF to the supplied network *)
	let untagged_PIF = introduce_internal
		~physical:false ~t ~__context ~host ~mAC:vlan_mac
		~device ~vLAN:tag ~mTU ~vLAN_master_of:vlan ~network () in
	let () = Db.VLAN.create
		~__context ~ref:vlan ~uuid:vlan_uuid
		~tagged_PIF ~untagged_PIF ~tag ~other_config:[] in
	vlan
...
let create_VLAN ~__context ~device ~network ~host ~vLAN =
	...
	let vlan = vLAN_create
		~__context ~tagged_PIF:base_pif ~tag:vLAN ~network in
...
let destroy ~__context ~self =
	...
	then raise (Api_errors.Server_error
		(Api_errors.pif_is_physical, []));
	(* Because of the precondition in create_VLAN, this will
	 * always be the only PIF connecting this host to the network.
	 * Therefore it is safe to detach the network. *)
...
let vLAN_destroy ~__context ~self =
	...
	if
		try ignore (Db.PIF.get_uuid ~__context ~self:untagged_PIF); false
		with _ -> true
	then begin
