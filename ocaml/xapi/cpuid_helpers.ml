...
let get_pool_feature_mask ~__context =
...
let get_host_compatibility_info ~__context ~host =
...
let populate_cpu_flags ~__context ~vm ~host =
	let host_cpu_vendor, host_cpu_features = get_host_compatibility_info ~__context ~host in
	let vm_cpu_flags = [
...
let assert_vm_is_compatible ~__context ~vm ~host =
	...
	if List.mem_assoc features_key vm_cpu_info then begin
		...
		let pool_mask = get_pool_feature_mask ~__context in
