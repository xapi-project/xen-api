...
let create ~__context ~pif ~network =
  ...
  if pif_rec.API.pIF_sriov_physical_PIF_of <> [] then
    raise (Api_errors.Server_error (Api_errors.network_sriov_already_enabled, [Ref.string_of pif]));
  if not (List.mem "sriov" pif_rec.API.pIF_capabilities) then
    raise (Api_errors.Server_error (Api_errors.pif_is_not_sriov_capable, [Ref.string_of pif]));
