  ...
  then Some(Api_errors.other_operation_in_progress,["VDI"; _ref])
    ...
    then Some(Api_errors.sr_no_pbds, [Ref.string_of sr])
      ...
      let allowed_for_cbt_metadata_vdi = match op with
        ...
        | `blocked | `data_destroy | `destroy | `export_changed_blocks | `force_unlock | `forget | `generate_config | `scan | `update -> true in
      ...
      let allowed_when_cbt_enabled = match op with
        ...
        | `blocked | `clone | `copy | `data_destroy | `destroy | `disable_cbt | `enable_cbt | `export_changed_blocks | `force_unlock | `forget | `generate_config | `resize | `resize_online | `scan | `snapshot | `update -> true in
      ...
      | `mirror | `clone | `generate_config | `scan | `force_unlock | `set_on_boot | `export_changed_blocks | `blocked | `update -> None
      end
