...
let hash_table_of_leaf_xml_element_list list =
  Hashtblext.of_list (List.filter_map hash_table_entry_of_leaf_xml_element list)
...
module DaemonConfiguration = struct
  ...
  module Host = struct
    ...
    let of_host_t host_t =
      {uuid= host_t.host_uuid; address= host_t.host_address}
  ...
  (** See interface. *)
  let create ?(common_udp_port = 49154) ?heart_beat_interval
      ?state_file_interval ?heart_beat_timeout ?state_file_timeout
      ?heart_beat_watchdog_timeout ?state_file_watchdog_timeout
      ?boot_join_timeout ?enable_join_timeout ?xapi_healthcheck_interval
      ?xapi_healthcheck_timeout ?xapi_restart_attempts ?xapi_restart_timeout
      ?xapi_licensecheck_timeout ~common_generation_uuid
    ...
    let common_hosts =
      List.map (fun (_, host) -> Host.of_host_t host) records
    in
    let local_host_uuid =
      Db.Host.get_uuid ~__context ~self:!Xapi_globs.localhost_ref
    in
    {
      common_hosts
  ...
  let int_parameter (name, param) =
    Option.fold ~none:[]
      ~some:(fun x -> [xml_leaf_element name (string_of_int x)])
      param
  ...
  let to_xml_string config =
    ...
    ^ Xml.to_string_fmt (to_xml_element config)
end
...
module LiveSetInformation = struct
  module Status = struct
    ...
    let of_string string =
      ...
      | "starting" ->
          Some Starting
      | _ ->
          invalid_arg "Invalid status string."
      ...
      | Xml.Element ("host_raw_data", _, children) ->
          ...
          Some
            ...
            ; time_since_last_update_on_statefile=
                int (find "time_since_last_update_on_statefile")
            ; time_since_last_heartbeat= int (find "time_since_last_heartbeat")
            ; time_since_xapi_restart_first_attempted=
                int (find "time_since_xapi_restart_first_attempted")
            ; heartbeat_active_list_on_heartbeat=
                set uuid (find "heartbeat_active_list_on_heartbeat")
            ; heartbeat_active_list_on_statefile=
                set uuid (find "heartbeat_active_list_on_statefile")
            }
      | _ ->
          None
  end
      ...
      | Xml.Element ("warning_on_local_host", _, children) ->
          ...
          Some
            ...
            ; heartbeat_approaching_timeout=
                bool "heartbeat_approaching_timeout"
            ; statefile_approaching_timeout=
                bool "statefile_approaching_timeout"
            ; xapi_healthcheck_approaching_timeout=
                bool "Xapi_healthcheck_approaching_timeout"
  ...
  (** See interface. *)
  let of_xml_string string = of_xml_element (Xml.parse_string string)
