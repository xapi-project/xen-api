  ...
  | local_pif :: _ ->
    begin match get_underlying_pif ~__context ~pif:local_pif with
      | Some underlying_pif ->
        ...
        if num > 0L then Some num else None
      | None -> None
    end
  | [] -> None
...
let group_hosts_by_best_sriov ~__context ~network =
  ...
  let attached_hosts, unattached_hosts = List.fold_left (fun (l1, l2) pif ->
    ...
    if Db.PIF.get_currently_attached ~__context ~self:pif then begin
      match get_remaining_capacity_on_localhost ~__context ~host ~network with
      | Some num -> ((host, num) :: l1, l2)
      | None -> (l1,l2)
    end else begin
...
let reserve_sriov_vfs ~__context ~host ~vm =
  ...
  List.iter (fun vif ->
      ...
      with
      | Not_found -> raise Api_errors.(Server_error (internal_error, ["Cannot find PF PCI for reservation on localhost"]))
      | e -> raise e
    ) vifs
