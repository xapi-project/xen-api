...
let gen_random_bool () = Random.int 2 = 0

let gen_random_list f =
  let len = Random.int 50 in
  let rec makesl x = match x with 0 -> [] | n -> f () :: makesl (n - 1) in
  makesl len
...
let gen_random_structured_op () =
  ...
  | 3 ->
      RemoveMap
  | _ ->
      raise RandomRangeError

(* should never be thrown *)

let gen_random_2string () = (gen_random_string (), gen_random_string ())
...
let test_gtfr_args () =
  tm unmarshall_get_table_from_ref_args marshall_get_table_from_ref_args
    (gen_random_string ())

let test_gtfr_response () =
  tm unmarshall_get_table_from_ref_response marshall_get_table_from_ref_response
    (gen_random_string_option ())

let test_ivr_args () =
  tm unmarshall_is_valid_ref_args marshall_is_valid_ref_args
    (gen_random_string ())

let test_ivr_response () =
  tm unmarshall_is_valid_ref_response marshall_is_valid_ref_response
    (gen_random_bool ())

let test_rr_args () =
  tm unmarshall_read_refs_args marshall_read_refs_args (gen_random_string ())

let test_rr_response () =
  tm unmarshall_read_refs_response marshall_read_refs_response
    (gen_random_list gen_random_string)

let test_rfw_args () =
  tm unmarshall_read_field_where_args marshall_read_field_where_args
    (gen_random_where ())
...
let test_dr_args () =
  tm unmarshall_delete_row_args marshall_delete_row_args (gen_random_2string ())

let test_dr_response () =
  tm unmarshall_delete_row_response marshall_delete_row_response ()

let test_wf_args () =
  tm unmarshall_write_field_args marshall_write_field_args
    (gen_random_4string ())

let test_wf_response () =
  tm unmarshall_write_field_response marshall_write_field_response ()

let test_rf_args () =
  tm unmarshall_read_field_args marshall_read_field_args (gen_random_3string ())

let test_rf_response () =
  tm unmarshall_read_field_response marshall_read_field_response
    (gen_random_string ())

let test_psf_args () =
  tm unmarshall_process_structured_field_args
...
let test_readrec_args () =
  tm unmarshall_read_record_args marshall_read_record_args
...
let () =
  let to_alco (test, name) = (name, [(name, `Quick, test)]) in
  Alcotest.run "Marshalling and unmarshalling database records"
    (List.map to_alco tests)
