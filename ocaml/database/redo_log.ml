...
let get_latest_response_time () =
  let now = Unix.gettimeofday() in
  now +. Xapi_globs.redo_log_max_block_time
...
let action_empty sock datasockpath =
  ...
  let latest_response_time = get_latest_response_time() in
  (* Empty *)
    ...
    try
      ...
      | Some (_, ctrlsockpath, _) -> 
        begin
          ...
          | None ->
            let latest_connect_time = get_latest_response_time() in
