...
functor
  ...
  struct
    ...
    let process_exception_xml xml =
      ...
      | _ ->
          raise Remote_db_server_returned_unknown_exception
    ...
    let do_remote_call marshall_args unmarshall_resp fn_name args =
      ...
      let xml =
        XMLRPC.To.array
          [XMLRPC.To.string fn_name; XMLRPC.To.string "" (* unused *); xml]
      in
      let resp = RPC.rpc (Xml.to_string xml) |> Xml.parse_string in
      match XMLRPC.From.array (fun x -> x) resp with
      | [status_xml; resp_xml] ->
          let status = XMLRPC.From.string status_xml in
          if status = "success" then
            unmarshall_resp resp_xml
          else
            process_exception_xml resp_xml
      | _ ->
          raise Remote_db_server_returned_bad_message

    let get_table_from_ref _ x =
      do_remote_call marshall_get_table_from_ref_args
        unmarshall_get_table_from_ref_response "get_table_from_ref" x

    let is_valid_ref _ x =
      do_remote_call marshall_is_valid_ref_args unmarshall_is_valid_ref_response
        "is_valid_ref" x

    let read_refs _ x =
      do_remote_call marshall_read_refs_args unmarshall_read_refs_response
        "read_refs" x

    let read_field_where _ x =
      do_remote_call marshall_read_field_where_args
        unmarshall_read_field_where_response "read_field_where" x
    ...
    let read_field _ x y z =
      do_remote_call marshall_read_field_args unmarshall_read_field_response
        "read_field" (x, y, z)

    let find_refs_with_filter _ s e =
      do_remote_call marshall_find_refs_with_filter_args
        unmarshall_find_refs_with_filter_response "find_refs_with_filter" (s, e)

    let read_record _ x y =
      do_remote_call marshall_read_record_args unmarshall_read_record_response
        "read_record" (x, y)
