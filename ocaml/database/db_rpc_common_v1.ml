...
let unmarshall_2strings xml =
  ...
  | [x1; x2] ->
      (XMLRPC.From.string x1, XMLRPC.From.string x2)
  | _ ->
...
let unmarshall_3strings xml =
  match XMLRPC.From.array (fun x -> x) xml with
  | [x1; x2; x3] ->
      (XMLRPC.From.string x1, XMLRPC.From.string x2, XMLRPC.From.string x3)
  | _ ->
      raise DB_remote_marshall_error

let marshall_stringlist sl = XMLRPC.To.array (List.map XMLRPC.To.string sl)
...
let marshall_stringopt x =
  ...
  | None ->
      XMLRPC.To.array []
  | Some x ->
      XMLRPC.To.array [XMLRPC.To.string x]
...
let unmarshall_stringopt xml =
  ...
  | [] ->
      None
  | [xml] ->
      Some (XMLRPC.From.string xml)
  | _ ->
      raise DB_remote_marshall_error

let marshall_expr x = Db_filter.xml_of_expr x

let unmarshall_expr xml = Db_filter.expr_of_xml xml
...
let marshall_read_field_where_response sl = marshall_stringlist sl

let unmarshall_read_field_where_response xml = unmarshall_stringlist xml

(* db_get_by_uuid *)
let marshall_db_get_by_uuid_args (s1, s2) = marshall_2strings (s1, s2)

let unmarshall_db_get_by_uuid_args xml = unmarshall_2strings xml
...
let unmarshall_db_get_by_name_label_args xml = unmarshall_2strings xml

let marshall_db_get_by_name_label_response sl = marshall_stringlist sl

let unmarshall_db_get_by_name_label_response xml = unmarshall_stringlist xml
...
let marshall_create_row_response () = marshall_unit ()

let unmarshall_create_row_response xml = unmarshall_unit xml

(* delete_row *)
let marshall_delete_row_args (s1, s2) =
  XMLRPC.To.array [XMLRPC.To.string s1; XMLRPC.To.string s2]
...
let marshall_delete_row_response () = marshall_unit ()

let unmarshall_delete_row_response xml = unmarshall_unit xml

(* write_field *)
let marshall_write_field_args (s1, s2, s3, s4) =
  XMLRPC.To.array (List.map XMLRPC.To.string [s1; s2; s3; s4])
...
let marshall_write_field_response () = marshall_unit ()

let unmarshall_write_field_response xml = unmarshall_unit xml

(* read_field *)
let marshall_read_field_args (s1, s2, s3) =
  XMLRPC.To.array (List.map XMLRPC.To.string [s1; s2; s3])

let unmarshall_read_field_args xml =
  match XMLRPC.From.array (fun x -> x) xml with
  | [s1x; s2x; s3x] ->
      (XMLRPC.From.string s1x, XMLRPC.From.string s2x, XMLRPC.From.string s3x)
  | _ ->
      raise DB_remote_marshall_error

let marshall_read_field_response s = XMLRPC.To.string s

let unmarshall_read_field_response xml = XMLRPC.From.string xml
...
let unmarshall_find_refs_with_filter_args xml =
  match XMLRPC.From.array (fun x -> x) xml with
  | [s; e] ->
      (XMLRPC.From.string s, unmarshall_expr e)
  | _ ->
      raise DB_remote_marshall_error

let marshall_find_refs_with_filter_response sl = marshall_stringlist sl

let unmarshall_find_refs_with_filter_response xml = unmarshall_stringlist xml
...
let marshall_process_structured_field_response () = marshall_unit ()

let unmarshall_process_structured_field_response xml = unmarshall_unit xml
