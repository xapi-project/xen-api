...
let redo_log_max_block_time_empty =
  ref (Mtime.Span.(2 * s) |> Unixext.Timeout.of_span)
...
let redo_log_max_block_time_read =
  ref (Mtime.Span.(30 * s) |> Unixext.Timeout.of_span)
...
let redo_log_max_block_time_writedelta =
  ref (Mtime.Span.(2 * s) |> Unixext.Timeout.of_span)
...
let redo_log_max_block_time_writedb =
  ref (Mtime.Span.(30 * s) |> Unixext.Timeout.of_span)

(** The maximum amount of time to wait for a connection (used to be hardcoded to 3600s) *)
let redo_log_max_connect_time =
  ref (Mtime.Span.(1 * hour) |> Unixext.Timeout.of_span)
...
let redo_log_max_startup_time =
  ref (Mtime.Span.(5 * s) |> Unixext.Timeout.of_span)
