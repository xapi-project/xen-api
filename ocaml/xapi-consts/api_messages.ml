...
let ha_pool_overcommitted = addMessage "HA_POOL_OVERCOMMITTED" 3L

(* GUI maximizes ntol, which means this is often subsumed by DROP_IN_PLAN_EXISTS_FOR; hence low priority *)
...
let vif_qos_failed = addMessage "VIF_QOS_FAILED" 3L

(* Used in idl/datamodel.ml *)

let vbd_qos_failed = addMessage "VBD_QOS_FAILED" 3L

(* Used in idl/datamodel.ml *)

let vcpu_qos_failed = addMessage "VCPU_QOS_FAILED" 3L

(* Used in idl/datamodel.ml *)

let vm_started = addMessage "VM_STARTED" 5L

let vm_shutdown = addMessage "VM_SHUTDOWN" 5L
...
let vm_secure_boot_failed = addMessage "VM_SECURE_BOOT_FAILED" 5L

(* Raised by varstored *)

let host_sync_data_failed = addMessage "HOST_SYNC_DATA_FAILED" 3L

(* Kept for backward compatibility; used in XenCenter *)

let host_clock_skew_detected = addMessage "HOST_CLOCK_SKEW_DETECTED" 3L

let host_clock_went_backwards = addMessage "HOST_CLOCK_WENT_BACKWARDS" 1L
...
let wlb_optimization_alert = addMessage "WLB_OPTIMIZATION_ALERT" 3L
...
let v6_server_up = addMessage "LICENSE_SERVER_CONNECTED" 4L

(* Used in XenCenter *)

let v6_server_down = addMessage "LICENSE_SERVER_UNAVAILABLE" 3L
...
let pvs_proxy_setup_failed = addMessage "PVS_PROXY_SETUP_FAILED" 3L

(* Setting up pvs proxy rules or pvs proxy daemon initialisation failed internally *)
...
let pvs_proxy_sr_out_of_space = addMessage "PVS_PROXY_SR_OUT_OF_SPACE" 3L

(* PVS cache storage size exceeds available space on SR *)

(* VMPP message types *)
let vmpp_snapshot_lock_failed = addMessage "VMPP_SNAPSHOT_LOCK_FAILED" 3L

(*'The snapshot phase is already executing for this protection policy. Please try again later'*)

let vmpp_snapshot_succeeded = addMessage "VMPP_SNAPSHOT_SUCCEEDED" 5L

(*'Successfully performed the snapshot phase of the protection policy'*)

let vmpp_snapshot_failed = addMessage "VMPP_SNAPSHOT_FAILED" 3L

(*'The snapshot phase of the protection policy failed.'*)

let vmpp_archive_lock_failed = addMessage "VMPP_ARCHIVE_LOCK_FAILED" 3L

(*'The archive sub-policy is already executing for some protection policy in the pool.Please try again later'*)

let vmpp_archive_failed_0 = addMessage "VMPP_ARCHIVE_FAILED_0" 3L

(*'The archive phase failed for this protection policy'*)

let vmpp_archive_suceeded = addMessage "VMPP_ARCHIVE_SUCCEEDED" 5L
...
let vmpp_license_error = addMessage "VMPP_LICENSE_ERROR" 3L

(*'This operation is not allowed under your license.  Please contact your support representative'*)

let vmpp_xapi_logon_failure = addMessage "VMPP_XAPI_LOGON_FAILURE" 3L

(*'Could not login to API session.'*)

let vmpp_snapshot_missed_event = addMessage "VMPP_SNAPSHOT_MISSED_EVENT" 3L

(*'A scheduled snapshot event was missed due to another on-going scheduled snapshot run. This is unexpected behaviour, please re-configure your snapshot sub-policy',*)

let vmpp_archive_missed_event = addMessage "VMPP_ARCHIVE_MISSED_EVENT" 3L

(*'A scheduled archive event was missed due to another on-going scheduled archive run. This is unexpected behaviour, please re-configure your archive sub-policy'*)
...
(* VMSS message types *)
let vmss_snapshot_lock_failed = addMessage "VMSS_SNAPSHOT_LOCK_FAILED" 3L

(*'The snapshot is already executing for schedule snapshot. Please try again later'*)

let vmss_snapshot_succeeded = addMessage "VMSS_SNAPSHOT_SUCCEEDED" 5L

(*'Successfully performed the snapshot of the schedule snapshot'*)

let vmss_snapshot_failed = addMessage "VMSS_SNAPSHOT_FAILED" 3L

(*'The snapshot of the schedule snapshot has failed.'*)

let vmss_license_error = addMessage "VMSS_LICENSE_ERROR" 3L

(*'This operation is not allowed under your license.  Please contact your support representative'*)

let vmss_xapi_logon_failure = addMessage "VMSS_XAPI_LOGON_FAILURE" 3L

(*'Could not login to API session.'*)

let vmss_snapshot_missed_event = addMessage "VMSS_SNAPSHOT_MISSED_EVENT" 3L

(*'A scheduled snapshot event was missed due to another on-going scheduled snapshot run. This is unexpected behaviour, please re-configure your schedule snapshot',*)
...
let vdi_cbt_snapshot_failed = addMessage "VDI_CBT_SNAPSHOT_FAILED" 3L

(*'Failed to snapshot the cbt enabled VDI'*)

let vdi_cbt_resize_failed = addMessage "VDI_CBT_RESIZE_FAILED" 3L

(*'Resizing of the cbt enabled VDI failed'*)

let bond_status_changed = addMessage "BOND_STATUS_CHANGED" 3L

(* A link in a bond went down or came back up *)
(* Previously missing from table *)
...
let tls_verification_emergency_disabled = addMessage "TLS_VERIFICATION_EMERGENCY_DISABLED" 3L
