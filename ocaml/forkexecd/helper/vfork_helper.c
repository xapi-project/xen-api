        ...
        case FD_OP_DUP:
            if (op->fd_from == op->fd_to) {
                ...
                if (dup2(op->fd_from, op->fd_to) < 0)
                    err = errno;
                // Track last file descriptor.
                // File descriptors are usually duplicated in order to be
                // replaced later.
...
clear_cgroup(void)
{
    ...
    if (last_error_operation && getuid() == 0)
...
init_syslog(const char *key, bool redirect_stderr_to_stdout)
{
    ...
    wait(&status);
    if (!WIFEXITED(status))
        error(EPIPE, "syslogger killed by signal");
