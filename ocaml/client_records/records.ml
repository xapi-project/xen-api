		...
		fields =
			[
				...
				make_field ~name:"type"
					...
					();
				make_field ~name:"retention-value"
					~get:(fun () -> string_of_int (Int64.to_int (x ()).API.vMSS_retention_value))
					~set:(fun x -> Client.VMSS.set_retention_value rpc session_id vmss (safe_i64_of_string "retention-value" x))
					();
...
let vm_record rpc session_id vm =
	...
	let xgm () = lzy_get guest_metrics in
	{
	  ...
	  fields = [
			...
			make_field ~name:"schedule-snapshot"
				~get:(fun () -> get_uuid_from_ref (x ()).API.vM_schedule_snapshot)
				~set:(fun x -> if x="" then Client.VM.set_schedule_snapshot rpc session_id vm Ref.null else Client.VM.set_schedule_snapshot rpc session_id vm (Client.VMSS.get_by_uuid rpc session_id x)) ();
			make_field ~name:"is-snapshot-from-vmss"
				~get:(fun () -> string_of_bool (x ()).API.vM_is_snapshot_from_vmss) ();
