...
let vm_record rpc session_id vm =
	...
	let xgm () = lzy_get guest_metrics in
	{
	  ...
	  fields = [
			...
			make_field ~name:"scheduled-snapshot"
				~get:(fun () -> get_uuid_from_ref (x ()).API.vM_scheduled_snapshot)
				~set:(fun x -> if x="" then Client.VM.set_scheduled_snapshot rpc session_id vm Ref.null else Client.VM.set_scheduled_snapshot rpc session_id vm (Client.VMSS.get_by_uuid rpc session_id x)) ();
