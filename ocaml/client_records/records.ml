...
let pool_record rpc session_id pool =
	...
	{ setref=(fun r -> _ref := r; record := empty_record );
      ...
      make_field ~name:"wlb-verify-cert" ~get:(fun () -> string_of_bool (x ()).API.pool_wlb_verify_cert) ~set:(fun x -> Client.Pool.set_wlb_verify_cert rpc session_id pool (bool_of_string x)) ();
			...
			make_field ~name:"live-patching-enabled"  ~get:(fun () -> string_of_bool (x ()).API.pool_live_patching_enabled) ~set:(fun s -> Client.Pool.set_live_patching_enabled rpc session_id pool (safe_bool_of_string "live-patching-enabled" s)) ();
		]}
