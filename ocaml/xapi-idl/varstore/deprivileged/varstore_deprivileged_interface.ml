...
module RPC_API (R : RPC) = struct
  ...
  (* A restricted interface to VM's NVRAM, varstored is sandboxed and has no
     direct access to XAPI *)
  let description =
  ...
  let session_login =
    declare "session.login_with_password"
      ...
      @-> string_p
      @-> string_p
      @-> returning string_p err
      )

  let session_logout =
    declare "session.logout"
      ["Dummy, for wire compatibility with XAPI"]
      (string_p @-> returning unit_p err)

  let get_by_uuid =
    declare "VM.get_by_uuid"
      ["Dummy, for wire compatibility with XAPI"]
      (string_p @-> string_p @-> returning string_p err)

  let get_NVRAM =
    declare "VM.get_NVRAM"
