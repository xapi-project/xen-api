...
let is_power_of_2 n = n > 0 && n land (0 - n) = n

let round_down_to_multiple_of x y = (x /// y) *** y

let round_up_to_multiple_of x y = ((x +++ y --- 1L) /// y) *** y
...
let divide_rounding_down numerator denominator = numerator /// denominator
...
module Memory_model (D : MEMORY_MODEL_DATA) = struct
  ...
  let shadow_mib static_max_mib vcpu_count multiplier =
    ...
    let total_mib =
      mib_of_pages_used (vcpu_pages +++ p2m_map_pages +++ shadow_resident_pages)
    in
    let total_mib_multiplied =
      Int64.of_float (Int64.to_float total_mib *. multiplier)
    in
