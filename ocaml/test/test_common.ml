...
let make_pool ~__context ~master ?(name_label="") ?(name_description="")
		...
		?(ha_cluster_stack="xhad") ?(guest_agent_config=[]) ?(cpu_info=[]) ?(policy_no_vendor_device=false) () =
	...
	Db.Pool.create ~__context ~ref:pool_ref
		...
		~current_operations ~allowed_operations
		~restrictions ~other_config ~ha_cluster_stack ~guest_agent_config ~cpu_info ~policy_no_vendor_device;
	pool_ref
