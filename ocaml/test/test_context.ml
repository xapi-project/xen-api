...
let test_get_client_ip () =
  ...
  |> List.iter (fun (ip, exp) ->
         let client_ip =
           proxy_string ip |> make_rq |> Context._client_of_rq
         in
         match client_ip with
         | None -> failwith "client_ip was None"
         | Some x ->  assert_equal exp (Ipaddr.to_string x))

let test =
  "test_context" >:::
  [
    "test_get_client_ip" >:: test_get_client_ip;
  ]
