...
let start rpc session_id sr_info () =
  ...
  write_to_vdi ~rpc ~session_id ~vdi:original
    (fun fd ->
  ...
  (* Snapshot the disk *)
  let snapshot =
    Client.VDI.snapshot ~rpc ~session_id ~vdi:original ~driver_params:[] in
  ...
  let snapshot_backup =
    Client.VDI.copy ~rpc ~session_id ~vdi:snapshot ~sr
  ...
  let delta_backup =
    Client.VDI.copy ~rpc ~session_id ~vdi:original ~sr
  ...
  List.iter (wait_for_no_vbds_then_destroy ~rpc ~session_id) [ original; snapshot; snapshot_backup; delta_backup ]

let tests () =
  let open Qt_filter in
  (* XXX VDI.copy from a base VDI is currently not implemented for GFS2, remove this filter when it is done *)
  [ ["Check VDI.copy delta handling", `Quick, start] |> conn |> sr SR.(all |> allowed_operations [`vdi_create; `vdi_destroy] |> not_iso |> not_type "gfs2" )]
  |> List.concat
