...
let tests () =
  ...
  [ ["sr_scan_test", `Slow, sr_scan_test ] |> conn |> sr SR.(all |> allowed_operations [`scan])
  ; ["sr_update_test", `Slow, sr_update_test] |> conn |> sr SR.(all |> has_capabilities [Sr_capabilities.sr_update])
  ; ["sr_probe_test", `Slow, sr_probe_test] |> conn |> sr SR.(all |> has_capabilities [Sr_capabilities.sr_probe] |> can_unplug)
  ; ["sr_set_name_test", `Quick, sr_set_name_test] |> conn |> sr SR.(all |> allowed_operations [`update])
  ]
  |> List.concat
