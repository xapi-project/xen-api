...
let event_next_unblocking_test () =
  ...
  let session_id = QC.init_session !Quicktest_args.username !Quicktest_args.password in
  let () = Client.Client.Event.register !QC.rpc session_id [] in (* no events *)
  let m = Mutex.create () in
  let unblocked = ref false in
  let (_: Thread.t) = Thread.create
      (fun () ->
         begin
           try ignore(Client.Client.Event.next !QC.rpc session_id)
           with e ->
  ...
  Client.Client.Session.logout !QC.rpc session_id;
...
let event_next_test session_id =
  let () = Client.Client.Event.register !QC.rpc session_id [ "pool" ] in
  let m = Mutex.create () in
  let finished = ref false in
  let pool = Client.Client.Pool.get_all !QC.rpc session_id |> List.hd in
  let key = "event_next_test" in
  begin try Client.Client.Pool.remove_from_other_config !QC.rpc session_id pool key with _ -> () end;
  let (_: Thread.t) = Thread.create
      (fun () ->
         while not (Mutex.execute m (fun () -> !finished)) do
           ignore (Client.Client.Event.next !QC.rpc session_id);
           let oc = Client.Client.Pool.get_other_config !QC.rpc session_id pool in
  ...
  Thread.delay 1.;
  Client.Client.Pool.add_to_other_config !QC.rpc session_id pool key "1";
  Thread.delay 1.;
  Alcotest.(check bool) "failed to see pool.other_config change" true (Mutex.execute m (fun () -> !finished))

let wait_for_pool_key session_id key =
  let token = ref "" in
  let finished = ref false in
  let pool = Client.Client.Pool.get_all !QC.rpc session_id |> List.hd in
  while not !finished do
    let events = Client.Client.Event.from !QC.rpc session_id [ "pool" ] (!token) 10. |> Event_types.event_from_of_rpc in
    token := events.token;
    let oc = Client.Client.Pool.get_other_config !QC.rpc session_id pool in
...
let event_from_test session_id =
  ...
  let (_: Thread.t) = Thread.create
      (fun () ->
         wait_for_pool_key session_id key;
...
let event_from_parallel_test session_id =
  ...
  let (i_should_succeed: Thread.t) = Thread.create
      (fun () ->
         try
           let _ = Client.Client.Event.from !QC.rpc session_id [] "" 10. in
           () (* good *)
         with e ->
...
let object_level_event_test session_id =
  ...
  let vms = Client.Client.VM.get_all !QC.rpc session_id in
  ...
  begin try Client.Client.VM.remove_from_other_config !QC.rpc session_id vm_a key with _ -> () end;
  begin try Client.Client.VM.remove_from_other_config !QC.rpc session_id vm_b key with _ -> () end;

  let (_: Thread.t) = Thread.create
      (fun () ->
         let token = ref "" in
         while not (Mutex.execute m (fun () -> !finished)) do
           let events = Client.Client.Event.from !QC.rpc session_id [ Printf.sprintf "vm/%s" (Ref.string_of vm_a) ] (!token) 10. |> Event_types.event_from_of_rpc in
           List.iter
             (fun event ->
           ...
           token := events.token;
           let oc = Client.Client.VM.get_other_config !QC.rpc session_id vm_a in
           if List.mem_assoc key oc && (List.assoc key oc) = "1"
           then Mutex.execute m (fun () ->
  ...
  Thread.delay 1.;
  Client.Client.VM.add_to_other_config !QC.rpc session_id vm_b key "1";
  Thread.delay 1.;
  Client.Client.VM.remove_from_other_config !QC.rpc session_id vm_b key;
  Client.Client.VM.add_to_other_config !QC.rpc session_id vm_a key "1";
  Thread.delay 1.;
  Mutex.execute m
    (fun () ->
...
let event_message_test session_id =
  ...
  let events = Client.Client.Event.from !QC.rpc session_id [ "message" ] "" 1.0 |> Event_types.event_from_of_rpc in
  let token = events.token in
  let pool = List.hd (Client.Client.Pool.get_all !QC.rpc session_id) in
  let obj_uuid = Client.Client.Pool.get_uuid !QC.rpc session_id pool in
  print_endline "Creating message";
  let cls = `Pool in
  let message = Client.Client.Message.create ~rpc:!QC.rpc ~session_id ~name:"test" ~priority:1L ~cls
      ~obj_uuid ~body:"Hello" in
  print_endline (Printf.sprintf "Created message: %s" (Ref.string_of message));
  let events = Client.Client.Event.from !QC.rpc session_id [ "message" ] token 1.0 |> Event_types.event_from_of_rpc
  in
  ...
  Client.Client.Message.destroy !QC.rpc session_id message;
  let events = Client.Client.Event.from !QC.rpc session_id [ "message" ] token 1.0 |> Event_types.event_from_of_rpc in
  ...
  let message1 = Client.Client.Message.create ~rpc:!QC.rpc ~session_id ~name:"test" ~priority:1L ~cls
      ~obj_uuid ~body:"Hello" in
  let message2 = Client.Client.Message.create ~rpc:!QC.rpc ~session_id ~name:"test" ~priority:1L ~cls
      ~obj_uuid ~body:"Hello" in
  let events = Client.Client.Event.from !QC.rpc session_id [ "message" ] token 1.0 |> Event_types.event_from_of_rpc in
  let token = events.token in
  let message3 = Client.Client.Message.create ~rpc:!QC.rpc ~session_id ~name:"test" ~priority:1L ~cls
      ~obj_uuid ~body:"Hello" in
  let events2 = Client.Client.Event.from !QC.rpc session_id [ "message" ] token 1.0 |> Event_types.event_from_of_rpc in
  ...
  print_endline (Printf.sprintf "Finding messages for object: %s" (Client.Client.Pool.get_uuid !QC.rpc session_id pool));
  let messages = Client.Client.Message.get ~rpc:!QC.rpc ~session_id  ~cls ~obj_uuid ~since:(Xapi_stdext_date.Date.never) in
...
let event_inject_test session_id =
  let events = Client.Client.Event.from !QC.rpc session_id [ "pool" ] "" 1.0 |> Event_types.event_from_of_rpc in
  let token = events.token in
  let pool = List.hd (Client.Client.Pool.get_all !QC.rpc session_id) in
  let starttime = Unix.gettimeofday () in
  let (x: Thread.t) = Thread.create
      (fun () ->
         let _ = Client.Client.Event.from !QC.rpc session_id [ "pool" ] token 5.0 in
         ()
      ) () in
  ignore(Client.Client.Event.inject ~rpc:!QC.rpc ~session_id ~_class:"pool" ~_ref:(Ref.string_of pool));
  Thread.join x;
...
let event_from_number_test session_id =
  let events = Client.Client.Event.from !QC.rpc session_id [ "vm" ] "" 10. |> Event_types.event_from_of_rpc in
...
let tests session_id =
  [ "event_next_unblocking_test", `Slow, event_next_unblocking_test
  ; "event_next_test", `Slow, (fun () -> event_next_test session_id)
  ; "event_from_test", `Slow, (fun () -> event_from_test session_id)
  ; "event_from_parallel_test", `Slow, (fun () -> event_from_parallel_test session_id)
  ; "object_level_event_test", `Slow, (fun () -> object_level_event_test session_id)
  ; "event_message_test", `Slow, (fun () -> event_message_test session_id)
  ; "event_inject_test", `Slow, (fun () -> event_inject_test session_id)
  ; "event_from_number_test", `Slow, (fun () -> event_from_number_test session_id)
  ]
