...
let packages_iso_test session_id =
  ...
  let sr = Client.Client.SR.introduce ~rpc:!rpc ~session_id ~uuid:(Uuid.string_of_uuid (Uuid.make_uuid ()))
  ...
  Xapi_stdext_pervasives.Pervasiveext.finally
    (fun () ->
       let device_config = [ "location", !iso_path;
                             "legacy_mode", "true" ] in
       let pbd = Client.Client.PBD.create ~rpc:!rpc ~session_id ~sR:sr ~host ~device_config ~other_config:[] in
       Xapi_stdext_pervasives.Pervasiveext.finally
         (fun () ->
            ...
            List.iter (fun vdi ->
                ...
                if not(vdir.API.vDI_read_only) then begin
                  Alcotest.fail (Printf.sprintf "ISO VDI has read_only set to false (%s)" vdir.API.vDI_name_label);
                end;
                print_endline (Printf.sprintf "ISO VDI %s looks ok" vdir.API.vDI_name_label);
              ) vdis;
         ) (fun () ->
             Client.Client.PBD.unplug !rpc session_id pbd;
             Client.Client.PBD.destroy !rpc session_id pbd)
    ) (fun () -> Client.Client.SR.forget ~rpc:!rpc ~session_id ~sr)

let tests session_id =
  [ "packages_iso_test", `Slow, (fun () -> packages_iso_test session_id)
  ]
