...
let vdi_create_destroy session_id sr_info =
  ...
  List.iter
    (fun virtual_size ->
       let uuid = Storage_test.VDI.with_new session_id ~virtual_size sr (fun vdi ->
...
let vdi_generate_config_test session_id sr_info =
...
let vdi_update_test session_id sr_info =
  Storage_test.VDI.with_any session_id sr_info (fun vdi ->
...
let vdi_resize_test session_id sr_info =
  Storage_test.VDI.with_new session_id sr_info.Storage_test.sr (fun vdi ->
...
let vdi_db_forget session_id sr_info =
  Storage_test.VDI.with_any session_id sr_info (fun vdi ->
      try
...
let vdi_bad_introduce session_id sr_info =
  Storage_test.VDI.with_any session_id sr_info (fun vdi ->
      ...
      begin
        try
          ...
          let (_: API.ref_VDI) = Client.Client.VDI.introduce ~rpc:!rpc ~session_id
...
let test_vdi_snapshot session_id sr_info =
  Storage_test.VDI.with_any session_id sr_info
    (check_vdi_snapshot session_id)

let test_vdi_clone session_id sr_info =
  Storage_test.VDI.with_any session_id sr_info (fun vdi ->
      let vdi' = Client.Client.VDI.clone ~rpc:!rpc ~session_id ~vdi ~driver_params:[] in
...
let vbd_create_helper ~session_id ~vM ~vDI ?(userdevice="autodetect") () : API.ref_VBD =
  Client.Client.VBD.create ~rpc:!rpc ~session_id ~vM ~vDI ~userdevice ~bootable:false ~mode:`RW
...
let vdi_snapshot_in_pool session_id sr_info =
  Storage_test.VDI.with_any session_id sr_info (fun vdi ->
      let hosts = Client.Client.Host.get_all !rpc session_id in
      let do_test () =
        check_vdi_snapshot session_id vdi
      in
...
let vdi_create_destroy_plug_checksize session_id sr_info =
  ...
  Storage_test.VDI.with_new session_id ~virtual_size:small_size sr (fun small_vdi ->
      print_endline (Printf.sprintf "Creating VDI with requested size: %Ld" large_size);
      Storage_test.VDI.with_new session_id ~virtual_size:large_size sr (fun large_vdi ->
          plug_in_check_size session_id host small_vdi |> ignore;
          plug_in_check_size session_id host large_vdi |> ignore
        )
    )

(** Make a VDI, find a host to put it on, create a VBD to dom0 on that host,
    Attach, Unattach, destroy VBD, destroy VDI *)
let vdi_general_test session_id sr_info =
  ...
  Storage_test.VDI.with_new session_id sr (fun newvdi ->
      ...
      let vbd =
        Client.Client.VBD.create ~rpc:!rpc ~session_id
      ...
      Xapi_stdext_pervasives.Pervasiveext.finally
        (fun () ->
           ...
           Storage_test.VDI.with_destroyed session_id newvdi2 (fun () ->
...
let tests session_id =
  ...
  ; "vdi_generate_config_test", `Slow, vdi_generate_config_test, F.has_capabilities [Sr_capabilities.vdi_generate_config]
  ; "vdi_update_test", `Slow, vdi_update_test, F.(has_capabilities [Sr_capabilities.vdi_update] ||> with_any_vdi)
  ; "vdi_resize_test", `Slow, vdi_resize_test, F.has_capabilities [Sr_capabilities.vdi_resize]
  ; "vdi_db_forget", `Slow, vdi_db_forget, F.with_any_vdi
  ; "vdi_bad_introduce", `Slow, vdi_bad_introduce, F.(has_capabilities [Sr_capabilities.vdi_introduce] ||> with_any_vdi)
  ; "test_vdi_snapshot", `Slow, test_vdi_snapshot, F.has_capabilities [Sr_capabilities.vdi_snapshot]
  ; "test_vdi_clone", `Slow, test_vdi_clone, F.(allowed_operations [`scan] ||> has_capabilities [Sr_capabilities.vdi_clone])
