...
let time_of_rfc3339 date =
  ...
  | Ok (time, _, _) ->
      time
  | Error _ ->
      raise (Failure ("Date is in the wrong format: " ^ date))
...
let key_chain =
  List.init 3 (fun _ -> `RSA (Mirage_crypto_pk.Rsa.generate ~bits:2048 ()))
...
let all_tests =
  valid_keys_tests
  @ invalid_keys_tests
  @ valid_leaf_cert_tests
  @ invalid_leaf_cert_tests
  @ valid_chain_cert_tests
  @ invalid_chain_cert_tests

let all = List.map load_test all_tests
