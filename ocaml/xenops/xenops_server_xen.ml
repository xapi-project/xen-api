...
module Actions = struct
	...
	let fire_event_on_device domid kind devid domains =
		let d = int_of_string domid in
		let open Xenstore_watch in
		if not(IntMap.mem d domains)
		then debug "Ignoring watch on shutdown domain %d" d
			...
			let update = match kind with
				...
				| x ->
					...
					None in
			Opt.iter (fun x -> Updates.add x updates) update

	let watch_fired xs path domains watches look_for_different_devices =
		match List.filter (fun x -> x <> "") (String.split '/' path) with
			| "local" :: "domain" :: domid :: "backend" :: kind :: frontend :: devid :: _ ->
				...
				fire_event_on_device frontend kind devid domains
