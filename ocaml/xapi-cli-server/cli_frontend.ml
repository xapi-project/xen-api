...
let rec cmdtable_data : (string * cmd_spec) list =
  ...
  ; ( "rate-limit-set-burst-size"
    , {
        reqd= ["uuid"; "value"]
      ; optn= []
      ; help= "Set the burst size of a rate limiter."
      ; implementation= No_fd Cli_operations.Rate_limit.set_burst_size
      ; flags= []
      }
    )
  ; ( "rate-limit-set-fill-rate"
    , {
        reqd= ["uuid"; "value"]
      ; optn= []
      ; help= "Set the fill rate of a rate limiter."
      ; implementation= No_fd Cli_operations.Rate_limit.set_fill_rate
      ; flags= []
      }
    )
  ]
