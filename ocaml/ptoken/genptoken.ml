...
let options =
  ref {force= false; tgtfile= Filename.concat (Sys.getcwd ()) "ptoken"}
...
let opt_force =
  ( "-f"
  , Arg.Unit set_force
  , "force generation of pool token, overwriting any existing one" )

let opt_target =
  ("-o", Arg.String set_target, "name of file to write to [ptoken]")

let opts = [opt_force; opt_target]

let _ =
  Arg.parse opts (fun _ -> ()) "Generate a pool token" ;
  if Sys.file_exists !options.tgtfile then
    if !options.force then
      Sys.remove !options.tgtfile
    else (
