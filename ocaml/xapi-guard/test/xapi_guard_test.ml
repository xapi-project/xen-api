...
let xapi_rpc call =
  ...
  let ret_ok contents = Lwt.return Rpc.{success = true; contents = Rpc.String contents; is_notification=false} in
  ...
  let expect_session_id session_id_rpc =
    ...
    Alcotest.(check' ref) ~expected:expected_session_id ~actual ~msg:"session id"
  in
  ...
  | "VM.get_VTPMs", [session_id_rpc; vm_rpc] ->
    expect_session_id session_id_rpc;
    expect_vm vm_rpc;
    Lwt.return Rpc.{success=true; contents=Rpc.Enum [Rpc.String (Ref.string_of vtpm)];
                    is_notification=false}
  | "VTPM.get_profile", [session_id_rpc; vtpm_rpc] ->
    expect_session_id session_id_rpc;
    expect_vtpm vtpm_rpc;
    Lwt.return Rpc.{success=true; contents=Rpc.Dict [];
                    is_notification=false}
  ...
  | "VM.get_NVRAM", [session_id_rpc; vm_rpc] ->
    expect_session_id session_id_rpc;
    expect_vm vm_rpc;
    Lwt.return Rpc.{success=true; contents=API.rpc_of_string_to_string_map !nvram_contents;is_notification=false}
...
let with_rpc f switch () =
  ...
  Lwt.finalize (fun () ->
      ...
      let* session_id = Session.login_with_password ~rpc ~uname:"root" ~pwd:"" ~version:"0.0" ~originator:"test"
      in
...
let uefi_tests = [
  test_case "NVRAM" `Quick @@ with_rpc test_change_nvram
]

let () =
  Debug.log_to_stdout ();
  Lwt_main.run @@ Alcotest_lwt.run "xapi_guard_test"
    [ "VTPM", vtpm_tests
    ; "UEFI", uefi_tests
    ]
