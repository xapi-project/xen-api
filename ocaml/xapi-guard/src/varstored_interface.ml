...
let make_server_rpcfn path vm_uuid =
  ...
  let with_vtpm f =
    ret
      (with_xapi @@ VM.get_VTPMs ~self:vm >>= function
       | [] ->
  ...
  *)
  let get_profile _ _ =
    with_vtpm @@ fun ~self -> with_xapi @@ VTPM.get_profile ~self
  in
  let get_contents _ _ =
    with_vtpm @@ fun ~self -> with_xapi @@ VTPM.get_contents ~self
  in
  let set_contents _ _ contents =
    with_vtpm @@ fun ~self -> with_xapi @@ VTPM.set_contents ~self ~contents
  in
