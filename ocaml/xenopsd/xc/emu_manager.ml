...
exception
  Domain_builder_error of
    string (* function name *) * int (* error code *) * string
...
let connect path (args : string list) (fds : (string * Unix.file_descr) list) :
  ...
  let slave_to_server_w_uuid = Uuidx.(to_string (make ())) in
  let server_to_slave_r_uuid = Uuidx.(to_string (make ())) in
...
(* request the caller to prepare for the next step *)

let string_of_message = function
  | Stdout x ->
      "stdout:" ^ String.escaped x
  | Stderr x ->
      "stderr:" ^ String.escaped x
  | Error x ->
      "error:" ^ String.escaped x
  | Suspend ->
      "suspend:"
  | Info x ->
      "info:" ^ String.escaped x
  | Result x ->
...
let message_of_string x =
  ...
  and suffix = String.sub x (i + 1) (String.length x - i - 1) in
  match prefix with
  | "stdout" ->
      Stdout suffix
  | "stderr" ->
...
let rec non_debug_receive ?(debug_callback = fun s -> debug "%s" s) cnx =
  match receive cnx with
  | Stdout x ->
      debug_callback x ;
      non_debug_receive ~debug_callback cnx
  | Stderr x ->
      debug_callback x ;
      non_debug_receive ~debug_callback cnx
  | Info x ->
