...
let numa_placement domid ~vcpus ~memory =
  ...
  let hint =
    with_lock numa_mutex (fun () ->
        ...
        in
        numa_resources := Some nodes ;
        Softaffinity.plan ~vm host nodes
    )
  in
  let xcext = get_handle () in
  let memory_planning =
    ...
    | Some (cpu_affinity, mem_plan) ->
        let cpua = CPUSet.to_mask cpu_affinity in
        for i = 0 to vcpus - 1 do
          Xenctrlext.vcpu_setaffinity_soft xcext domid i cpua
        done ;
        mem_plan
  in
  (* Xen is currently only allows a single node when using memory claims, or
     none. *)
  let numa_node =
