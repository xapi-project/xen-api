...
let get_dbg task =
  ...
  | None -> Xenops_task.get_dbg task
  | Some tracing -> (Xenops_task.get_dbg task) ^ "\n" ^ tracing

let epoch_begin task sr vdi domid persistent =
  transform_exception
    (fun () ->
      Client.VDI.epoch_begin (get_dbg task) sr vdi domid persistent
    )
    ()
...
let attach_and_activate ~task ~_vm ~vmdomid ~dp ~sr ~vdi ~read_write =
  ...
  let result =
    Xenops_task.with_subtask task
      ...
      (transform_exception (fun () ->
           Client.VDI.attach3 dbg dp sr vdi vmdomid
             read_write
       )
      )
  in
  Xenops_task.with_subtask task
    ...
    (transform_exception (fun () ->
         Client.VDI.activate3 dbg dp sr vdi vmdomid
     )
    ) ;
  result
...
let deactivate task dp sr vdi vmdomid =
  ...
  Xenops_task.with_subtask task
    ...
    (transform_exception (fun () ->
         Client.VDI.deactivate dbg dp sr vdi vmdomid
     )
    )
