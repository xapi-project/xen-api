...
let reason__migrate_source = "source"

(* passed to pre-migrate hook on source host *)

let reason__migrate_dest = "destination"

(* passed to post-migrate hook on destination host *)
...
let vm ~script ~reason ~id =
  let script_name =
    match script with
    | VM_pre_destroy ->
        scriptname__vm_pre_destroy
    | VM_pre_migrate ->
        scriptname__vm_pre_migrate
    | VM_post_migrate ->
        scriptname__vm_post_migrate
    | VM_pre_suspend ->
        scriptname__vm_pre_suspend
    | VM_pre_start ->
        scriptname__vm_pre_start
    | VM_pre_reboot ->
        scriptname__vm_pre_reboot
    | VM_pre_resume ->
        scriptname__vm_pre_resume
    | VM_post_resume ->
        scriptname__vm_post_resume
    | VM_post_destroy ->
        scriptname__vm_post_destroy
  in
