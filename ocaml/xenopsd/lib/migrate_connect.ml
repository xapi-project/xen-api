...
(* TLS transport for the VM-migration data connections in [Xenops_server].
   ...
   uses https, this module spawns the external [xenopsd-tls-helper] binary to
...
let helper_path = ref "/usr/libexec/xenopsd/xenopsd-tls-helper"
...
let helper_args ~host ~port ~fd_uuid ~verify_cert =
  ...
  let sni_name =
    ...
    | Some cfg ->
        Option.value ~default:"pool" cfg.Stunnel.sni
    | None ->
        "pool"
  in
  let verify_args =
...
let connect_via_ktls_helper ~host ~port ~verify_cert =
  ...
  (* [sock_xenopsd] is closed on every path by [finally]; the helper is always
     reaped via [waitpid] (killed first if it times out) so it cannot leak. *)
  finally
    (fun () ->
      let received_fd =
        try
          ...
          then (
            ...
            failwith
              (Printf.sprintf "helper did not respond within %.0fs"
                 !helper_timeout
              )
          ) ;
        ...
        with e ->
          ...
          raise e
      in
      (* Reap the helper and check its exit status; the fd we would return is
         closed if the helper did not exit cleanly. *)
      ( match snd (Forkhelpers.waitpid pid) with
      | Unix.WEXITED 0 ->
          ()
      | status ->
          close_ignore received_fd ;
          let reason =
            match status with
            | Unix.WEXITED n ->
          ...
          in
          failwith (Printf.sprintf "xenopsd-tls-helper %s" reason)
      ) ;
      D.debug "received kTLS fd from helper for %s:%d" host port ;
      received_fd
    )
    (fun () -> close_ignore sock_xenopsd)
  ...
  if not (is_https && ktls_enabled ()) then
    ...
    | Some host -> (
        ...
        | None ->
            fallback ()
        | Some fd ->
            finally (fun () -> f fd) (fun () -> close_ignore fd)
      )
