...
module QueryImpl (M : META) = struct
  ...
  let query_impl dbg =
    let th =
      ...
      M.version := Some required_api_version ;
...
module DPImpl (M : META) = struct
  ...
  let dp_attach_info_impl dbg sr vdi dp vm =
    VDI.vdi_attach3_impl dbg dp sr vdi vm ()
end
