...
module Deferred = struct
  let all_unit (lst : unit Lwt.t list) : unit Lwt.t =
    let (_ : unit list Lwt.t) = Lwt.all lst in
    Lwt.return ()
...
module Sys = struct
  ...
  let read_file_contents path =
    let open Lwt_io in
    with_file ~mode:input ~flags:[O_RDONLY] ~perm:0o000 path @@ fun chan ->
    read chan

  let save ~contents path =
    let open Lwt_io in
    with_file ~mode:output path @@ fun chan -> write chan contents
  ...
  let mkdir ?(perms = 0o777) path = Lwt_unix.mkdir path perms
end
...
module Process : sig
  module Output : sig
    ...
    val exit_or_signal_of_unix :
      Unix.process_status -> (unit, exit_or_signal) Result.t
  end

  type t = Lwt_process.process_full

  val create : prog:string -> args:string list -> t

  val collect_output_and_wait : t -> Output.t Lwt.t
end = struct
  type t = Lwt_process.process_full
  ...
  let collect_output_and_wait p =
    let read_all chan = Lwt_io.read chan in
    Lwt.both (read_all p#stdout) (read_all p#stderr) >>= fun (stdout, stderr) ->
    p#close >>= fun status ->
...
let api_max = List.fold_left string_max "" supported_api_versions
...
module Compat (V : sig
  ...
  let with_pvs_version f rpc =
    ...
    | Some v when String.equal v pvs_version ->
        f rpc
    | _ ->
        rpc
  ...
  let compat_uri device_config =
    match !V.version with
    | Some version when String.equal version pvs_version -> (
      match List.assoc_opt "uri" device_config with
      | None ->
          fail (missing_uri ())
      | Some uri ->
...
let check_plugin_version_compatible query_result =
  ...
  ( if not (String.equal required_api_version api_max) then
      warn (fun m ->
          m
...
let fork_exec_rpc :
 ...
 fun ~script_dir ?missing ?(compat_in = id) ?(compat_out = id) ->
  let invoke_script call script_name :
    ...
    let p = Process.create ~prog:script_name ~args:["--json"] in
    Lwt.catch
      (fun () ->
        (* Send the request as json on stdin *)
        let w = p#stdin in
        Lwt_io.write w (Jsonrpc.to_string args) >>= fun () -> Lwt_io.close w
      )
      (fun _ -> p#close >>= fun _ -> Lwt.return_unit)
    >>= fun () ->
...
module Attached_SRs = struct
  ...
  let add smapiv2 plugin uids =
    ...
    | Some path ->
        ...
        Sys.mkdir dir >>= fun () -> Sys.save path ~contents
    )
    >>= fun () -> return ()
