...
let lvcreate vg_name lv_name bytes =
  let size_mb =
    Int64.to_string (Int64.div (Int64.add 1048575L bytes) 1048576L)
  in
  ignore_string
    (Common.run "lvcreate" ["-L"; size_mb; "-n"; lv_name; vg_name; "-Z"; "n"])

let lvremove vg_name lv_name =
  ignore_string
    (Common.run "lvremove" ["-f"; Printf.sprintf "%s/%s" vg_name lv_name])

type lv = {name: string; size: int64}
...
let to_lines output =
  List.filter (fun x -> x <> "") (Re_str.split_delim newline output)

let lvs vg_name =
  Common.run "lvs"
    ["-o"; "lv_name,lv_size"; "--units"; "b"; "--noheadings"; vg_name]
  |> to_lines
  |> List.map (fun line ->
         match
         ...
         | [x; y] ->
             let size =
               Int64.of_string (String.sub y 0 (String.length y - 1))
             in
             {name= x; size}
         | _ ->
...
let lvresize vg_name lv_name size =
  ...
  if cur_mb <> size_mb_rounded then (
    debug
      "lvresize: current size is %Ld MiB <> requested size %Ld MiB (rounded \
       from %Ld); resizing"
