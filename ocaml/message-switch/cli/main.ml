...
let diagnostics common_opts =
  ...
  let payload common_opts payload =
    ...
    let trim txt =
      ...
      if common_opts.Common.verbose || len < max_len then
        txt
      else
        String.sub txt 0 max_len
    in
    try
    ...
    with _ ->
      ...
      Printf.printf "      - %s\n" (trim payload)
  in
  let classify (_, queue) =
    match queue.Diagnostics.next_transfer_expected with
    | None ->
        `Not_started
    | Some t ->
    ...
    List.iter
      (fun (id, entry) ->
        ...
        | Message.Request name ->
            if List.mem_assoc name d.Diagnostics.transient_queues then
  ...
  let expected =
    List.concat
      (List.map expected_transient_queues d.Diagnostics.permanent_queues)
  in
  let to_show =
    List.filter
      (fun (name, queue) ->
...
let destroy common_opts name =
  match name with
  | None ->
      `Error (true, "Please supply a queue name")
  | Some name ->
      Client.connect ~switch:common_opts.Common.path () >>|= fun t ->
      Client.destroy ~t ~queue:name () >>|= fun _ -> `Ok ()
...
let summarise_payload m =
  ...
  with _ -> (
    ...
    with _ ->
      let limit = 10 in
      if String.length m > limit then
        String.sub m 0 limit
      else
        m
  )
...
let mscgen common_opts =
  Client.connect ~switch:common_opts.Common.path () >>|= fun t ->
  Client.trace ~t ~from:0L () >>|= fun trace ->
  let quote x = "\"" ^ x ^ "\"" in
  ...
  let queues =
    List.fold_left
      (fun acc (_, event) -> StringSet.add event.Event.queue acc)
      StringSet.empty trace.Out.events
  in
  let inputs =
    List.fold_left
      (fun acc (_, event) ->
        match event.Event.input with
        | None ->
            acc
        | Some x ->
            StringSet.add x acc)
...
let tail common_opts follow =
  ...
  while not !finished do
    ...
    | Error _ ->
        failwith "Trace failed"
    | Ok trace -> (
        let endpoint = function None -> "-" | Some x -> x in
          ...
          List.map
            (fun (_id, event) ->
              ...
              @ ( match m with
                | Event.Message (_, {Message.kind= Message.Response _; _}) ->
                    [
                      endpoint event.Event.output
                    ; "<-"
                    ; event.Event.queue
                    ; "<-"
                    ; endpoint event.Event.input
                    ]
                | Event.Message (_, {Message.kind= Message.Request _; _}) ->
                    [
                      endpoint event.Event.input
        ...
        finished := not follow ;
        from :=
          match trace.Out.events with
          | [] ->
              !from
          | (id, _) :: ms ->
              Int64.add 1L (List.fold_left max id (List.map fst ms))
      )
  done ;
  `Ok ()
...
let list_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Print a list of all queues registered with the message switch. By \
         default this only shows queues which have had a process start \
         listening. If you want to see all queues, see the 'all' flag."
    ]
    @ help
  in
  let prefix =
...
let tail_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Display the most recent trace events captured within the message \
         switch. Similar to the shell command 'tail'"
    ]
    @ help
  in
  let follow =
...
let mscgen_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Display the most recent trace events in mscgen input format, allowing \
         message sequence charts to be rendered"
    ]
    @ help
  in
...
let ack_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Acknowledge processing of a specific message and remove it from any \
         queue."
    ]
    @ help
  in
...
let destroy_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Destroy a whole named queue, including all the messages queued inside."
    ]
    @ help
  in
  let n =
  ...
  try
    while true do
      ...
      | None ->
          ()
      | Some x ->
          if x = line then raise End_of_file
      ) ;
      lines := line :: !lines
    done ;
    ""
...
let serve common_options_t name program =
  ...
  | Some name ->
      let _ =
        Message_switch_unix.Protocol_unix.Server.listen
          ~process:(fun req ->
            match program with
            | None ->
            ...
            | Some program ->
                let stdout, stdin, stderr =
                  Unix.open_process_full program [|program|]
                in
                output_string stdin req ;
                close_out stdin ;
...
let serve_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Listen for remote procedure calls and run the specified program with \
         the body, returning the program's output as the response."
    ]
    @ help
  in
  let qname =
