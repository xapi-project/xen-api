...
module In = struct
  ...
  let of_request body meth path =
    ...
    | "", `GET, "" :: "admin" :: path ->
        Some (Get path)
    | "", `GET, "" :: (("js" | "css" | "images") :: _ as path) ->
        Some (Get path)
    | "", `GET, [""; ""] ->
        Some Diagnostics
    | "", `GET, [""; "login"; token] ->
        Some (Login (Uri.pct_decode token))
    | "", `GET, [""; "persistent"; name] ->
        Some (CreatePersistent (Uri.pct_decode name))
    | "", `GET, [""; "transient"; name] ->
        Some (CreateTransient (Uri.pct_decode name))
    | "", `GET, [""; "destroy"; name] ->
...
type origin =
  | Anonymous of string
      (** An un-named connection, probably a temporary client connection *)
