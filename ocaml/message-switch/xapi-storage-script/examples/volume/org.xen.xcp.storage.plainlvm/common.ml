...
let log fmt =
  Printf.ksprintf
    (fun s -> output_string stderr s ; output_string stderr "\n" ; flush stderr)
    fmt
...
type t = {verbose: bool; debug: bool; test: bool; json: bool}

let make verbose debug test json = {verbose; debug; test; json}
...
let rm_f x =
  try Unix.unlink x ; debug "rm %s" x
  with _ ->
...
let retry_every n f =
  ...
  while not !finished do
    try
      ...
      finished := true
    with e ->
      debug "Caught %s: sleeping %f. before trying again" (Printexc.to_string e)
        n ;
      Thread.delay n
  done
  ...
  try
    ...
    let pid =
      Unix.create_process_env cmd
        ...
        env stdin_readable writable Unix.stderr
    in
    close writable ;
...
let common_options_t =
  ...
  let test =
    ...
    Arg.(value & flag & info ["test"] ~docs ~doc)
  in
  let json =
...
module Make (C : Command) (T : Test) = struct
  let wrap common args =
    let args =
      if common.json then
        read_line () |> Jsonrpc.of_string |> C.In.t_of_rpc
      else
        args
    in
    if common.test then (
...
let vg_of_uri uri =
  ...
  | Some "vg" ->
      ...
      if vg <> "" && vg.[0] = '/' then
        ...
        vg
  | _ ->
      raise (Xapi_storage.V.SR_does_not_exist uri)
