...
let get (logger : logger) =
  ...
  if all <> [] then
    return_lines all
  else (* Block for at least one line *)
    Lwt_stream.nget 1 logger.stream >>= fun all -> return_lines all
...
let log_lwt level key (fmt : (_, _, _, _) format4) =
  let level = string_of_level level in
  Printf.ksprintf
    (fun x -> logger.push x ; Lwt.return ())
    ("[%5s|%s] " ^^ fmt) level key
...
let rec logging_thread () =
  ...
  Lwt_list.iter_s
    (fun x -> Lwt_log.log ~logger:!Lwt_log.default ~level:Lwt_log.Notice x)
    lines
  >>= logging_thread
