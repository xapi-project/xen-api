...
let start_local (module D : Debug.DEBUG) ~reporter ~uid ~neg_shift ~page_count
    ~protocol ~dss_f =
  try
    let path = RRDD.Plugin.get_path uid in
    D.info "Obtained path=%s\n" path ;
    let _ = Unixext.mkdir_safe (Filename.dirname path) 0o644 in
    let id = Rrd_file_writer.{path; shared_page_count= page_count} in
    let _, writer = Rrd_file_writer.create id (choose_protocol protocol) in
