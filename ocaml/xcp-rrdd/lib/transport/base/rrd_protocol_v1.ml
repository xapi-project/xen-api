...
let payload_start = header_bytes + length_bytes + checksum_bytes + 2
...
let ds_of_rpc ((name, rpc) : string * Rpc.t) : Rrd.ds_owner * Ds.ds =
  try
    ...
    in
    let min =
      float_of_string (Rrd_rpc.assoc_opt ~key:"min" ~default:"-infinity" kvs)
    in
    let max =
      float_of_string (Rrd_rpc.assoc_opt ~key:"max" ~default:"infinity" kvs)
    in
    let owner =
