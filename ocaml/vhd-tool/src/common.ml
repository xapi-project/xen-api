...
let size bytes =
  ...
  if div bytes tib > 0L then
    Printf.sprintf "%Ld TiB" (div bytes tib)
  else if div bytes gib > 0L then
    Printf.sprintf "%Ld GiB" (div bytes gib)
  else if div bytes mib > 0L then
    Printf.sprintf "%Ld MiB" (div bytes mib)
  else if div bytes kib > 0L then
    Printf.sprintf "%Ld KiB" (div bytes kib)
  else
    Printf.sprintf "%Ld bytes" bytes
...
let print_table header rows =
  ...
  let print_row row =
    List.iter
      (fun (n, s) -> Printf.printf "%s |" (padto ' ' n s))
      (List.combine widths row) ;
    Printf.printf "\n"
  in
  print_row header ;
  List.iter
    (fun (n, _) -> Printf.printf "%s-|" (padto '-' n ""))
    (List.combine widths header) ;
  Printf.printf "\n" ;
