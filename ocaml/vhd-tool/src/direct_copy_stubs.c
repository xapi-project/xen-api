...
CAMLprim value stub_init(value in_fd, value out_fd)
{
    ...
    cpinfo->buffer = NULL;
    if ( posix_memalign((void **)&cpinfo->buffer, sysconf(_SC_PAGESIZE),
                        XFER_BUFSIZ) )
    {
        free(cpinfo);
        caml_raise_out_of_memory();
...
CAMLprim value stub_direct_copy(value handle, value len)
{
    ...
    cpinfo = (struct direct_copy_handle *)Field(handle, 0);
    if ( !cpinfo )
        caml_failwith("direct_copy: NULL handle");
    ...
    while ( remaining > 0 )
    {
        ...
        bread = read(cpinfo->in_fd, cpinfo->buffer,
                     (remaining < XFER_BUFSIZ) ? remaining : XFER_BUFSIZ);
        /* If we previously hit exactly the end of the input by accident, we're
         * done. */
        if ( bread == 0 )
            break;
        if ( bread < 0 )
        {
            ...
            if ( errno == EAGAIN )
            {
                ...
                {
                    /* If poll() got interrupted, hitting read() (or, later,
                     * write() again one extra time to try again is
                     * insignificant, and avoids another loop */
                    if ( errno == EINTR )
                        continue;
                    rc = READ_POLL_FAILED;
                    goto fail;
        ...
        while ( bwritten < bread )
        {
            ...
            ret = write(cpinfo->out_fd, cpinfo->buffer + bwritten,
                        bread - bwritten);
            if ( ret == 0 )
            {
