let assert_equal msg p1 p2 =
  let network_purpose =
    Alcotest_comparators.from_rpc_of_t API.rpc_of_network_purpose
  in
  Alcotest.(check (slist network_purpose compare)) msg p1 p2
...
let test_add_purpose () =
  with_test (fun network add_purpose _ get_purpose ->
      ...
      assert_equal
        "Should be able to add 'nbd' purpose when none of the networks have a \
         purpose"
        [`nbd]
  ...
  with_test (fun network add_purpose _ get_purpose ->
      ...
      add_purpose ~self:network1 ~value:`nbd ;
      assert_equal
        "Should be able to add 'nbd' purpose when the other network has the \
         same purpose"
        [`nbd]
        (get_purpose ~self:network1)
  ) ;
  with_test (fun network add_purpose _ get_purpose ->
      ...
      assert_equal
        "Should be able to add 'insecure_nbd' purpose when none of the \
         networks have a purpose"
        [`insecure_nbd]
        (get_purpose ~self:network1)
  ) ;
  with_test (fun network add_purpose _ get_purpose ->
      ...
      add_purpose ~self:network1 ~value:`insecure_nbd ;
      assert_equal
        "Should be able to add 'insecure_nbd' purpose when the other network \
  ...
  with_test (fun network add_purpose _ get_purpose ->
      ...
      add_purpose ~self:network1 ~value:`nbd ;
      assert_equal
        "add_purpose should be idempotent and should do nothing when adding an \
         existing purpose"
        [`nbd]
        (get_purpose ~self:network1)
  ) ;
  with_test (fun network add_purpose _ _ ->
      ...
      Alcotest.check_raises
        ...
        Api_errors.(
          Server_error (network_incompatible_purposes, ["insecure_nbd"; "nbd"])
        )
  ...
  with_test (fun network add_purpose _ _ ->
      ...
      let _network2 : _ API.Ref.t = network ~purpose:[`nbd] in
      Alcotest.check_raises
        "Should not be allowed to add 'insecure_nbd' when another network \
         already has the 'nbd' purpose"
        Api_errors.(
          Server_error (network_incompatible_purposes, ["insecure_nbd"; "nbd"])
        )
        (fun () -> add_purpose ~self:network1 ~value:`insecure_nbd)
  ...
  with_test (fun network add_purpose _ _ ->
      ...
      let _network2 : _ API.Ref.t = network ~purpose:[`insecure_nbd] in
      Alcotest.check_raises
        "Should not be allowed to add 'nbd' when another network already has \
         the 'insecure_nbd' purpose"
        Api_errors.(
          Server_error (network_incompatible_purposes, ["nbd"; "insecure_nbd"])
        )
        (fun () -> add_purpose ~self:network1 ~value:`nbd)
...
let test_remove_purpose () =
  with_test (fun network _ remove_purpose get_purpose ->
      ...
      assert_equal
        "remove_purpose should be idempotent and should do nothing when \
         removing an invalid purpose that is not present"
        [`nbd]
        (get_purpose ~self:network1)
  ) ;
  with_test (fun network _ remove_purpose get_purpose ->
      ...
      assert_equal
        "remove_purpose should be idempotent and should do nothing when \
         removing a valid purpose that isn't present"
        []
        (get_purpose ~self:network1)
  ) ;
  with_test (fun network _ remove_purpose get_purpose ->
