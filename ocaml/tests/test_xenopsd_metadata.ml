...
type vm_config = {oc: (string * string) list; platform: (string * string) list}
...
let load_vm_config __context conf =
  let (self : API.ref_VM) =
    make_vm ~__context ~name_label:test_vm_name ~domain_type:`hvm
      ~other_config:conf.oc ~platform:conf.platform ()
  in
  let flags =
    [
...
module HVMSerial = Generic.MakeStateful (struct
  module Io = struct
...
module VideoMode = Generic.MakeStateful (struct
  module Io = struct
    type input_t = vm_config
...
module VideoRam = Generic.MakeStateful (struct
  module Io = struct
    type input_t = vm_config
...
module GenerateVGPUMetadata = Generic.MakeStateful (struct
  ...
  let load_input __context (vm_config, pgpus_and_vgpu_types) =
    ...
    List.iteri
      (fun index (pgpu, vgpu_type) ->
        let pgpu_ref =
          make_pgpu ~__context
            ~address:(Printf.sprintf "0000:%02d:00.0" index)
            pgpu
        in
        let (_ : API.ref_VGPU) =
...
module GenerateMultiVGPUMetadata = Generic.MakeStateful (struct
  open Test_vgpu_common
  ...
  let tests =
    `QuickAndAutoDocumented
      [
        (* 2 NVIDIA vGPUs. *)
        ( ({oc= []; platform= []}, [(default_k1, k100); (default_k1, k100)])
        , [
            Xenops_interface.Vgpu.(
              Nvidia
                {
