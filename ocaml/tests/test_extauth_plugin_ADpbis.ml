  ...
  module Io = struct
    ...
    let string_of_output_t output =
      ...
      | Auth_signature.E_GENERIC ->
          "E_GENERIC"
      | Auth_signature.E_LOOKUP ->
          "E_LOOKUP"
      | Auth_signature.E_DENIED ->
          "E_DENIED"
      | Auth_signature.E_CREDENTIALS ->
          "E_CREDENTIALS"
      | Auth_signature.E_UNAVAILABLE ->
          "E_UNAVAILABLE"
      | Auth_signature.E_INVALID_OU ->
          "E_INVALID_OU"
      | Auth_signature.E_INVALID_ACCOUNT ->
          "E_INVALID_ACCOUNT"
  end
  ...
  let tests =
    `QuickAndAutoDocumented
      ...
      ; ( [
          ...
          ; ( ""
            , "Group[1 of 1] name = NVC\\testg(ab) (gid = 564135020, sid = \
      ...
      ; ( [
          ...
          ; ( ""
            , "Group[1 of 1] name = UN\\KnmOJ (gid = 492513842, sid = \
      ...
      ; ( [
          ...
          ; ( ""
            , "Group[1 of 2] name = testdomain\\dnsadmins (gid = 580912206, \
               sid = S-1-5-21-791009147-1041474540-2433379237-1102)" )
          ; ( ""
            , "Group[2 of 2] name = testdomain\\domain+users (gid = 580911617, \
               sid = S-1-5-21-791009147-1041474540-2433379237-513)" )
          ]
        , [
            "S-1-5-21-791009147-1041474540-2433379237-1102"
          ; "S-1-5-21-791009147-1041474540-2433379237-513"
          ] )
      ]
end)
