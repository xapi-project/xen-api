...
let k100 =
  ...
  ; internal_config= [(Xapi_globs.vgpu_type_id, "type_id_1")]
  ; identifier=
      Identifier.(
        Nvidia
          {
...
let k140q =
  ...
  ; internal_config= [(Xapi_globs.vgpu_type_id, "type_id_2")]
  ; identifier=
      Identifier.(
        Nvidia
          {
...
let k200 =
  ...
  ; max_resolution_y= 1200L
  ; size= Int64.div Constants.pgpu_default_size 8L
  ; internal_config= [(Xapi_globs.vgpu_type_id, "type_id_3")]
  ; identifier=
      Identifier.(
        Nvidia
          {
...
let k240q =
  ...
  ; max_resolution_y= 1600L
  ; size= Int64.div Constants.pgpu_default_size 4L
  ; internal_config= [(Xapi_globs.vgpu_type_id, "type_id_4")]
  ; identifier=
      Identifier.(
        Nvidia
          {
...
let k260q =
  ...
  ; internal_config= [(Xapi_globs.vgpu_type_id, "type_id_5")]
  ; identifier=
      Identifier.(
        Nvidia
          {
...
let k1_vgpu_types = [k100; k140q; passthrough_gpu]

let k2_vgpu_types = [k200; k240q; k260q; passthrough_gpu]
...
let intel_041a_vgpu_types = [gvt_g_041a; passthrough_gpu]
...
let string_of_vgpu_type vgpu_type = vgpu_type.model_name
...
let make_vgpu ~__context ?(vm_ref = Ref.null) ?(gPU_group = Ref.null)
    ?(device = "0") ?(resident_on = Ref.null)
    ?(scheduled_to_be_resident_on = Ref.null) ?(uuid = Test_common.make_uuid ())
    ?(extra_args = "") vgpu_type =
  ...
  (* For the passthrough VGPU type, create a VM and mark it as attached to the
     	 * PGPU's PCI device. *)
  let vm_ref =
    if Db.is_valid_ref __context vm_ref then
      vm_ref
    else
  ...
  Test_common.make_vgpu ~__context ~vM:vm_ref ~_type:vgpu_type_ref ~uuid
    ~gPU_group ~device ~resident_on ~scheduled_to_be_resident_on ~extra_args ()

let make_pgpu ~__context ?address ?(host = Ref.null) ?(gPU_group = Ref.null)
    pgpu =
  let pCI =
    Test_common.make_pci ~__context ?pci_id:address ~host ~functions:1L ()
  in
  ...
  in
  let pgpu_ref =
    Test_common.make_pgpu ~__context ~pCI ~host ~gPU_group ~supported_VGPU_types
      ~enabled_VGPU_types ()
  in
  List.iter
    (fun vgpu_type ->
