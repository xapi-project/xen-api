...
let test_create_invalid_site () =
  ...
  let vIF = T.make_vif ~__context ~device:"0" () in
  Alcotest.check_raises "test_create_invalid_site should raise invalid_value"
    Api_errors.(Server_error (invalid_value, ["site"; Ref.string_of site]))
    (fun () -> ignore (Xapi_pvs_proxy.create ~__context ~site ~vIF))
...
let test_create_invalid_vif () =
  ...
  let site = T.make_pvs_site ~__context () in
  let vIF = Ref.make () in
  Alcotest.check_raises "test_create_invalid_vif should raise invalid_value"
    Api_errors.(Server_error (invalid_value, ["VIF"; Ref.string_of vIF]))
    (fun () -> ignore (Xapi_pvs_proxy.create ~__context ~site ~vIF))
...
let test_destroy () =
  ...
  Alcotest.(check bool)
    "test_destroy: PVS proxy ref should no longer be valid" false
    (Db.is_valid_ref __context pvs_proxy)
...
let test_gc_proxy () =
  ...
  let compare_refs msg x y =
    Alcotest.(check (Alcotest_comparators.ref ())) msg x y
  in
  Db_gc_util.gc_PVS_proxies ~__context ;
  compare_refs "test_gc_proxy: get_site" site
    (Db.PVS_proxy.get_site ~__context ~self:proxy) ;
  compare_refs "test_gc_proxy: get_VIF" vIF
    (Db.PVS_proxy.get_VIF ~__context ~self:proxy) ;
  Db.PVS_proxy.set_site ~__context ~self:proxy ~value:Ref.null ;
  Db_gc_util.gc_PVS_proxies ~__context (* should collect the proxy *) ;
  Alcotest.(check bool)
    "test_gc_proxy: proxy ref should be invalid" false
    (Db.is_valid_ref __context proxy) ;
  let proxy = Xapi_pvs_proxy.create ~__context ~site ~vIF in
  Db_gc_util.gc_PVS_proxies ~__context ;
  compare_refs "test_gc_proxy: get_site (newly created proxy)" site
    (Db.PVS_proxy.get_site ~__context ~self:proxy) ;
  compare_refs "test_gc_proxy: get_VIF (newly created proxy)" vIF
    (Db.PVS_proxy.get_VIF ~__context ~self:proxy) ;
  Db.PVS_proxy.set_VIF ~__context ~self:proxy ~value:Ref.null ;
  Db_gc_util.gc_PVS_proxies ~__context (* should collect the proxy *) ;
  Alcotest.(check bool)
    "test_gc_proxy: proxy ref has been set to null" false
    (Db.is_valid_ref __context proxy)
