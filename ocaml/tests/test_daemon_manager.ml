...
type stop_failure = {
    error: exn  (** The exception thrown when trying to stop the daemon. *)
...
let check_times_called ~start ~stop =
  Alcotest.(check int)
    "times_called_start"
    !Mock_daemon.times_called_start
    start ;
...
let spawn_threads_and_wait task count =
  let rec spawn_threads task count acc =
    if count > 0 then
      ...
      acc
  in
  spawn_threads task count [] |> List.iter Thread.join
...
let test_timeout_succeed () =
  ...
  Mock_daemon.stop_failure :=
    Some {error= Failure "stop failed"; time_until_stopped= 2.0} ;
