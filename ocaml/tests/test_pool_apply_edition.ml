...
let setup ~host_count ~edition =
  ...
  List.iter
    (fun self -> Db.Host.set_edition ~__context ~self ~value:edition)
...
let test_basic_operation () =
  ...
  Xapi_pool_license.apply_edition_with_rollback ~__context ~hosts
    ~edition:"per-socket" ~apply_fn:apply_edition_succeed ;
  List.iter
    (fun host ->
...
let test_rollback_logic () =
  ...
  let apply_fn ~__context ~host ~edition =
    if host = offline_host then
      apply_edition_fail_host_offline ~__context ~host ~edition
    else
      apply_edition_succeed ~__context ~host ~edition
  in
