...
let assert_snapshot_of_is_not_null ~__context ~vdi_snapshot =
  ...
  Alcotest.(check bool)
    "VDI snapshot's `snapshot_of` reference has become null" true
    (vdi <> Ref.null)
...
let test_update_existing_snapshot () =
  ...
  let vdi_snapshot =
    T.make_vdi ~__context ~uuid:vdi_snapshot_uuid ~sR:sr
      ~location:vdi_snapshot_uuid ~snapshot_of:vdi ~is_a_snapshot:true ()
  in
...
let test_sharable_field_updated_for_existing_vdi () =
  ...
  let vdi =
    T.make_vdi ~__context ~sR:sr ~uuid:vdi_uuid ~location:vdi_uuid
      ~sharable:false ()
  in
  ...
  Alcotest.(check bool)
    "test_sharable_field_updated_for_existing_vdi" true
    (Db.VDI.get_sharable ~__context ~self:vdi)
...
let test_sharable_field_correct_for_new_vdi () =
  ...
  Alcotest.(check bool)
    "test_sharable_field_correct_for_new_vdi" true
    (Db.VDI.get_sharable ~__context ~self:vdi)
