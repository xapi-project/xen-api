...
let test_unlicensed () =
  ...
  Alcotest.check_raises "test_unlicensed"
...
let test_introduce_duplicate_addresses () =
  ...
  in
  Alcotest.(check (slist string compare))
    "test_introduce_duplicate_addresses" ["10.80.12.34"]
    (Db.PVS_server.get_addresses ~__context ~self:pvs_server)
...
let test_introduce_invalid_address () =
  ...
  let __context = T.make_test_database () in
  let valid_site = T.make_pvs_site ~__context ~name_label:"site" () in
  Alcotest.check_raises "test_introduce_invalid_address"
...
let test_introduce_invalid_ports () =
  ...
  let __context = T.make_test_database () in
  let valid_site = T.make_pvs_site ~__context ~name_label:"site" () in
  Alcotest.check_raises "test_introduce_invalid_ports"
    Api_errors.(
      Server_error
        ( value_not_supported
...
let test_forget () =
  ...
  Alcotest.(check bool)
    "test_forget" false
    (Db.is_valid_ref __context pvs_server)
...
let test_gc () =
  ...
  let server =
    Xapi_pvs_server.introduce ~__context ~addresses ~first_port ~last_port ~site
  in
  Db_gc_util.gc_PVS_servers ~__context ;
  Alcotest.check
    (Alcotest_comparators.ref ())
    "test_gc: PVS site" site
  ...
  Alcotest.(check bool)
    "test_gc: PVS server is valid ref" false
