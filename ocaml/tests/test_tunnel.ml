...
let test_create_on_unmanaged_pif () =
  ...
  let host = T.make_host ~__context () in
  let transport_PIF =
    T.create_physical_pif ~__context ~host ~managed:false ()
  in
  let network = T.make_network ~__context ~bridge:"xapi0" () in
...
let test_create_network_already_connected () =
  ...
  let network = T.make_network ~__context () in
  let transport_PIF =
    T.create_physical_pif ~__context ~host ~network ~managed:false ()
  in
  Alcotest.check_raises "test_create_network_already_connected"
    Api_errors.(
      Server_error
...
let test_create_on_vlan_on_sriov_logical () =
  ...
  in
  let network = T.make_network ~__context ~bridge:"xapi01" () in
  Alcotest.check_raises "test_create_on_vlan_on_sriov_logical"
    Api_errors.(
      Server_error
