...
let alco_power_state =
  Alcotest_comparators.from_rpc_of_t API.rpc_of_vm_power_state

let run_test vm =
  let nameLabel, isControlDomain, powerState = vm in
  let __context = Test_common.make_test_database () in
  let vm = Test_common.make_vm ~__context ~name_label:nameLabel () in
...
let test_reset_vms_on_missing_host () = List.iter run_test test_cases
