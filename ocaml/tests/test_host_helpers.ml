...
let compare_calls msg expected actual =
  let call_testable =
    let string_of_call = function
      | host, `set_multipathing b ->
          Printf.sprintf "%s, set_multipathing %b" (Ref.string_of host) b
      | host, `set_iscsi_iqn str ->
          Printf.sprintf "%s, set_iscsi_iqn %s" (Ref.string_of host) str
      | _ ->
          ""
    in
    Alcotest_comparators.from_to_string string_of_call
  in
  Alcotest.(check (slist call_testable compare)) msg expected actual
...
let setup_test_oc_watcher () =
  ...
  let watcher =
    Xapi_host_helpers.Configuration.watch_other_configs ~__context 0.0
  in
...
let test_host1 () =
  ...
  calls := [] ;
  Db.Host.add_to_other_config ~__context ~self:host1 ~key:"multipathing"
    ~value:"true" ;
...
let test_host2 () =
  ...
  calls := [] ;
  Db.Host.add_to_other_config ~__context ~self:host2 ~key:"multipathing"
    ~value:"false" ;
  let _token = watcher token in
...
let test_host_set_iscsi_iqn () =
  ...
  Db.Host.add_to_other_config ~__context ~self:host1 ~key:"iscsi_iqn"
    ~value:"test1" ;
  let token = watcher token in
  compare_calls "test_host_set_iscsi_iqn: add_to_other_config test1"
    [(host1, `set_iscsi_iqn "test1")]
    !calls ;
  calls := [] ;
...
let test_host_set_multipathing () =
  ...
  let token = watcher token in
  compare_calls
    "test_host_set_multipathing: add_to_other_config [host1, multipathing]"
    [(host1, `set_multipathing true)]
    !calls ;
  calls := [] ;
  Db.Host.remove_from_other_config ~__context ~self:host1 ~key:"multipathing" ;
  let token = watcher token in
  compare_calls
    "test_host_set_multipathing: remove_from_other_config [host1, multipathing]"
    [] !calls ;
  Db.Host.set_multipathing ~__context ~self:host1 ~value:false ;
  let token = watcher token in
