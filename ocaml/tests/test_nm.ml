...
module MaybeUpdateMasterPifMac = Generic.Make(Generic.EncapsulateState(struct
  module Io = struct
    ...
    let show_primary_slave_mac_change = function
      ...
      | Some new_primary_slave_mac -> Printf.sprintf "primary_slave mac -> %s" new_primary_slave_mac

    let string_of_input_t = Test_printers.pair Test_printers.bool show_primary_slave_mac_change
    let string_of_output_t (rc,db) = Printf.sprintf "Bond master macs. record: %s db : %s" rc db
  end
  ...
  let load_input (__context : State.state_t) (auto_update_mac, o_new_primary_slave_mac) =
    let network = Test_common.make_network ~__context () in
    let host = Test_common.make_host ~__context () in
    let bond_master = Test_common.make_pif ~__context ~network ~host () ~mAC:original_master_mac in
    let primary_slave = Test_common.make_pif ~__context ~network ~host ~bond_slave_of:bond_master () ~mAC:"aa:11:11:11:11:11" in
    let _ = Test_common.make_pif ~__context ~network ~host ~bond_slave_of:bond_master () ~mAC:"bb:11:11:11:11:11" (* lowly slave *) in
    let bond = Test_common.make_bond ~__context ~master:bond_master ~primary_slave () in
    (Db.Bond.set_auto_update_mac ~__context ~self:bond ~value:auto_update_mac;
     match o_new_primary_slave_mac with
      | None -> ()
      | Some new_primary_slave_mac -> Db.PIF.set_MAC ~__context ~self:primary_slave ~value:new_primary_slave_mac)
  ...
  let extract_output (__context : State.state_t) _ =
    ...
    let bond_master_record = (Nm.maybe_update_master_pif_mac ~__context bond_record bond_master_record bond_master) in
