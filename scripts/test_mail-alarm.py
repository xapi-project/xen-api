...
def get_alarm_xml(xmlalarm_str, xmlcls_str, xmlname_str, xmlbody_str):
    return XML_MESSAGE_TEMPLATE.format(alarm=xmlalarm_str, cls=xmlcls_str, body=xmlbody_str.format(name=xmlname_str))
...
class TestXapiMessage(unittest.TestCase):
    ...
    @nottest
    def common_test_good_input(self, xmlalarm_str, xmlcls_str, xmlname_str, subject_str, body_str, xmlbody_str=XML_BODY_COMMON):
        import mailalarm
    ...
    @nottest
    def common_test_bad_input(self, xmlalarm_str, xmlcls_str, xmlname_str, title_str, subtitle_str, xmlbody_str=XML_BODY_COMMON):
        global log_file_global
        import mailalarm
        ...
        self.assertIn('[{title}] error in looking for ({subtitle}:subject). Mail language pack:(None)'.format(title=title_str, subtitle=subtitle_str), log_strs)
        self.assertIn('[{title}] error in looking for ({subtitle}:body). Mail language pack:(None)'.format(title=title_str, subtitle=subtitle_str), log_strs)

        os.remove(log_file_global)

    def test_good_mail_language(self):
        # Test cpu_usage alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: CPU usage on Host "test_host"'
        ...
        self.common_test_good_input('ALARM', 'Host', 'cpu_usage', mail_subject_expected, mail_body_expected)

        # Test network_usage alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: Network usage on Host "test_host"'
        ...
        self.common_test_good_input('ALARM', 'Host', 'network_usage', mail_subject_expected, mail_body_expected)

        # Test memory_free_kib alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: Memory usage on Host "test_host"'
        ...
        self.common_test_good_input('ALARM', 'Host', 'memory_free_kib', mail_subject_expected, mail_body_expected)

        # Test disk_usage alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: Disk usage on VM "test_VM"'
        ...
        self.common_test_good_input('ALARM', 'VM', 'disk_usage', mail_subject_expected, mail_body_expected)

        # Test fs_usage alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: Filesystem nearly full on "test_VM"'
        ...
        self.common_test_good_input('ALARM', 'VM', 'fs_usage', mail_subject_expected, mail_body_expected)

        # Test log_fs_usage alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: Log partition nearly full on "test_VM"'
        ...
        self.common_test_good_input('ALARM', 'VM', 'log_fs_usage', mail_subject_expected, mail_body_expected)

        # Test mem_usage alarm
        mail_subject_expected = '[test_pool] XenServer Alarm: Dom0 memory demand is high on "test_VM"'
        ...
        self.common_test_good_input('ALARM', 'VM', 'mem_usage', mail_subject_expected, mail_body_expected)

        # Test sr_io_throughput_total_11111111 alarm
        mail_subject_expected = '[test_pool] Storage Throughput Alarm: The total IO throughput on "test_sr"'
        ...
        mail_subject_expected = 'Workload Balancing Alert: Optimization alert from pool test_pool'
        ...
        self.common_test_good_input('WLB_OPTIMIZATION_ALERT', 'Host', '', mail_subject_expected, mail_body_expected, xmlbody_str)

    def test_bad_mail_language(self):
        # Test cpu_usage alarm
        title_expected = 'CpuUsageAlarmETG'
        subtitle_expected = 'cpu_usage_alarm'
        self.common_test_bad_input('ALARM', 'Host', 'cpu_usage', title_expected, subtitle_expected)

        # Test network_usage alarm
        title_expected = 'NetworkUsageAlarmETG'
        subtitle_expected = 'network_usage_alarm'
        self.common_test_bad_input('ALARM', 'Host', 'network_usage', title_expected, subtitle_expected)

        # Test memory_free_kib alarm
        title_expected = 'MemoryUsageAlarmETG'
        subtitle_expected = 'memory_usage_alarm'
        self.common_test_bad_input('ALARM', 'Host', 'memory_free_kib', title_expected, subtitle_expected)

        # Test disk_usage alarm
        title_expected = 'DiskUsageAlarmETG'
        subtitle_expected = 'disk_usage_alarm'
        self.common_test_bad_input('ALARM', 'VM', 'disk_usage', title_expected, subtitle_expected)

        # Test fs_usage alarm
        title_expected = 'Dom0FSUsageAlarmETG'
        subtitle_expected = 'dom0fs_usage_alarm'
        self.common_test_bad_input('ALARM', 'VM', 'fs_usage', title_expected, subtitle_expected)

        # Test log_fs_usage alarm
        title_expected = 'Dom0LogFSUsageAlarmETG'
        subtitle_expected = 'dom0logfs_usage_alarm'
        self.common_test_bad_input('ALARM', 'VM', 'log_fs_usage', title_expected, subtitle_expected)

        # Test mem_usage alarm
        title_expected = 'Dom0MemUsageAlarmETG'
        subtitle_expected = 'dom0memory_usage_alarm'
        self.common_test_bad_input('ALARM', 'VM', 'mem_usage', title_expected, subtitle_expected)
        ...
        xmlbody_str = 'WLB consultation failed'
        self.common_test_bad_input('WLB_CONSULTATION_FAILED', 'VM', '', title_expected, subtitle_expected, xmlbody_str)

        # Test WLB_OPTIMIZATION_ALERT
        title_expected = 'WlbOptimizationAlert'
        subtitle_expected = 'wlb_optimization_alert'
        xmlbody_str = 'severity:Critical mode:MaxPerformance'
        self.common_test_bad_input('WLB_OPTIMIZATION_ALERT', 'Host', '', title_expected, subtitle_expected, xmlbody_str)
