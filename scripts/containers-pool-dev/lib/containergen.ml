...
module Command : sig
   ...
   *)
  val with_cache : ?id:string -> ?sharing:sharing -> ?from:string -> ?source:Fpath.t -> ?uidgid:(int*int) -> ?mode:int -> target:Fpath.t -> t -> t
   ...
   *)
  val with_committed_cache : ?id:string -> ?sharing:sharing -> ?from:string ->
    ?source:Fpath.t -> ?uidgid:(int*int) -> ?mode:int -> target:Fpath.t -> Cmd.t -> Dockerfile.t
  ...
  *)
  ...
  let string_of_uidgid (uid, gid) = Printf.sprintf "%d:%d" uid gid
  ...
  let with_mount typ ~target options cmd =
    ...
    let mount =
      ...
      |> List.filter_map string_of_kv |> String.concat ","
      |> Printf.sprintf "--mount=type=%s%s" typ
    in
    { cmd with mounts = StringSet.add mount cmd.mounts }

  let with_cache ?id ?(sharing=Shared) ?from ?source
    ?uidgid ?mode =
      ...
      with_mount "cache"
        ...
        ; "uidgid", Option.map string_of_uidgid uidgid
  ...
  let rsync ~src ~dst =
    Cmd.(v "rsync" % "-a" % "--whole-file" % "--no-compress" % p Fpath.(src / "") % p Fpath.(dst / ""))

  let with_committed_cache ?id ?sharing ?from ?source ?uidgid ?mode ~target
    cmd =
      let cache_target = Fpath.(home / ".cache" // target) in
      let with_cache cmd =
        with_cache ?id ?sharing ?from ?source ?uidgid ?mode ~target:cache_target
        @@ v cmd
      in
      run
      [ with_cache Cmd.(rsync ~src:cache_target ~dst:target)
      ; with_cache cmd
      ; with_cache Cmd.(rsync ~src:target ~dst:cache_target)
      ]
end
...
module Dune = struct
  let with_dune cmd =
    ...
    ~target:Fpath.(home // v ".cache/dune") cmd
end
...
module Opam = struct
  ...
  let install ?(ignore_pin_depends=false) ?(deps_only=false) ?(locked=false) packages =
    with_opam_download_cache @@
    Command.v Cmd.(opam % "install"
...
module Yum = struct
  ...
  let install packages =
    with_cache @@ Command.v Cmd.(v "sudo" % "yum" % "install" % "-y" %% of_list packages)
end
