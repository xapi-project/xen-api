...
module Connections = struct
  ...
  let get_session conn_id =
    ...
    if sessions = StringStringRelation.B_Set.empty then
      None
    else
      Some (StringStringRelation.B_Set.choose sessions)

  let get_origin conn_id =
    match get_session conn_id with
    | None ->
        Message_switch_core.Protocol.Anonymous conn_id
    | Some x ->
        Message_switch_core.Protocol.Name x
...
let snapshot queues =
  ...
  let all_queues =
    List.map
      (fun n -> (n, Q.Directory.find queues n))
      (Q.Directory.list queues "")
  in
...
let process_request conn_id queues session request =
  ...
  | _, In.Get path ->
      ...
      Lwt_io.close ic >>= fun () -> return (None, Out.Get txt)
  | None, _ ->
  ...
  | Some session, In.Send (name, data) -> (
      ...
      match Q.send queues origin name data with
      | None ->
  ...
  | Some _session, In.Shutdown ->
      ...
      let (_ : unit Lwt.t) = Lwt_unix.sleep 1. >>= fun () -> exit 0 in
