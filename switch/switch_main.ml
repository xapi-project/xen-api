...
module Config = struct
  ...
  let term =
    ...
    let pidfile =
      ...
      Arg.(value & opt (some string) None & info ["pidfile"] ~doc)
    in
    let configfile =
      let doc = "Path to a config file" in
      Arg.(value & opt (some string) default.configfile & info ["config"] ~doc)
    in
    let statedir =
...
let make_server config =
  ...
  let load statedir =
    let final_path = Filename.concat statedir _dump_file in
    Lwt_unix.stat final_path >>= fun stats ->
  ...
  in
  let module Redo_log =
    Shared_block.Journal.Make (Logging.Lwt_logger) (Block) (Time) (Clock) (Q.Op)
  in
  (* In-memory cache *)
  ...
  let process_redo_log statedir ops =
    ...
    save statedir on_disk_queues' >>= fun () ->
    on_disk_queues := on_disk_queues' ;
    return (Result.Ok ())
  in
  let redo_log =
    match config.statedir with
    | None ->
        return None
    | Some statedir ->
        ...
        then (
            ...
            Lwt.catch
              (fun () -> Lwt_unix.unlink redo_log_path)
              (fun _ -> return ())
            >>= fun () ->
            Lwt_unix.openfile redo_log_path
            ...
            Lwt_unix.write fd byte 0 1 >>= fun n ->
            if n <> 1 then (
        ...
        load statedir >>= fun qs ->
        on_disk_queues := qs ;
        info "Reading the redo-log from %s" redo_log_path ;
        Block.connect ~buffered:true redo_log_path >>= fun block ->
        let open Lwt_result in
        Redo_log.start ~flush_interval:5_000_000_000L block
          (process_redo_log statedir)
        >>= fun redo_log ->
        info "Redo-log playback complete: everything should be in sync" ;
        queues := !on_disk_queues ;
  ...
  let conn_closed (_, conn_id) =
    ...
    | None ->
        ()
    | Some session ->
        if not (Connections.is_session_active session) then
          let (_ : unit Lwt.t) =
            info "Session %s cleaning up" session ;
            let qs = Q.owned_queues !queues session in
            let ops =
              Q.StringSet.fold
                (fun x ops -> Q.Directory.remove !queues x :: ops)
                qs []
            in
            Lwt_mutex.with_lock Switch_main_helper.m (fun () -> perform ops)
          in
          ()
  in
  ...
  Lwt.catch
    ...
    (function Unix.Unix_error (Unix.ENOENT, _, _) -> return () | e -> fail e)
  >>= fun () ->
  ...
  let on_exn = function
    ...
    | Unix.Unix_error (error, func, arg) ->
        let msg =
          Printf.sprintf "Client connection error %s: %s(%S)"
            (Unix.error_message error) func arg
        in
        Lwt_log.ign_warning msg
    | exn ->
        Lwt_log.ign_error ~exn "Unhandled exception"
  in
  Cohttp_lwt_unix.Server.create ~on_exn
    ~mode:(`Unix_domain_socket (`File config.path))
    t
...
let cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "The switch stores a set of named queues, each containing an ordered \
         sequence of messages. Clients may enqueue and acknowledge messages \
