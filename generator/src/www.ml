...
let rec html_of_t env =
  ...
  | Name x ->
    ...
    let target = Uri.of_string ("#a-" ^ x) in
...
let sidebar env x =
  let of_typedecl name_opt t =
    ...
    li (a ~href:(Uri.of_string target) name)
  in
  ...
  div ~cls:"large-3 medium-3 columns" (
    ul ~cls:"menu vertical" ([
        ...
        li (a ~href:(Uri.of_string "#a-exceptions") (string "Exceptions"))
      ])
  )
...
let tabs_of env is i m =
  ...
  let hash_ocaml = Uri.of_string ("#ocaml-" ^ mname) in
  let hash_python = Uri.of_string ("#python-" ^ mname) in
  ...
  list [
    ...
    div ~cls:"tabs-content" ~attrs:["data-tabs-content", id_tab] (list [
      ...
      div ~cls:"tabs-panel" ~id:id_ocaml (list [
        h4 (string "Client");
        string (
          Ocaml.caml2html (Ocaml.example_stub env is i m |> Ocaml.string_of_ts)
        );
        h4 (string "Server");
        string (
          Ocaml.caml2html (Ocaml.example_skeleton_user env is i m |> Ocaml.string_of_ts)
        )
      ]);
      div ~cls:"tabs-panel" ~id:id_python (list [
        h4 (string "Client");
        tag "pre" ~cls:"prettyprint lang-py" (string 
          (Python.example_stub_user env i m |> Python.string_of_ts)
        );
        h4 (string "Server");
        tag "pre" ~cls:"prettyprint lang-py" (string 
          (Python.example_skeleton_user env i m |> Python.string_of_ts)
        )
      ])
    ])
  ]
...
let topbar pages =
  let link_of_page page =
    let html = Uri.of_string (page.name ^ ".html") in
  ...
  let header = header (list [
    div ~cls:"row" (
      div ~cls:"large-12 columns" (list [
        ...
        h3 ~cls:"subheader" (list [
          string "An easy way to connect ";
          a ~href:(Uri.of_string "http://www.xenproject.org/developers/teams/xapi.html")
            (string "Xapi");
    ...
    div ~cls:"row" (list [
      ...
      div ~cls:"large-6 columns" (list [
        p (list [
          string "This is also for anyone who wants to manage their \
    ...
    div ~cls:"row" (
      div ~cls:"large-12 columns panel callout" (list [
        ...
        ul ([
          li (list[
            string "view the ";
            a ~href:(Uri.of_string "https://github.com/djs55/xapi-storage/issues")
              (string "issues on github"); 
            string " or"
          ]);
          li (list [
            string "join the ";
            a ~href:(Uri.of_string "http://lists.xenproject.org/mailman/listinfo/xen-api")
              (string "mailing list")
          ])
        ])
      ])
