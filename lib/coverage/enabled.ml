...
module Bisect = struct
  ...
  let dump jobid =
    ...
    (* dump coverage information in same location as it would normally get
       ...
       prefix/suffix to avoid clashes with filenames created at exit by bisect
       itself. *)
    let tmp, ch =
      Filename.open_temp_file
        ~temp_dir:(Filename.dirname bisect_prefix)
        (Filename.basename bisect_prefix)
        (Printf.sprintf ".%Ld.out" jobid)
    in
    try
...
let dispatcher_init _name = Dispatcher.init ()
