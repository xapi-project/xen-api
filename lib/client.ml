...
module ClientF = functor(X : IO) ->struct
  ...
  module Async = struct
    module Session = struct
      ...
      let create_from_db_file ~rpc ~session_id ~filename =
        ...
        rpc_wrapper rpc "Async.session.create_from_db_file" [ session_id; filename ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
    ...
    module Pool = struct
      ...
      let management_reconfigure ~rpc ~session_id ~network =
        ...
        rpc_wrapper rpc "Async.pool.management_reconfigure" [ session_id; network ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let get_license_state ~rpc ~session_id ~self =
        ...
        rpc_wrapper rpc "Async.pool.get_license_state" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let apply_edition ~rpc ~session_id ~self ~edition =
        ...
        rpc_wrapper rpc "Async.pool.apply_edition" [ session_id; self; edition ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let enable_ssl_legacy ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool self in

        rpc_wrapper rpc "Async.pool.enable_ssl_legacy" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
    ...
    module Pool_update = struct
      ...
      let introduce ~rpc ~session_id ~vdi =
        ...
        rpc_wrapper rpc "Async.pool_update.introduce" [ session_id; vdi ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let precheck ~rpc ~session_id ~self ~host =
        let session_id = rpc_of_ref_session session_id in
        ...
        rpc_wrapper rpc "Async.pool_update.precheck" [ session_id; self; host ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let apply ~rpc ~session_id ~self ~host =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool_update self in
        ...
        rpc_wrapper rpc "Async.pool_update.apply" [ session_id; self; host ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let pool_apply ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool_update self in

        rpc_wrapper rpc "Async.pool_update.pool_apply" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let pool_clean ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool_update self in

        rpc_wrapper rpc "Async.pool_update.pool_clean" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let destroy ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool_update self in

        rpc_wrapper rpc "Async.pool_update.destroy" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let attach ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool_update self in

        rpc_wrapper rpc "Async.pool_update.attach" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let detach ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_pool_update self in

        rpc_wrapper rpc "Async.pool_update.detach" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let resync_host ~rpc ~session_id ~host =
        let session_id = rpc_of_ref_session session_id in
        let host = rpc_of_ref_host host in

        rpc_wrapper rpc "Async.pool_update.resync_host" [ session_id; host ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    module VM = struct
      (**  *)
      let create ~rpc ~session_id ~name_label ~name_description ~user_version ~is_a_template ~affinity ~memory_target ~memory_static_max ~memory_dynamic_max ~memory_dynamic_min ~memory_static_min ~vCPUs_params ~vCPUs_max ~vCPUs_at_startup ~actions_after_shutdown ~actions_after_reboot ~actions_after_crash ~pV_bootloader ~pV_kernel ~pV_ramdisk ~pV_args ~pV_bootloader_args ~pV_legacy_args ~hVM_boot_policy ~hVM_boot_params ~hVM_shadow_multiplier ~platform ~pCI_bus ~other_config ~recommendations ~xenstore_data ~ha_always_run ~ha_restart_priority ~tags ~blocked_operations ~protection_policy ~is_snapshot_from_vmpp ~snapshot_schedule ~is_vmss_snapshot ~appliance ~start_delay ~shutdown_delay ~order ~suspend_SR ~version ~generation_id ~hardware_platform_version ~has_vendor_device ~reference_label =
        let session_id = rpc_of_ref_session session_id in
      ...
      let shutdown ~rpc ~session_id ~vm =
        ...
        rpc_wrapper rpc "Async.VM.shutdown" [ session_id; vm ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let set_is_default_template ~rpc ~session_id ~vm ~value =
        ...
        rpc_wrapper rpc "Async.VM.set_is_default_template" [ session_id; vm; value ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let get_SRs_required_for_recovery ~rpc ~session_id ~self ~session_to =
        ...
        rpc_wrapper rpc "Async.VM.get_SRs_required_for_recovery" [ session_id; self; session_to ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
    ...
    module VMSS = struct
      ...
      let create ~rpc ~session_id ~name_label ~name_description ~enabled ~_type ~retained_snapshots ~frequency ~schedule =
        ...
        rpc_wrapper rpc "Async.VMSS.create" [ session_id; args ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let destroy ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_VMSS self in

        rpc_wrapper rpc "Async.VMSS.destroy" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let create_from_record ~rpc ~session_id ~value =
        create
          ~rpc
    ...
    module VM_appliance = struct
      ...
      let get_SRs_required_for_recovery ~rpc ~session_id ~self ~session_to =
        ...
        rpc_wrapper rpc "Async.VM_appliance.get_SRs_required_for_recovery" [ session_id; self; session_to ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let recover ~rpc ~session_id ~self ~session_to ~force =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_VM_appliance self in
    ...
    module Host = struct
      ...
      let declare_dead ~rpc ~session_id ~host =
        ...
        rpc_wrapper rpc "Async.host.declare_dead" [ session_id; host ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let enable_display ~rpc ~session_id ~host =
        let session_id = rpc_of_ref_session session_id in
        let host = rpc_of_ref_host host in

        rpc_wrapper rpc "Async.host.enable_display" [ session_id; host ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let disable_display ~rpc ~session_id ~host =
        let session_id = rpc_of_ref_session session_id in
        let host = rpc_of_ref_host host in

        rpc_wrapper rpc "Async.host.disable_display" [ session_id; host ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let mxgpu_vf_setup ~rpc ~session_id ~host =
        ...
        rpc_wrapper rpc "Async.host.mxgpu_vf_setup" [ session_id; host ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    ...
    module VIF = struct
      ...
      let move ~rpc ~session_id ~self ~network =
        ...
        rpc_wrapper rpc "Async.VIF.move" [ session_id; self; network ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
    ...
    module PIF = struct
      ...
      let introduce ~rpc ~session_id ~host ~mAC ~device ~managed =
        ...
        rpc_wrapper rpc "Async.PIF.introduce" [ session_id; host; mAC; device; managed ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let pool_introduce ~rpc ~session_id ~device ~network ~host ~mAC ~mTU ~vLAN ~physical ~ip_configuration_mode ~iP ~netmask ~gateway ~dNS ~bond_slave_of ~vLAN_master_of ~management ~other_config ~disallow_unplug ~ipv6_configuration_mode ~iPv6 ~ipv6_gateway ~primary_address_type ~managed ~properties =
        ...
        rpc_wrapper rpc "Async.PIF.pool_introduce" [ session_id; device; network; host; mAC; mTU; vLAN; physical; ip_configuration_mode; iP; netmask; gateway; dNS; bond_slave_of; vLAN_master_of; management; other_config; disallow_unplug; ipv6_configuration_mode; iPv6; ipv6_gateway; primary_address_type; managed; properties ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let db_introduce ~rpc ~session_id ~device ~network ~host ~mAC ~mTU ~vLAN ~physical ~ip_configuration_mode ~iP ~netmask ~gateway ~dNS ~bond_slave_of ~vLAN_master_of ~management ~other_config ~disallow_unplug ~ipv6_configuration_mode ~iPv6 ~ipv6_gateway ~primary_address_type ~managed ~properties =
        ...
        rpc_wrapper rpc "Async.PIF.db_introduce" [ session_id; device; network; host; mAC; mTU; vLAN; physical; ip_configuration_mode; iP; netmask; gateway; dNS; bond_slave_of; vLAN_master_of; management; other_config; disallow_unplug; ipv6_configuration_mode; iPv6; ipv6_gateway; primary_address_type; managed; properties ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let set_property ~rpc ~session_id ~self ~name ~value =
        ...
        rpc_wrapper rpc "Async.PIF.set_property" [ session_id; self; name; value ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    ...
    module VLAN = struct
      ...
      let pool_introduce ~rpc ~session_id ~tagged_PIF ~untagged_PIF ~tag ~other_config =
        ...
        rpc_wrapper rpc "Async.VLAN.pool_introduce" [ session_id; tagged_PIF; untagged_PIF; tag; other_config ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
    ...
    module VDI = struct
      ...
      let pool_introduce ~rpc ~session_id ~uuid ~name_label ~name_description ~sR ~_type ~sharable ~read_only ~other_config ~location ~xenstore_data ~sm_config ~managed ~virtual_size ~physical_utilisation ~metadata_of_pool ~is_a_snapshot ~snapshot_time ~snapshot_of ~cbt_enabled =
        ...
        rpc_wrapper rpc "Async.VDI.pool_introduce" [ session_id; uuid; name_label; name_description; sR; _type; sharable; read_only; other_config; location; xenstore_data; sm_config; managed; virtual_size; physical_utilisation; metadata_of_pool; is_a_snapshot; snapshot_time; snapshot_of; cbt_enabled ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let db_introduce ~rpc ~session_id ~uuid ~name_label ~name_description ~sR ~_type ~sharable ~read_only ~other_config ~location ~xenstore_data ~sm_config ~managed ~virtual_size ~physical_utilisation ~metadata_of_pool ~is_a_snapshot ~snapshot_time ~snapshot_of ~cbt_enabled =
        ...
        rpc_wrapper rpc "Async.VDI.db_introduce" [ session_id; uuid; name_label; name_description; sR; _type; sharable; read_only; other_config; location; xenstore_data; sm_config; managed; virtual_size; physical_utilisation; metadata_of_pool; is_a_snapshot; snapshot_time; snapshot_of; cbt_enabled ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let copy ~rpc ~session_id ~vdi ~sr ~base_vdi ~into_vdi =
        ...
        rpc_wrapper rpc "Async.VDI.copy" [ session_id; vdi; sr; base_vdi; into_vdi ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      ...
      let enable_cbt ~rpc ~session_id ~self =
        ...
        rpc_wrapper rpc "Async.VDI.enable_cbt" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
    ...
    module PGPU = struct
      ...
      let add_enabled_VGPU_types ~rpc ~session_id ~self ~value =
        ...
        rpc_wrapper rpc "Async.PGPU.add_enabled_VGPU_types" [ session_id; self; value ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let remove_enabled_VGPU_types ~rpc ~session_id ~self ~value =
        ...
        rpc_wrapper rpc "Async.PGPU.remove_enabled_VGPU_types" [ session_id; self; value ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let set_enabled_VGPU_types ~rpc ~session_id ~self ~value =
        ...
        rpc_wrapper rpc "Async.PGPU.set_enabled_VGPU_types" [ session_id; self; value ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let set_GPU_group ~rpc ~session_id ~self ~value =
        ...
        rpc_wrapper rpc "Async.PGPU.set_GPU_group" [ session_id; self; value ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let get_remaining_capacity ~rpc ~session_id ~self ~vgpu_type =
        ...
        rpc_wrapper rpc "Async.PGPU.get_remaining_capacity" [ session_id; self; vgpu_type ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let enable_dom0_access ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_PGPU self in

        rpc_wrapper rpc "Async.PGPU.enable_dom0_access" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let disable_dom0_access ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_PGPU self in

        rpc_wrapper rpc "Async.PGPU.disable_dom0_access" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    module GPU_group = struct
      ...
      let update_enabled_VGPU_types ~rpc ~session_id ~self =
        ...
        rpc_wrapper rpc "Async.GPU_group.update_enabled_VGPU_types" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let update_supported_VGPU_types ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_GPU_group self in

        rpc_wrapper rpc "Async.GPU_group.update_supported_VGPU_types" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let get_remaining_capacity ~rpc ~session_id ~self ~vgpu_type =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_GPU_group self in
        ...
        rpc_wrapper rpc "Async.GPU_group.get_remaining_capacity" [ session_id; self; vgpu_type ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    module VGPU = struct
      (**  *)
      let create ~rpc ~session_id ~vM ~gPU_group ~device ~other_config ~_type =
        ...
        rpc_wrapper rpc "Async.VGPU.create" [ session_id; vM; gPU_group; device; other_config; _type ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let destroy ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
      ...
      let atomic_set_resident_on ~rpc ~session_id ~self ~value =
        ...
        rpc_wrapper rpc "Async.VGPU.atomic_set_resident_on" [ session_id; self; value ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    ...
    module PVS_site = struct
      ...
      let introduce ~rpc ~session_id ~name_label ~name_description ~pVS_uuid =
        ...
        rpc_wrapper rpc "Async.PVS_site.introduce" [ session_id; name_label; name_description; pVS_uuid ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let forget ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_PVS_site self in

        rpc_wrapper rpc "Async.PVS_site.forget" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let set_PVS_uuid ~rpc ~session_id ~self ~value =
        ...
        rpc_wrapper rpc "Async.PVS_site.set_PVS_uuid" [ session_id; self; value ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    module PVS_server = struct
      ...
      let introduce ~rpc ~session_id ~addresses ~first_port ~last_port ~site =
        ...
        rpc_wrapper rpc "Async.PVS_server.introduce" [ session_id; addresses; first_port; last_port; site ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let forget ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_PVS_server self in

        rpc_wrapper rpc "Async.PVS_server.forget" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    module PVS_proxy = struct
      ...
      let create ~rpc ~session_id ~site ~vIF =
        ...
        rpc_wrapper rpc "Async.PVS_proxy.create" [ session_id; site; vIF ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let destroy ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_PVS_proxy self in

        rpc_wrapper rpc "Async.PVS_proxy.destroy" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
    end
    module PVS_cache_storage = struct
      ...
      let create ~rpc ~session_id ~host ~sR ~site ~size =
        ...
        rpc_wrapper rpc "Async.PVS_cache_storage.create" [ session_id; args ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let destroy ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
        let self = rpc_of_ref_PVS_cache_storage self in

        rpc_wrapper rpc "Async.PVS_cache_storage.destroy" [ session_id; self ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let create_from_record ~rpc ~session_id ~value =
        create
          ~rpc
    ...
    module SDN_controller = struct
      ...
      let introduce ~rpc ~session_id ~protocol ~address ~port =
        ...
        rpc_wrapper rpc "Async.SDN_controller.introduce" [ session_id; protocol; address; port ] >>= fun x -> return (ref_task_of_rpc  x)
      (**  *)
      let forget ~rpc ~session_id ~self =
        let session_id = rpc_of_ref_session session_id in
  ...
  module Pool_update = struct
    ...
    let attach ~rpc ~session_id ~self =
      ...
      rpc_wrapper rpc "pool_update.attach" [ session_id; self ] >>= fun x -> return (string_of_rpc  x)
    (**  *)
    let detach ~rpc ~session_id ~self =
      let session_id = rpc_of_ref_session session_id in
      let self = rpc_of_ref_pool_update self in
  ...
  module VM = struct
    ...
    let migrate_send ~rpc ~session_id ~vm ~dest ~live ~vdi_map ~vif_map ~options =
      ...
      rpc_wrapper rpc "VM.migrate_send" [ session_id; vm; dest; live; vdi_map; vif_map; options ] >>= fun x -> return (ref_VM_of_rpc  x)
    (**  *)
    let assert_can_migrate ~rpc ~session_id ~vm ~dest ~live ~vdi_map ~vif_map ~options =
      let session_id = rpc_of_ref_session session_id in
      let vm = rpc_of_ref_VM vm in
