...
let rpc_of_pool_update_t x = Rpc.Dict [ "uuid",rpc_of_string x.pool_update_uuid; "name_label",rpc_of_string x.pool_update_name_label; "name_description",rpc_of_string x.pool_update_name_description; "version",rpc_of_string x.pool_update_version; "installation_size",rpc_of_int64 x.pool_update_installation_size; "key",rpc_of_string x.pool_update_key; "after_apply_guidance",rpc_of_update_after_apply_guidance_set x.pool_update_after_apply_guidance; "vdi",rpc_of_ref_VDI x.pool_update_vdi; "hosts",rpc_of_ref_host_set x.pool_update_hosts; "other_config",rpc_of_string_to_string_map x.pool_update_other_config; "enforce_homogeneity",rpc_of_bool x.pool_update_enforce_homogeneity ]
let pool_update_t_of_rpc x = on_dict (fun x -> { pool_update_uuid = string_of_rpc (List.assoc "uuid" x); pool_update_name_label = string_of_rpc (List.assoc "name_label" x); pool_update_name_description = string_of_rpc (List.assoc "name_description" x); pool_update_version = string_of_rpc (List.assoc "version" x); pool_update_installation_size = int64_of_rpc (List.assoc "installation_size" x); pool_update_key = string_of_rpc (List.assoc "key" x); pool_update_after_apply_guidance = update_after_apply_guidance_set_of_rpc (List.assoc "after_apply_guidance" x); pool_update_vdi = ref_VDI_of_rpc (List.assoc "vdi" x); pool_update_hosts = ref_host_set_of_rpc (List.assoc "hosts" x); pool_update_other_config = string_to_string_map_of_rpc (List.assoc "other_config" x); pool_update_enforce_homogeneity = bool_of_rpc (List.assoc "enforce_homogeneity" x) }) x
