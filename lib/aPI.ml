...
type vdi_operations = [ `clone | `copy | `resize | `resize_online | `snapshot | `mirror | `destroy | `forget | `update | `force_unlock | `generate_config | `enable_cbt | `disable_cbt | `set_on_boot | `blocked ] [@@deriving rpc]
...
type storage_operations = [ `scan | `destroy | `forget | `plug | `unplug | `update | `vdi_create | `vdi_introduce | `vdi_destroy | `vdi_resize | `vdi_clone | `vdi_snapshot | `vdi_mirror | `vdi_enable_cbt | `vdi_disable_cbt | `vdi_set_on_boot | `pbd_create | `pbd_destroy ] [@@deriving rpc]
