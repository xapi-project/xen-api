...
module Domain = struct
	type t = {
		...
		| HVM hvm_info ->
			...
			| Some (Xen(_, _)) -> string "loader" !Resources.hvmloader output
			| _ -> ()
			end;
		...
		let of_vif vif output =
			...
			(* empty ~attr:["path", !Resources.vif_script] "script" output; *)
		...
		let of_vm x output =
			...
			| Some (Xen(_, _)) -> !Resources.qemu_system_i386
			| _ -> qemu_path in
			emulator e output;
...
module VBD = struct
	...
	| None ->
		None
	| Some (Local path) ->
		Some { Storage_interface.params=path; o_direct=true; o_direct_reason=""; xenstore_data=[]; }
	| Some (VDI path) ->
