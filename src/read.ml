...
let safe_int_of_int32 pos x =
  if arch_sixtyfour then
    Int32.to_int x
  else if x >= min_int_int32 && x <= max_int_int32 then
    Int32.to_int x
  else
...
let safe_nativeint_of_int64 =
  if arch_sixtyfour then
    fun _pos x -> Int64.to_nativeint x
  else
    fun pos x ->
      if x >= Int64.of_nativeint Nativeint.min_int &&
         x <= Int64.of_nativeint Nativeint.max_int then
...
let unsafe_get16be_unsigned =
  if arch_big_endian then
    ...
    fun buf pos -> unsafe_get16 buf pos |> bswap16
let unsafe_get32be =
  if arch_big_endian then
    unsafe_get32
  else
    fun buf pos -> unsafe_get32 buf pos |> bswap32
let unsafe_get64be =
  if arch_big_endian then
    unsafe_get64
  else
    fun buf pos -> unsafe_get64 buf pos |> bswap64
  ...
  then
    ...
    if len = 0 then [||]
      ...
      if arch_sixtyfour then begin
        ...
        if len > Sys.max_array_length / 2 then begin
          let maybe_float =
            try
