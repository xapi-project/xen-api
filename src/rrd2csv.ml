...
module Stdout = struct
  ...
  let time_of_float x =
    ...
    Printf.sprintf "%04d-%02d-%02dT%02d:%02d:%02dZ" (time.Unix.tm_year + 1900)
      (time.Unix.tm_mon + 1) time.Unix.tm_mday time.Unix.tm_hour
  ...
  let string_of_float flt =
    if fst (modf flt) = 0. then
      Printf.sprintf "%.0f" flt
    else
      Printf.sprintf "%f" flt
end
...
module XAPI = struct
  ...
  let rec retry_with_session f x =
    let session =
      let rec aux () =
        try
          Client.Session.login_with_password ~rpc ~uname:"" ~pwd:""
            ~version:"1.4" ~originator:"rrd2csv"
  ...
  let to_string ?(escaped = false) ds_s =
    ...
    let string_repr =
      Printf.sprintf "%s:%s:%s:%s"
        ...
        ( try
            match Option.get ds_s.owner with
            | VM _ ->
                "vm"
            | Host ->
                "host"
    ...
    let string_repr =
      Printf.sprintf "%s:%s:%s (%s):%s"
        ...
        ( try
            match Option.get ds_s.owner with
            | VM _ ->
                get_vm_name_label ds_s.uuid
            | Host ->
                get_host_name_label ds_s.uuid
            | SR _ ->
                ""
          with _ -> ""
        )
...
module RRDD = struct include Rrd_client.Client end
...
module Xport = struct
  ...
  *)
  ...
  type update = {timestamp: int64; values: float array}

  type t = {header: meta; data: update list}
  ...
  let from_xml (input : Xmlm.input) =
    ...
    let tree =
      ...
      | `Dtd _ ->
          snd (Xmlm.input_doc_tree ~data ~el input)
      | _ ->
    ...
    let find_elt (key : string) (elts : xml_tree list) =
      match
        List.find (function El (k, _elts) -> k = key | _ -> false) elts
      with
      | El (_k, elts) ->
          elts
      | D _ ->
    ...
    let process_row (elts : xml_tree list) =
      let ts =
        match find_elt "t" elts with
        | [D value] ->
            Int64.of_string value
        | _ ->
            raise (Parse_error "process_row/ts")
      in
      let values =
        List.fold_left
          (function
    ...
    let process_root = function
      | El ("xport", elts) ->
          process_xport elts
      | El _ | D _ ->
          raise (Parse_error "process_root")
    in
  ...
  module Assoc_list = struct
    ...
    *)
    let to_csv ?data_sources (last_values : values_list) =
      let assoc_value ds =
        Stdout.string_of_float
          (try List.assoc ds last_values with Not_found -> nan)
      in
      let filtered_values =
        match data_sources with
        | None ->
            List.map (fun (_, v) -> Stdout.string_of_float v) last_values
        | Some dss ->
            List.map assoc_value dss
      in
  ...
  *)
  let get_update session_id =
    let gen_query ?(host = false) ?(backstep = 10.) ?cf ?vm session =
      ...
      let qstring =
        Printf.sprintf "/rrd_updates?session_id=%s&start=%.0f"
          (Ref.string_of session) (now -. backstep)
      in
      let qstring =
        Option.fold ~none:qstring
          ~some:(fun vm_uuid -> qstring ^ "&vm_uuid=" ^ vm_uuid)
          vm
      in
      let qstring =
    ...
    in
    let transport =
      Xmlrpc_client.Unix (Filename.concat "/var/lib/xcp" "xapi")
    in
    let request_url = gen_query ~host:true session_id in
    let get_update socket =
      let http_request =
        Http.Request.make ~user_agent:"rrd2csv" Http.Get request_url
      in
  ...
  let get_last_timestamp (update : t) = (List.hd update.data).timestamp
end
...
let print_last session_id data_sources =
  ...
  let csv_of_assoc_list =
    Xport.Assoc_list.to_csv ~data_sources assoc_list_of_update
  in
...
let main session_id user_filters sampling_period show_name show_uuid =
  ...
  List.iter
    (fun ds ->
      if
        filter_ds_that_starts_with_name init_update.header.entries
...
let _ =
  ...
  *)
  let user_filters, sampling_period, show_name, show_uuid =
    ...
    let usage =
      Printf.sprintf "%s [-s sampling-period] [datasource_selector]*" command
    in
    let specs =
      Arg.align
        [
