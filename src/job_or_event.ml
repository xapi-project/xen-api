...
module Event_is_block : sig end = struct
  open Types
  open Event
  type _t = t = (* must never be immediate *)
...
module Job_is_not_block : sig end = struct
  module Ensure_private_int (M : sig type t = private int end) = struct type _t = M.t end
  include Ensure_private_int (Job)
end

let of_event event = (Obj.magic (event : Event.t) : t)
let of_job   job   = (Obj.magic (job   : Job.t)   : t)
...
module Match = struct
  ...
  let project (type a) (_ : a kind) job_or_event =
    (Obj.magic (job_or_event : t) : a)
  ;;
end
