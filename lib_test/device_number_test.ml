...
let device_number = Alcotest.testable (Fmt.of_to_string Device_number.to_debug_string) (=)
  ...
  let tests =
    List.map
      (fun (_, linux, xenstore) ->
         "test_deprecated " ^ linux, `Quick, (fun () ->
             let of_linux = of_linux_device linux in
             let of_xenstore = of_xenstore_key xenstore in
             Alcotest.check device_number "must be equal"
...
let test_2_way_convert =
  ...
  in
  let device_number_equal_linux = Alcotest.testable (Fmt.of_to_string Device_number.to_debug_string) equal_linux in
  ["test_2_way_convert", `Slow, (fun () ->
       for disk_number = 0 to ((1 lsl 20) -1) do
         List.iter
           (fun hvm ->
              ...
              Alcotest.check device_number_equal_linux "of_linux must be equal to original"
                original of_linux;
