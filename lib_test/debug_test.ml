...
let assert_levels brand (crit, err, warn, info, debug) =
  Alcotest.(check bool) (brand ^ " critical") (Debug.is_disabled brand Syslog.Crit)    crit;
  Alcotest.(check bool) (brand ^ " error") (Debug.is_disabled brand Syslog.Err)     err;
  Alcotest.(check bool) (brand ^ " warning") (Debug.is_disabled brand Syslog.Warning) warn;
  Alcotest.(check bool) (brand ^ " info") (Debug.is_disabled brand Syslog.Info)    info;
  Alcotest.(check bool) (brand ^ " debug") (Debug.is_disabled brand Syslog.Debug)   debug
...
let tests =
  ...
  [
    ...
    test_case "Test Debug.set_level (multiple loggers)" `Quick test_debug_set_level_multiple_loggers;
  ]