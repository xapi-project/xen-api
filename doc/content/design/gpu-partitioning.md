...
that should be available against what the card admits to. Admission recomputes that
verdict rather than reading a stored one, so a wedged card stops receiving new VMs:
a stale "wedged" turns a transient fault into permanently lost capacity, and a stale
"healthy" makes every placement on that card fail. A card whose geometry could not be
read is neither, and admission declines to decide rather than guessing either way.
`PGPU.requires_reset` is the conclusion drawn from that verdict rather than the
verdict itself — xapi sets it once a reset is warranted, and it is what an operator
reads. That is why it is a boolean while the verdict behind it is not.
...
| `xe pgpu-get-remaining-capacity uuid=<uuid> vgpu-type-uuid=<uuid>` | Free partitions of that type |
