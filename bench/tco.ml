...
let bind_upon t f = ignore (t >>= (fun a -> f a; Deferred.unit)) ;;
...
let pure_loop n = Deferred.create (fun i ->
  let rec f = function
    | 0 -> Ivar.fill i ()
    | n -> f (n-1)
  in f n) ;;
...
let slow_upon_loop n = Deferred.create (fun i ->
  let rec f = function
    | 0 -> Ivar.fill i ()
    | n -> bind_upon tick (fun () -> f (n-1))
  in f n) ;;
let upon_loop n = Deferred.create (fun i ->
  let rec f = function
    | 0 -> Ivar.fill i ()
...
let () =
  loop 2000000 >>> fun () -> Shutdown.shutdown 0 ;;
