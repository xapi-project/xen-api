		...
		if (arguments.length > 1) {
			...
			if (second_arg.constructor == String) {
				...
				var attributes = arguments[1];
			        var elt;
 			        if(attributes && attributes.ns && document.createElementNS) {elt=document.createElementNS(attributes.ns,first_arg)} else {elt=document.createElement(first_arg)}
				
				for (var attr in attributes) {
				    if(attr=="events") {
					var events = attributes['events']
					for(var event in events) {
					    $(elt).bind(event,events[event]);
					}
				    } else if(attr!="ns") {					
