...
module Suspend_restore_xenguest: SUSPEND_RESTORE = struct
  ...
  let write_libxc_record' (task: Xenops_task.task_handle) ~xc ~xs ~domain_type ~dm manager_path domid uuid fd flags progress_callback qemu_domid do_suspend_callback =
    let fd_uuid = Uuid.(to_string (create `V4)) in
    let mode =
      match domain_type with
      | `hvm | `pvh -> "hvm_save"
  ...
  let write_libxc_record (task: Xenops_task.task_handle) ~xc ~xs ~domain_type ~dm  manager_path domid uuid fd flags progress_callback qemu_domid do_suspend_callback =
    Device.Dm.with_dirty_log dm domid ~f:(fun () ->
        write_libxc_record' task ~xc ~xs ~domain_type ~dm manager_path domid uuid fd flags progress_callback qemu_domid do_suspend_callback
      )
  ...
  *)
  let suspend (task: Xenops_task.task_handle) ~xc ~xs ~domain_type ~dm ~manager_path vm_str domid main_fd vgpu_fd flags ?(progress_callback = fun _ -> ()) ~qemu_domid do_suspend_callback =
    ...
    let res =
      ...
      debug "Writing Libxc record";
      write_libxc_record task ~xc ~xs ~domain_type ~dm manager_path domid uuid main_fd flags
        progress_callback qemu_domid do_suspend_callback;
      (* Qemu record (if this is a hvm domain) *)
      (* Currently Qemu suspended inside above call with the libxc memory
       * image, we should try putting it below in the relevant section of the
       * suspend-image-writing *)
      (if domain_type = `hvm then write_qemu_record domid uuid legacy_libxc main_fd else return ()) >>= fun () ->
      debug "Qemu record written";
...
let suspend (task: Xenops_task.task_handle) ~xc ~xs ~domain_type ~dm ~xenguest_path ~emu_manager_path vm_str domid fd vgpu_fd flags ?(progress_callback = fun _ -> ()) ~qemu_domid do_suspend_callback =
  let suspend =
    if use_emu_manager () then
  ...
  in
  suspend task ~xc ~xs ~domain_type ~dm vm_str domid fd vgpu_fd flags ~progress_callback ~qemu_domid do_suspend_callback
