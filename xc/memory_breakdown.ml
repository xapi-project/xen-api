...
let cli_description =
  "Displays a breakdown of host and guest memory usage.\n\n\
   When run with no arguments, memory_breakdown periodically prints a single \
   line to stdout containing memory data for the host and all currently active \
   domains.\n\n\
...
let memory_target_path = Printf.sprintf "/local/domain/%s/memory/target"
...
let xs_read_bytes_from_kib_key xs path =
  match xs_read xs path with
  | None ->
      "n/a"
  | Some string ->
...
let print_memory_field_names () =
  let host_field_names = host_field_names in
  let guest_field_names =
    List.map (fun n -> "[" ^ n ^ "...]") guest_field_names
  in
  print_string "| " ;
...
let print_memory_field_values xc xs =
  ...
  let print_host_info field =
    print_string " " ;
    print_string (field host)
  in
  let print_guest_info field =
    print_string " | " ;
    print_string (String.concat " " (List.map (field xc xs) guests))
  in
  print_string "|" ;
...
let record_new_data () =
  ...
  Xenops_helpers.with_xc_and_xs (fun xc xs ->
      while true do
