...
let hashtbl_of_domaininfo (x : Xenctrl.domaininfo) : (string, string) Hashtbl.t
  ...
  let state =
    ...
    bool "D" x.Xenctrl.dying
    ^ bool "S" x.Xenctrl.shutdown
    ^ bool "P" x.Xenctrl.paused
    ^ bool "B" x.Xenctrl.blocked
    ^ bool "R" x.Xenctrl.running
    ^ bool "H" x.Xenctrl.hvm_guest
  in
  Hashtbl.add table "state" state ;
  Hashtbl.add table "shutdown code" (int x.Xenctrl.shutdown_code) ;
  Hashtbl.add table "tot bytes"
    (pages_to_string_bytes x.Xenctrl.total_memory_pages) ;
  Hashtbl.add table "tot pages"
    (pages_to_string_pages x.Xenctrl.total_memory_pages) ;
  Hashtbl.add table "tot MiB"
    (pages_to_string_mib_used x.Xenctrl.total_memory_pages) ;
  Hashtbl.add table "max bytes"
    ( if x.Xenctrl.domid = 0 then
        "N/A"
    else
      pages_to_string_bytes x.Xenctrl.max_memory_pages
    ) ;
  Hashtbl.add table "max pages"
    ( if x.Xenctrl.domid = 0 then
        "N/A"
    else
      pages_to_string_pages x.Xenctrl.max_memory_pages
    ) ;
  Hashtbl.add table "max MiB"
    ( if x.Xenctrl.domid = 0 then
        "N/A"
    else
      pages_to_string_mib_used x.Xenctrl.max_memory_pages
    ) ;
  ...
  Hashtbl.add table "uuid"
    (Uuid.to_string (Uuid.uuid_of_int_array x.Xenctrl.handle)) ;
  (* Ask for shadow allocation separately *)
  let shadow_mib =
    try
      Some
        (Int64.of_int (Xenctrl.shadow_allocation_get xc_handle x.Xenctrl.domid))
    with _ -> None
  in
