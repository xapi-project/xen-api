...
let print_debug fmt =
  Printf.kprintf (fun s -> if !debug_enabled then output_string stderr s) fmt
...
let readdir ?(full_paths = true) ~xs path =
  ...
  if paths = [""] then
    [] (* empty directory *)
  else if full_paths (* Note "//" is an Invalid path *) then
    List.map (fun x -> (if path = "/" then "" else path) ^ "/" ^ x) paths
  else
    paths
  ...
  | [path] ->
      ...
      List.iter
        (fun path ->
          let perm =
            catch_invalid
              (Printf.sprintf "getperms %s" path)
              xs.Xs.getperms path
          in
          print_string "    " ;
          print_string path ;
...
let read ~xs = function
  | [path] ->
      ...
      print_endline contents
  | _ ->
      failwith "read takes exactly one argument"

let rm ~xs = function
  | [path] ->
      xs.Xs.rm path
  | _ ->
      failwith "rm takes exactly one argument"

let mkdir ~xs = function
  | [path] ->
      xs.Xs.mkdir path
  | _ ->
      failwith "mkdir takes exactly one argument"
...
let watch ~xs = function
  ...
  | _ ->
      failwith "watch takes exactly one argument"

let write ~xs = function
  | [path; data] ->
      xs.Xs.write path data
  | _ ->
      failwith "write takes a path and a value to write"

let functions =
  [
    ("ls", (ls, "List a directory"))
  ; ("rm", (rm, "Remove a path"))
...
let _ =
  ...
  if not (List.mem_assoc mode functions) then (
